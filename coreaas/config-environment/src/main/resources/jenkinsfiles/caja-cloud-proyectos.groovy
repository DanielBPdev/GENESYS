node {
    ...
    stage('compilación servicios') {
        dir(pwd()+'/coreaas/services') {
            withMaven(jdk: 'JDK1.8', maven: 'maven 3.3.9') {
                sh 'mvn --projects ${proyectos} clean package -U -Pfabric8 -Ddocker.registry.name=${dockerregistry} -Dnombre.usuario=${nombreusuario} -Dpila.database=${piladatabase} -Dsubsidio.database=${subsidiodatabase} -Ddefault.schema=${defaultschema} -fn'
            }
        }
    }
    ...
    stage('build & push servicios') {
        dir(pwd()+'/coreaas/services') {
            withMaven(jdk: 'JDK1.8', maven: 'maven 3.3.9') {
                sh 'mvn --projects ${proyectos} fabric8:build fabric8:push -Pfabric8 -Ddocker.registry.name=${dockerregistry} "-Ddocker.registry.password=${registrykey}" -fn'
            }
        }
    } 
    stage('despliegue servicios') {
        dir(pwd()+'/coreaas/services') {
            withMaven(jdk: 'JDK1.8', maven: 'maven 3.3.9') {
                sh 'gcloud container clusters get-credentials comfa --zone=us-central1-a --project=${kubernetesproject}'
                sh 'kubectl get pods'
                sh 'mvn --projects ${proyectos} fabric8:deploy -Ddocker.registry.name=${dockerregistry} -Pfabric8 -fn'
            }
        }
    }   
	...
	stage('compilación despliegue esb') {
        dir(pwd()+'/coreaas/esb') {
            withMaven(jdk: 'JDK1.8', maven: 'maven 3.3.9') {
                sh 'gcloud container clusters get-credentials comfa --zone=us-central1-a --project=${kubernetesproject}'
                sh 'kubectl get pods'
                sh 'mvn clean package fabric8:build fabric8:push -Ddatabase.name=${databaseNameCore} -Ddatabase.username=${databaseusername} -DdatabaseUserPassword=${databaseUserPassword} -Dservice.port=${serviceport} -Dpila.username=${pilausername} -Dhost.ip=${dbserverpruebasasopagos} -DdatabasePilaPassword=${databasePilaPassword} -Dpila.database=${piladatabase} -Dactivemq.service.host=${activemqservicehost} -Ddocker.registry.name=${dockerregistry} "-Ddocker.registry.password=${registrykey}" -Pfabric8'
                sh 'mvn fabric8:deploy -Ddatabase.name=${databaseNameCore} -Ddatabase.username=${databaseusername} -DdatabaseUserPassword=${databaseUserPassword} -Dservice.port=${serviceport} -Dpila.username=${pilausername} -Dhost.ip=${dbserverpruebasasopagos} -DdatabasePilaPassword=${databasePilaPassword} -Dpila.database=${piladatabase} -Dactivemq.service.host=${activemqservicehost} -Ddocker.registry.name=${dockerregistry} "-Ddocker.registry.password=${registrykey}" -Pfabric8'
            }
        }
    }

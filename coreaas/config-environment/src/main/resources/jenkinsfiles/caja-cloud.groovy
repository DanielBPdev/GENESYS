    ...
    stage('despliegue servicios') {
        dir(pwd()+'/coreaas/services') {
            withMaven(jdk: 'JDK1.8', maven: 'maven 3.3.9') {
                sh 'gcloud container clusters get-credentials ${clusterkubernetes} --zone=${clusterlocation} --project=${kubernetesproject}'
                sh 'kubectl get pods'
                sh 'mvn -T 2C clean install fabric8:build fabric8:push fabric8:resource-apply -DskipTests -Dnombre.usuario=${nombreusuario} -Ddefault.schema=${defaultschema} -Dpila.database=${piladatabase} -Ddocker.registry.name=${dockerregistry} "-Ddocker.registry.password=${registrykey}" -Pdefault,fabric8'
            }
        }
    }   
}

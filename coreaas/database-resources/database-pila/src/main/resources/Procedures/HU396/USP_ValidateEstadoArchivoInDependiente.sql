...
-- Description:	Procedimiento almacenado encargado de calcular el estado del archivo PILA de In/Dependientes, despues de 
-- haber pasado por todos los procesos de validacion de la fase 1 de pila.
-- =============================================
CREATE OR ALTER PROCEDURE [dbo].[USP_ValidateEstadoArchivoInDependiente]
...
SET NOCOUNT ON;
	...
	IF(
	(SELECT COUNT(1) FROM staging.RegistroGeneral WITH (NOLOCK)
	...
	BEGIN
		...
		AND regEsSimulado = 0 --Actualiza el estado unicamente si no es una simulacion
	END
	ELSE
	BEGIN
		UPDATE staging.RegistroGeneral SET regOUTEstadoArchivo = 'PROCESADO_VS_BD',

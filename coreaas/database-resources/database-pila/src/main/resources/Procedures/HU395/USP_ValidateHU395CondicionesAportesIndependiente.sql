...
-- archivo I de PILA, debe ser evaluado como independiente.
-- =============================================
CREATE PROCEDURE [dbo].[USP_ValidateHU395CondicionesAportesIndependiente] 
...
BEGIN TRY 
	...
	SELECT
		redId
		,CASE WHEN ISNULL(redOUTEstadoSolicitante, '') IN ('ACTIVO') THEN 'VALIDAR_COMO_INDEPENDIENTE' ELSE 'NO_VALIDADO_BD' END
		,CASE WHEN ISNULL(redOUTEstadoSolicitante, '') NOT IN ('ACTIVO') THEN 'NO_VALIDADO_BD' END
		,dbo.GetLocalDate()
	...
	WHERE redTipoCotizante IN (Select tipoCotizante FROM #TablaDependientes WITH(NOLOCK))
...
BEGIN CATCH
    ...
    SELECT @ErrorMessage = '[dbo].[USP_ValidateHU395CondicionesAportesIndependiente]|@IdRegistroGeneral=' + CAST(ISNULL(@IdRegistroGeneral,0) AS VARCHAR(20)) + '|' + ERROR_MESSAGE(),

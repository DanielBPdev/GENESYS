...
CREATE OR ALTER   PROCEDURE [dbo].[USP_ExecuteRegistrarRelacionarAportesRegistro]
...
SET NOCOUNT ON;	
	...
	BEGIN
		...
		--================ Se deja comentareado este paso porque si pueden quedar no gestionados en BD desde aporte Manula GLPI 71768
		/*
		IF NOT EXISTS (			
			SELECT TOP 1 1 
	...
	BEGIN
		...
		if exists (select 1 from staging.RegistroGeneral as rg 
						inner join staging.RegistroDetallado as rd on rg.regId = rd.redRegistroGeneral
						inner join TemAporte as t on rg.regId = t.temRegistroGeneral and rd.redId = t.temRegistroDetallado
						where rg.regId = @IdRegistroGeneral and (t.temValTotalApoObligatorio <> rg.regValTotalApoObligatorio and rg.regTipoPlanilla <> 'N' and rd.redOUTTipoAfiliado = 'TRABAJADOR_DEPENDIENTE'))
			begin

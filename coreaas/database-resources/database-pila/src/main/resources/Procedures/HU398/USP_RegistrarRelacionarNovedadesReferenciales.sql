...
-- Description:	Procedimiento almacenado encargado de calcular si una novedad se debe registrar o relacionar.
-- =============================================
CREATE PROCEDURE [dbo].[USP_RegistrarRelacionarNovedadesReferenciales] 
...
SET NOCOUNT ON;
	...
	IF (SELECT COUNT(*) FROM #Novedades WHERE tenRegistroDetallado = @IdRegistroDetalle) = 0 
	BEGIN
		
		DECLARE @Novedades AS TABLE (TipoNovedad VARCHAR(15), Tipotransaccion VARCHAR(100), FechaInicio DATE, FechaFin DATE, SituacionPrimaria BIT, idRegistroNovedad BIGINT)
	
		INSERT INTO @Novedades (TipoNovedad,Tipotransaccion,FechaInicio,FechaFin,SituacionPrimaria,idRegistroNovedad)
		SELECT rdnTipoNovedad, rdnTipotransaccion, rdnFechaInicioNovedad, rdnFechaFinNovedad, 0, rdnId
		...
		SELECT redOUTTipoNovedadSituacionPrimaria, redOUTFechaInicioNovedadSituacionPrimaria, redOUTFechaFinNovedadSituacionPrimaria, 1, 0
		...
		SELECT TipoNovedad,TipoTransaccion,FechaInicio,FechaFin,idRegistroNovedad FROM @Novedades 
		WHERE SituacionPrimaria = 'False' ORDER BY fechaInicio ASC
		...
		BEGIN
			...
			/*IF (@TipoNovedad IN ('NOVEDAD_ING') AND @EsTrabajadorReintegrable = 0)
	        BEGIN
	        	SET @accionNovedad = 'NO_APLICADA'
	        END*/
	
			--print '@estadoCotizante'
			--print @estadoCotizante
			--print '@fechaIngresoCotizante'
			--print @fechaIngresoCotizante
			--print '@periodoAporte'
			--print @periodoAporte
			--print '@TipoNovedad'
			--print @TipoNovedad
			--print '@tipoNovedadReferencia'
			--print @tipoNovedadReferencia
			--print '@fechaInicioNovedad'
			--print @fechaInicioNovedad
			--print '@FechaFinNovedad'
			--print @FechaFinNovedad
			--print '@accionNovedad'
			--print @accionNovedad

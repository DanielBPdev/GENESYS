...
SET @historicoCursor = CURSOR FAST_FORWARD FOR
	SELECT ISNULL(ubi.ubiTelefonoFijo,''), ISNULL(ubi.ubiTelefonoCelular,''), ISNULL(ubi.ubiEmail,'')
	FROM dbo.Ubicacion_AUD ubi
	JOIN dbo.Persona_AUD per ON per.perUbicacionPrincipal = ubi.ubiId

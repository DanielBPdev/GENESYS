...
-- Description:	
-- =============================================
CREATE PROCEDURE USP_GET_BeneficiariosSubsidioUltimoEstado
(
...
BEGIN
	...
	SELECT distinct benAud.benId,benAud.benEstadoBeneficiarioAfiliado, benAud.REV
	FROM Beneficiario_aud benAud	
	JOIN (SELECT ben.benId, MAX(REV) maxRev
			FROM Beneficiario_aud ben WITH(NOLOCK)
			INNER JOIN Revision rev WITH(NOLOCK) ON (rev.revId = ben.REV)
			JOIN #TempBenId t on t.benId = ben.benId
			WHERE rev.revTimeStamp <= @nCorteFinal
			GROUP BY ben.benId
		  ) maxBen ON maxBen.benId = benAud.benId AND maxBen.maxRev = benAud.REV

...
-- Description:	
-- =============================================
CREATE PROCEDURE USP_GET_BeneficiariosSubsidioFechasRetiro
(
...
BEGIN	
	...
	SELECT ben.benId, ben.benFechaRetiro, maxRev.maxRev, ben.benEstadoBeneficiarioAfiliado
	FROM Beneficiario_aud ben
	join #TempBenId t on t.benId = ben.benId
	JOIN (SELECT ben.benId, MAX(ben.REV) maxRev
			FROM Beneficiario_aud ben			
			WHERE ben.benFechaRetiro <= @fechaFin    
			GROUP BY ben.benId

/****** Object:  StoredProcedure [dbo].[USP_GET_HistoricoAsignacionEntregaReintegroMicrodatoFOVIS]    Script Date: 29/07/2024 3:03:39 p. m. ******/
/****** Object:  StoredProcedure [dbo].[USP_GET_HistoricoAsignacionEntregaReintegroMicrodatoFOVIS]    Script Date: 2023-09-04 9:45:01 AM ******/
/****** Object:  StoredProcedure [dbo].[USP_GET_HistoricoAsignacionEntregaReintegroMicrodatoFOVIS]    Script Date: 2023-08-17 11:51:04 AM ******/
...
-- Reporte 5---5433 B
---EXEC [USP_GET_HistoricoAsignacionEntregaReintegroMicrodatoFOVIS] '2023-08-01','2023-08-31'
-- =============================================
CREATE OR ALTER PROCEDURE [dbo].[USP_GET_HistoricoAsignacionEntregaReintegroMicrodatoFOVIS]
(
	@fechaInicial DATE,
	@fechaFinal DATE
)
...
BEGIN TRY
	...
	drop table if exists #TABLE_DATA_SUBSIDIO_TEMP
	drop table if exists #TABLE_DATA_SUBSIDIO_TEMP_II
	...
	DROP TABLE IF EXISTS #Comun
		SELECT
			...
			, pof.pofModalidad
			,year(aaf.aafFechaOficio) as [aafFechaOficioAno]---cambio 20230321 de aafFechaOficio por aafFechaActaAsignacionFovis
		...
		SELECT distinct
			...
			,com.[aafFechaOficioAno] as [Ano de Asignación del Subsidio (Vigencia)]
     ...
     INNER JOIN #Comun as com on com.pofId = pof.pofId
		  WHERE saf.safEstadoSolicitudAsignacion in ('CERRADA')
		    ...
		    AND aaf.aafFechaOficio BETWEEN @fechaInicial AND dateadd(day,1,@fechaFinal)
		...
		WHERE sld.sldEstadoSolicitud in ('LEGALIZACION_Y_DESEMBOLSO_CERRADO')
		 AND lgd.lgdFechaTransferencia BETWEEN @fechaInicial AND dateadd(day,1,@fechaFinal)
		...
		AND solFechaRadicacion BETWEEN @fechaInicial AND dateadd(day,1,@fechaFinal)
		...
		WHERE sol.solTipoTransaccion = 'RENUNCIO_SUBISIDIO_ASIGNADO'
			...
			AND sol.solFechaRadicacion BETWEEN @fechaInicial AND dateadd(day,1,@fechaFinal)
		...
		SELECT DISTINCT 
			...
			PERINTE.perPrimerNombre AS primerNombre,
							...
							CASE 
								...
								WHEN INTEGRANTES.TIPOINTEGRANTE IN ('CONYUGE', 'CONYUGE_HOGAR','CONYUGE_CIUDADOR') THEN '5'
								...
								WHEN (select 1 from CondicionInvalidez ci inner join Beneficiario b on b.benId = ci.coiIdConyugeCuidador
							    inner join Persona p on p.perId = INTEGRANTES.IDPERSONA ) = 1 THEN '25'
								ELSE  '6'
							...
							END AS nivelIngreso,
	CASE 
		WHEN INTEGRANTES.TIPOINTEGRANTE IN ('PRINCIPAL') THEN
			CASE
				...
				'MEJORAMIENTO_VIVIENDA_URBANA',
				'MEJORAMIENTO_VIVIENDA_SALUDABLE')  
				AND (SELECT TOP 1 ISNULL(pasRecursoPrioridad,'RECURSO_POR_APROPIACION')  
					FROM  PostulacionAsignacion 
					WHERE pasPostulacionFovis = POF.pofId AND pasResultadoAsignacion = 'ESTADO_ASIGNADO') = ('RECURSO_POR_APROPIACION') THEN 1
				WHEN final.modalidad IN(
				'ADQUISICION_VIVIENDA_NUEVA_RURAL',
				'ADQUISICION_VIVIENDA_USADA_RURAL',
				...
				'MEJORAMIENTO_VIVIENDA_RURAL') 
					AND (SELECT TOP 1 ISNULL(pasRecursoPrioridad,'RECURSO_POR_APROPIACION')  
						FROM  PostulacionAsignacion 
						WHERE pasPostulacionFovis = POF.pofId 
							AND pasResultadoAsignacion = 'ESTADO_ASIGNADO') = ('RECURSO_POR_APROPIACION') THEN 2
				WHEN (SELECT ISNULL(pasRecursoPrioridad,'')  
					FROM  PostulacionAsignacion 
					WHERE  pasPostulacionFovis = POF.pofId 
						AND pasResultadoAsignacion= 'ESTADO_ASIGNADO' ) = 'RECURSO_SEGUNDA_PRIORIDAD' 
						AND pof.pofModalidad like '%URBAN%' THEN 5
				WHEN (SELECT ISNULL(pasRecursoPrioridad,'')
					FROM  PostulacionAsignacion 
					WHERE pasPostulacionFovis = POF.pofId AND pasResultadoAsignacion= 'ESTADO_ASIGNADO') = 'RECURSO_SEGUNDA_PRIORIDAD' 
						AND pof.pofModalidad like '%RURAL%'  THEN 6	
	END
	ELSE 4
	END AS componente,
							---(SELECT ISNULL(pasRecursoPrioridad,'')  FROM  PostulacionAsignacion WHERE  pasPostulacionFovis = POF.pofId AND pasResultadoAsignacion= 'ESTADO_ASIGNADO'  ) as recu, 
	
							--ISNULL(YEAR(SAF.safFechaAceptacion), YEAR(@FECHA_INICIAL)) AS anioVigencia,
							 final.[Ano de Asignación del Subsidio (Vigencia)]   AS anioVigencia,
							--pof.pofestadoHogar,
							...
							AS valorSubsidio, 
							CASE
								WHEN Movimiento = 'ASIGNACION' AND Integrantes.tipointegrante = 'PRINCIPAL'
								THEN '1'

								WHEN   Movimiento = 'RENUNCIA'AND Integrantes.tipointegrante = 'PRINCIPAL'
								THEN '2'
								
								WHEN Movimiento = 'VENCIMIENTO' AND Integrantes.tipointegrante = 'PRINCIPAL'
								THEN '3'

								WHEN  Movimiento = 'REINTEGRO'AND Integrantes.tipointegrante = 'PRINCIPAL'
								THEN '4'

								WHEN  Movimiento = 'INDEXADO'AND Integrantes.tipointegrante = 'PRINCIPAL'
								THEN '5'

								WHEN Movimiento = 'DESEMBOLSO'AND Integrantes.tipointegrante = 'PRINCIPAL'
								THEN '6'
								ELSE 
								'9'
							END   
			 ...
			 INNER JOIN #Final final ON final.pofid = pof.pofId  
					...
					---cambio 20230227 se adiciona tabla parametrica para los salarios minimos de cada Ano
 	...
 	------------Se obtiene informacion del estado-------
		 ...
		 ---	 SELECT '#TABLE_DATA_SUBSIDIO_TEMP	',* FROM #TABLE_DATA_SUBSIDIO_TEMP	
	 ...
	 INSERT INTO rno.HistoricoAsignacionEntregaReintegroMicrodatoFOVIS 
		...
		FROM(
			...
			SELECT 
				...
				CASE
					WHEN consolidado.componenteHogar = '2' THEN '8'
					ELSE '7'
...
--ALTER TABLE rno.HistoricoAsignacionEntregaReintegroMicrodatoFOVIS ADD hamCodigoTipoPlanVivienda int, hamFuenteFinanciamiento int
			SELECT 
					...
					CASE WHEN (select distinct 1 from Persona p inner join Afiliado a on p.perId = a.afiPersona
					inner join GrupoFamiliar gf on gf.grfAfiliado = a.afiId inner join Beneficiario b on b.benGrupoFamiliar = gf.grfId
					inner join Persona pb on pb.perId = b.benPersona inner join CondicionInvalidez ci on ci.coiIdConyugeCuidador = pb.perId
					where b.benTipoBeneficiario = 'CONYUGE' AND B.benEstadoBeneficiarioAfiliado = 'ACTIVO'
					and pb.perNumeroIdentificacion = hamNumeroIdIntegrante) = 1 THEN '25' ELSE CONVERT(BIGINT,hamParentezcoIntegrante) END AS 'Parentesco de la persona con el titular',

...
-- =============================================
CREATE PROCEDURE USP_GET_HistoricoAsignacionEntregaReintegroFOVIS
(
	@fechaInicio DATE,
	@fechaFin DATE,
	@historico BIT = NULL
...
BEGIN TRY
	...
	SET @sql = '
			...
			AND  DATEADD(SECOND, rev.revTimeStamp / 1000, ''19700101'') AT TIME ZONE ''UTC'' AT TIME ZONE ''SA Pacific Standard Time'' BETWEEN @fechaInicio AND @fechaFin
			'	
	...
	IF @historico = 1
	BEGIN
		INSERT rno.HistoricoAsignacionEntregaReintegroFOVIS(
				hapFechaHistorico,
				hapFuenteFinanciamiento,
				hapTipoPlanVivienda,
				hapCodigoMunicipio,
				hapGenero,
				hapRangoEdad,
				...
				hapEstadoSubsidio,
				hapAnioVigencia,
				hapCantidadSubsidios,
				hapValorSubsidios,
				hapFechaInicialReporte,
				hapFechaFinalReporte)
		SELECT @fechaFin,
		cons.fuenteFinanciamiento,
		...
		FROM (SELECT 
			...
			END AS genero,
			CASE 
				WHEN DATEDIFF(YEAR, ped.pedFechaNacimiento, dbo.GetLocalDate()) < 1
				THEN 11
				WHEN DATEDIFF(YEAR, ped.pedFechaNacimiento, dbo.GetLocalDate()) BETWEEN 1 AND 2
				THEN 12
				WHEN DATEDIFF(YEAR, ped.pedFechaNacimiento, dbo.GetLocalDate()) BETWEEN 3 AND 6
				THEN 13
				WHEN DATEDIFF(YEAR, ped.pedFechaNacimiento, dbo.GetLocalDate()) BETWEEN 7 AND 9
				THEN 14
				WHEN DATEDIFF(YEAR, ped.pedFechaNacimiento, dbo.GetLocalDate()) BETWEEN 10 AND 12
				THEN 15
				WHEN DATEDIFF(YEAR, ped.pedFechaNacimiento, dbo.GetLocalDate()) BETWEEN 13 AND 15
				THEN 16
				WHEN DATEDIFF(YEAR, ped.pedFechaNacimiento, dbo.GetLocalDate()) BETWEEN 16 AND 17
				THEN 17
				WHEN DATEDIFF(YEAR, ped.pedFechaNacimiento, dbo.GetLocalDate()) BETWEEN 18 AND 21
				THEN 18
				WHEN DATEDIFF(YEAR, ped.pedFechaNacimiento, dbo.GetLocalDate()) BETWEEN 22 AND 28
				THEN 19
				WHEN DATEDIFF(YEAR, ped.pedFechaNacimiento, dbo.GetLocalDate()) BETWEEN 29 AND 49
				THEN 20
				WHEN DATEDIFF(YEAR, ped.pedFechaNacimiento, dbo.GetLocalDate()) BETWEEN 50 AND 59
				THEN 21
			...
			JOIN ( SELECT pof.pofId,
				...
				LEFT JOIN SolicitudAsignacion saf ON (pof.pofSolicitudAsignacion = saf.safId)
				LEFT JOIN ActaAsignacionFovis aaf ON (saf.safId = aaf.aafSolicitudAsignacion AND aaf.aafIdentificadorDocumentoConsolidado IS NOT NULL)
				LEFT JOIN (

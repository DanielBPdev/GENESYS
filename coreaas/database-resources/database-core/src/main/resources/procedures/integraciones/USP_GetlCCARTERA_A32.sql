-- =============================================
-- Author:      <Author, , Camilo Gomez>
-- Create Date: <Create 23,11,2022 >
-- Description: <Description,Convenios de pago Cartera A32, >
-- =============================================
CREATE OR ALTER PROCEDURE [sap].[USP_GetlCCARTERA_A32] 	@FechaInicial date, @FechaFinal date
As
...
CREATE TABLE #IC_Aportes_Enc
	...
	(
		...
		[usuario] [varchar](50) NOT NULL,
		[deudaconv] NUMERIC(21,5) NOT NULL
		)
		Create table #IC_SaldoCarteraConvenio(
		[idSaldoConvenio] [int] not null primary key identity(1,1),
		[saldoActualConvenio] [bigint] Not Null,
...
ruta Varchar (max)
 ...
 DECLARE @SQL NVARCHAR(MAX)
	...
	Select @SaldoAnterior = saldo.[saldoActualConvenio] from ( Select top 1 [idSaldoConvenio], [saldoActualConvenio] from sap.IC_SaldoCarteraConvenio order by [idSaldoConvenio] desc) as saldo
	...
	--------se alimenta temporal con la clave 40
				INSERT INTO #IC_Aportes_Det ([consecutivo],[codigoSap],[importeDocumento],[operador],[claveCont],[asignacion],[ref1],[tipoDocumento],[ref3],[noIdentificado],[adelantado],
				...
				[codigoGenesys],[tipo],[claseCuenta],[usuario],[deudaconv])

				SELECT distinct 
				1 consecutivo,
				'' codigosap, --pendiente dice que aplica pero no se puede traer porque no va ref1 ni ref 2 
				isnull(ABS(sum(convert(numeric(19,0),[Valor Convenio Pago]))- @SaldoAnterior),0) importedocumento,
				case when sum(convert(numeric(19,0),[Valor Convenio Pago])- @SaldoAnterior) < 0 then '-' else '' end as operador,
				'40' clavecont, 
				...
				'' tipoInteres,
				'0' correccion,
				'MC' tipoMora, --validar que valor se envia
				'0' id,
				' ' codigoGenesys, --validar por lo mismo de codigo sap
				'' tipo, --validar por lo mismo de codigo sap
				'S' clasecuenta,
				'Integracíones' usuario,
				isnull(sum(convert(numeric(19,0),[Valor Convenio Pago])),0)deudaconv   

				FROM #cartera
				WHERE [FechaInicial] = @FechaInicial
				and [FechaFinal]  = @FechaFinal
				
				--------------------------------- actualizar en caso de que traiga valor en 0 ---------------------
				update #IC_Aportes_Det set importeDocumento =  case  when importeDocumento = '0' then isnull(@SaldoAnterior,0)ELSE importeDocumento END,
											operador = 	case  when importeDocumento = '0' Then '-'	ELSE operador END			
	...
	--------se alimenta temporal con la clave 50		
				...
				[identificadorDelBanco],[codigoBanco],[transitoria],[claseDeAporte],[claseDeAportePrescripcion],[tieneIntereses],[tipoInteres],[correccion],[tipoMora],[id],
				' ' as [codigoGenesys],[tipo],[claseCuenta],[usuario],deudaconv from #IC_Aportes_Det
	...
	------------------- Inicio Encabezado -------------------------------------
				...
				INSERT INTO #IC_SaldoCarteraConvenio
				([saldoActualConvenio], [salfechaContabilizacion],[consecutivo])
				
				Select deudaconv ,CONVERT(varchar, SAP.GetLocalDate(), 111) AS salfechaContabilizacion,
				'1' as consecutivo from #IC_Aportes_Det where claveCont = '40'

IF (Select importeDocumento from #IC_Aportes_Det where claveCont = '40') <> 0
	BEGIN	
...
--Fin comentario
	...
	--	UPDATE #IC_SaldoCarteraConvenio
	--	SET consecutivo = consecutivo + @consecutivo
...
--Comentarear
		...
		INSERT INTO sap.IC_SaldoCarteraConvenio
		Select [saldoActualConvenio],[salfechaContabilizacion],[consecutivo]
		From #IC_SaldoCarteraConvenio
...
--Fin comentario
				...
				--select * from #IC_SaldoCarteraConvenio
		
				drop table  #IC_Aportes_Det
				drop table #IC_Aportes_Enc
				drop table #IC_SaldoCarteraConvenio
	END		

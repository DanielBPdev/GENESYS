...
-- =====================================================================================================
CREATE OR ALTER PROCEDURE [sap].[USP_GetICFOVIS_Insert_F11_F12]
...
-- Declaracion de variables para el calculo de informacion de desembolsoDECLARE @idOferente AS VARCHAR(10), @psvid AS VARCHAR(10), @idbanco AS INT, @tipocuenta AS varchar(20), @medioTransferencia AS VARCHAR(800);
...
(
	...
	--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	-- F11: Pago por traslado anticipados a proyectos de vivienda rural/urbana
	--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
BEGIN TRY 
	...
	SELECT DISTINCT TOP 1	-- Ordena por el mas reciente y seleccciona el primer registro, cuando se encuentraan varios pagos parciales
	...
	AND pof.pofEstadoHogar IN ('SUBSIDIO_LEGALIZADO','SUBSIDIO_CON_ANTICIPO_DESEMBOLSADO') AND pof.pofMotivoDesistimiento IS NULL
	...
	WHERE  sol.solTipoTransaccion  LIKE  '%ADQUISICION_VIVIENDA%'
	AND YEAR(sol.solFechaRadicacion) > 2023 -- ENERO 17: REVISAR ESTE FILTRO DEL AÑO
	...
	ORDER BY sol.solNumeroRadicacion DESC		-- Toma solo la ultima radicacion de pago por anticipo
	...
	IF EXISTS (select * FROM #IC_FOVIS_Det) BEGIN
		...
		SELECT revTimeStamp, ''
		FROM core.aud.PostulacionFOVIS_aud WITH (NOLOCK) 
		...
		IF NOT EXISTS (SELECT * FROM #revTimeStamp) BEGIN
			...
			@stmt = N'SELECT revTimeStamp FROM PostulacionFOVIS_aud INNER JOIN Revision on revid = rev WHERE pofEstadoHogar IN (''SUBSIDIO_LEGALIZADO'',''SUBSIDIO_CON_ANTICIPO_DESEMBOLSADO'') AND pofid = @pofid',
...
select @idOferente = (select idOferente from openjson(@json, '$.oferenteLegalizacion.oferente')
		...
		ELSE BEGIN
			...
			--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			-- FIN F11: Pago por traslado anticipados a proyectos de vivienda rural/urbana
			--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
BEGIN 
 ...
 UPDATE #IC_FOVIS_Det SET ref1 = per2.perNumeroIdentificacion , tipoDocumento = mi.tipoDocumentoCaja, 
		...
		INNER JOIN core.dbo.Afiliado afi WITH (NOLOCK)  ON jeh.jehAfiliado = afi.afiId
		INNER JOIN core.dbo.Persona per WITH (NOLOCK)  ON afi.afiPersona = per.perid
		INNER JOIN core.dbo.PersonaDetalle ped WITH (NOLOCK)  ON per.perId = ped.pedPersona AND ped.pedFallecido = 1
		INNER JOIN core.dbo.IntegranteHogar inh WITH (NOLOCK)  ON jeh.jehid = inh.inhJefeHogar AND inh.inhIntegranteReemplazaJefeHogar = 1
...
------------------------------------------------------------------------------------------------------
	...
	END
			...
			--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			-- INICIO DE MOVIMIENTO CONTABLE F12
			--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		
			INSERT INTO #IC_FOVIS_Enc_F12([fecIng],[horaIng],[fechaDocumento],[fechaContabilizacion],[periodo]
				,[referencia],[tipoMovimiento],[observacion],[moneda],[documentoContable],[sociedad],[ejercicio]
				,[nroIntentos],[fecProceso],[horaProceso],[estadoReg], usuario, consecutivoTemp, Componente)
			SELECT [fecIng],[horaIng],[fechaDocumento],[fechaContabilizacion],[periodo]
				,[referencia],'F12',[observacion],[moneda],[documentoContable],[sociedad],[ejercicio]
				,[nroIntentos],[fecProceso],[horaProceso],[estadoReg], usuario, consecutivoTemp + 1, Componente
			FROM #IC_FOVIS_Enc;
	
			INSERT INTO #IC_FOVIS_Det_F12([consecutivo],[codigoSap],[claveCont],[claseCuenta],[importeDocumento],[ref1],[tipoDocumento],[ref3],[tipoSector]
				,[anulacion],[idProyecto],[nombreProyecto],[proyectoPropio],[rendimientoFinanciero],[asignacion],[clavePaisBanco],[claveBanco]
			...
			-- ACTUALIZACION DE MOVIMIENTOS CONTABLES, CLASE CUENTA Y CAMPOS PARA F12

			UPDATE #IC_FOVIS_Det_F12
			SET ClaveCont = 19, 
			claseCuenta = 'D',
			tipoSector = '',
			clavePaisBanco = '',
			...
			UPDATE #IC_FOVIS_Det_F12
			SET codigoSap = (SELECT codigoSAP FROM [sap].[CodSAPGenesysDeudor] WHERE codigoGenesys = (SELECT codigoGenesys FROM #IC_FOVIS_Det_F12 where claveCont = 19 ))

			UPDATE #IC_FOVIS_Det_F12
			SET ClaveCont = 40, 
			claseCuenta = 'S', 
			codigoSap = '',
			ref1 = (SELECT ref1 FROM #IC_FOVIS_Det_F12 WHERE claveCont = 19), 
			ref3 = (SELECT ref3 FROM #IC_FOVIS_Det_F12 WHERE claveCont = 19),
			tipoDocumento = (SELECT tipoDocumento FROM #IC_FOVIS_Det_F12 WHERE claveCont = 19),
			tipoSector = '',
			clavePaisBanco = '',
			claveBanco = '',
			nroCuentaBancaria = '',
			...
			tipoBancoInter = '',
			referenciaBancoCuenta = '',
			codigoGenesys = (SELECT codigoGenesys FROM #IC_FOVIS_Det_F12 WHERE claveCont = 19),
			tipo = (SELECT tipo FROM #IC_FOVIS_Det_F12 WHERE claveCont = 19)
			WHERE claveCont = 31
...
BEGIN 
		...
		UPDATE #IC_FOVIS_Det_F12 SET  codigoSap = (SELECT deu.codigoSAP 	 FROM core.dbo.jefehogar jeh  WITH (NOLOCK) 
		INNER JOIN core.dbo.Afiliado afi WITH (NOLOCK)  ON jeh.jehAfiliado = afi.afiId
		INNER JOIN core.dbo.Persona per WITH (NOLOCK)  ON afi.afiPersona = per.perid
		...
		INNER JOIN core.dbo.Persona per2 WITH (NOLOCK)  ON inh.inhPersona = per2.perId
		INNER JOIN  core.[sap].[CodSAPGenesysDeudor] deu WITH (NOLOCK)  ON per2.perId = deu.codigoGenesys
...
END
			...
			--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			-- INICIO INSERCION DE REGISTROS EN LAS TABLAS DE INTEGRACIoN F12
			--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

			-- Calcular el valor del campo referencia para el F12
			SELECT @referenciaNum = valorActual 
			FROM sap.IC_Referencia 
			WHERE comentario = 'PT'
			AND estado = 'A';
	
			UPDATE #IC_FOVIS_Enc_F12
			SET referencia = @referenciaNum;
	...
	---Determina si es el pago que completa el 100% del subsidio asignado y asi saber si se ejecuta el F12
...
BEGIN
	...
	END	
		...
		END
		 ...
		 --ELSE
			--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			-- FIN INSERCION DE REGISTROS EN LAS TABLAS DE INTEGRACIoN F12
			--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

			--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			-- FIN DE MOVIMIENTO CONTABLE F12
			--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

		-- Detalle
		SELECT * FROM #IC_FOVIS_Enc;
		SELECT * FROM #IC_FOVIS_Det;
		SELECT * FROM #IC_FOVIS_Enc_F12;
		SELECT * FROM #IC_FOVIS_Det_F12;
		SELECT * FROM #IC_FOVIS_Enc_F22;
		SELECT * FROM #IC_FOVIS_Det_F22;
	
		DROP TABLE #IC_FOVIS_Enc;
		DROP TABLE #IC_FOVIS_Det;
		DROP TABLE #IC_FOVIS_Enc_F12;
		DROP TABLE #IC_FOVIS_Det_F12;
		DROP TABLE #IC_FOVIS_Enc_F22;
		DROP TABLE #IC_FOVIS_Det_F22;
...
BEGIN CATCH
	...
	,ERROR_STATE() AS ErrorState,ERROR_PROCEDURE() AS ErrorProcedure  
    	,ERROR_LINE() AS ErrorLine,ERROR_MESSAGE() AS ErrorMessage;  
			...
			SELECT  GETDATE() -'05:00:00', @solNumeroRadicacion, @documentoId, @tipoDocumento,'F11_F12', ERROR_NUMBER(),ERROR_SEVERITY(),ERROR_STATE(),ERROR_PROCEDURE(),ERROR_LINE(),ERROR_MESSAGE()

...
-- Modificado por Yesika Bernal, adicion tabla generacion de errores
-- =====================================================================================================
CREATE OR ALTER PROCEDURE [sap].[USP_GetICFOVIS_Insert_F01] 
...
(
	...
	--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	-- F01: Registro Renuncia a subsidio de vivienda
	--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
BEGIN TRY 
	BEGIN TRANSACTION
	...
	SELECT  DISTINCT 
		...
		INNER JOIN SolicitudNovedadFovis snf WITH (NOLOCK) ON sol.solid = snf.snfSolicitudGlobal AND snf.snfEstadoSolicitud = 'NOV_FOVIS_CERRADA'
		...
		INNER JOIN PostulacionFOVIS pof WITH (NOLOCK) ON pof.pofId = spf.spfPostulacionFovis AND pof.pofEstadoHogar IN ('RENUNCIO_A_SUBSIDIO_ASIGNADO')
		...
		WHERE  YEAR(sol.solFechaRadicacion) >= 2022
	...
	IF EXISTS (select * FROM #IC_FOVIS_Det) BEGIN
		...
		SET asignacion = (
				...
				WHERE pof.pofEstadoHogar IN ('RENUNCIO_A_SUBSIDIO_ASIGNADO') --F01
				AND per.perNumeroIdentificacion = @documentoId
				AND per.perTipoIdentificacion = @tipoDocumento
				AND pofid = @pofID)
		...
		-- INICIO: Calcular la fecha de contabilizacion (FECHA DE APROBACION) desde la base de datos de auditoria
		...
		WHERE pofEstadoHogar = 'RENUNCIO_A_SUBSIDIO_ASIGNADO' 
		AND pofId = @pofID 

		IF NOT EXISTS (SELECT * FROM #revTimeStamp) BEGIN
			INSERT INTO #revTimeStamp
			EXEC sp_execute_remote @data_source_name = N'CoreAudReferenceData',  
			@stmt = N'SELECT revTimeStamp FROM PostulacionFOVIS_aud INNER JOIN Revision on revid = rev WHERE pofEstadoHogar = ''RENUNCIO_A_SUBSIDIO_ASIGNADO'' AND pofid = @pofid',
		...
		--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		-- FIN F01: Registro Renuncia a subsidio de vivienda
		--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
BEGIN CATCH
    ...
    ,ERROR_LINE() AS ErrorLine      ,ERROR_MESSAGE() AS ErrorMessage;  
		...
		INSERT INTO sap.LogErrorFovis 
			SELECT  GETDATE() -'05:00:00', @solNumeroRadicacion, @documentoId, @tipoDocumento,'F01', ERROR_NUMBER(),ERROR_SEVERITY(),ERROR_STATE(),ERROR_PROCEDURE(),ERROR_LINE(),ERROR_MESSAGE()

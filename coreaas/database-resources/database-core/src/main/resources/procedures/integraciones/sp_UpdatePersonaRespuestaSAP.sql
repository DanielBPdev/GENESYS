CREATE OR ALTER PROCEDURE [sap].[sp_UpdatePersonaRespuestaSAP] (
	@consecutivo AS INT,
...
BEGIN
	...
	FROM   SAP.Personas_G2E p WITH (NOLOCK)
	WHERE  p.consecutivo = @consecutivo;
	
	IF @estadoreg = 'S'
		EXEC SAP.sp_HomologarCodSAPGenesysDeudor 'P', @CodigoGenesys, @codigosap, @estado = @estadoreg OUTPUT, @obs = @observacion OUTPUT;
	...
	UPDATE SAP.Personas_G2E SET fecproceso = @FecProceso, 
	...
	/*
	IF @estadoreg = 'E' AND (@observacion LIKE '%La cuenta%siendo tratada%usuario%' OR @observacion LIKE '%Cuenta bloqueada por%')
		UPDATE SAP.Personas_G2E SET estadoreg = 'P', 
		                            fecproceso = NULL, 
									horaproceso = NULL,
	...
	ELSE
		UPDATE SAP.Personas_G2E SET fecproceso = [SAP].GetLocalDate(), 

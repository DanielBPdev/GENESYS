...
-- Actualizado: SEPTIEMBRE 22 DE 2023
-- Description: Cambio de condiciones,
-- =====================================================================================================
 
CREATE OR ALTER  PROCEDURE  [sap].[USP_InterfazCM_C02] @liq varchar(30),@tipdoc varchar(20),@doc varchar(20) AS 

--DECLARE @liq varchar(30),@tipdoc varchar(20),@doc varchar(20)
...
IF (SELECT TOP 1 solNumeroRadicacion FROM SAP.ContablesCtrl WHERE solNumeroRadicacion = @liq AND perTipoIdentificacion = @tipdoc AND perNumeroIdentificacion = @doc AND observaciones LIKE '%C02%' AND fechaIngreso >='2023-08-15') IS NULL
	...
	--------------------------------------------------------------------40------------------------------------------------------------------------------------------------------------			
			...
			WHERE dsa.dsaEstado IN ('ANULADO_REEMPLAZADO', 'ANULADO')
			AND ISNULL(dsaMotivoAnulacion,'') != 'PRESCRIPCION'
		...
		INSERT INTO #IC_CM_Det 		
		SELECT 2,[codigoSap], SUM([importeDocumento]) AS importeDocumento , [claveCont], [claseCuenta], [asignacion], [periodoliquidado], [ref1],[ref2], [tipoDocumento],[ref3]
		...
		GROUP BY [codigoSap], asignacion,[periodoliquidado],  [claveCont], [claseCuenta],ref1,ref2, tipoDocumento, ref3,[tipoCuotaMonetaria],[idTipoCm],
	...
	----------------------------------------------------------------------------------------40D------------------------------------------------------------------------------------------------
		...
		BEGIN
			...
			SELECT  1 AS Consecutivo,
					'' as codigosap,--No aplica para este movimiento
					 SUM (dsa.dsaValorDescuento)  AS Importedocumento,--valor pago
					'40' AS claveCont, -- credito
					'S' as claseCuenta, --segun archivo excel 40 y 50 = 'S'
					casId AS asignacion,  --Id unico,
					dsaPeriodoLiquidado AS periodoliquidado,
			...
			sls.slsFechaDispersion,pri.priperiodo, dsaTipoliquidacionSubsidio, solFechaRadicacion, dsaPeriodoLiquidado,casUsuarioTransaccion
		END	

	------------------------------------------------------------------FIN-----------------------------------------------------------------------------------------------------------
	----------------------------------------------------------------------------------50--------------------------------------------------------------------------------------------
	
	--Actualizacion asignaciones para descuetos  mayor valor 

	--String or binary data would be truncated in table 'tempdb.dbo.#IC_CM_Det__________________________________________________________________________________________________________000000001231', column 'claseCuenta'. Truncated value: '5'.

	
	INSERT INTO #IC_CM_Det 
		
	SELECT 1,[codigoSap], SUM([importeDocumento]) AS importeDocumento ,  '50' as [claveCont],'S'AS [claseCuenta], [asignacion], [periodoliquidado], [ref1],[ref2], [tipoDocumento],[ref3]
	,[tipoCuotaMonetaria],[idTipoCm],'','',[bancoDispersionDevolucion],[cajaPago],[codigoGenesys],det.tipo,fechaDocumento,getdate(),'', '', '',[usuario]
	FROM #IC_CM_Det det

	GROUP BY [codigoSap], asignacion, [periodoliquidado], ref1,ref2, tipoDocumento, ref3,[tipoCuotaMonetaria],[idTipoCm],
	[bancoDispersionDevolucion],[cajaPago],[codigoGenesys],det.tipo,fechaDocumento,[usuario]
	...
	--- Fin de comentario 
	 ...
	 --SE QUITAN DATOS NO NECESARIOS EN CADA MOVIMIENTO
		UPDATE #IC_CM_Det SET tipoCuotaMonetaria = '' , idTipoCm = '' WHERE claveCont = '40'
		UPDATE #IC_CM_Det SET formaPago = '' , codigoEntidadDescuento = '' WHERE claveCont = '50'
		UPDATE #IC_CM_Det SET formaPago ='' WHERE claveCont ='40' AND (codigoEntidadDescuento <> 0) 
	...
	SELECT @observacion = (SELECT TOP 1 CONCAT(min(asignacion), 'C02') FROM #IC_CM_Det)

	IF (SELECT top 1 asignacion from #IC_CM_Det) is not null
	BEGIN
		  INSERT INTO sap.ContablesCtrl
			SELECT @liq, 1, @doc, @tipdoc, getdate(), 'CM', @Observacion
		IF (SELECT TOP 1 CONCAT (asignacion,'C02')  FROM #IC_CM_Det WHERE CONCAT (asignacion,'C02') <> @Observacion)  <>
			@Observacion
			BEGIN
				SELECT @liq, 1, @doc, @tipdoc, getdate(), 'CM', (SELECT TOP 1 CONCAT(max(asignacion), 'C02') FROM #IC_CM_Det)
			END
		END
	END
	...
	END TRY
		BEGIN CATCH
			...
			SELECT  GETDATE() -'05:00:00', @liq, @doc, @tipdoc,'C02', ERROR_NUMBER(),ERROR_SEVERITY(),ERROR_STATE(),ERROR_PROCEDURE(),ERROR_LINE(),ERROR_MESSAGE()

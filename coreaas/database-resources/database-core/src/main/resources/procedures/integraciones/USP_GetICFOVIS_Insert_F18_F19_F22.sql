...
-- Modificado: Yesika Bernal, adicion tabla generacion de errores , Nueva modificacion componente y 21022024 validacion de valores en cero
-- =====================================================================================================
CREATE OR ALTER   PROCEDURE [sap].[USP_GetICFOVIS_Insert_F18_F19_F22] 
...
< (SELECT pof.pofValorAsignadoSFV
								...
								AND per.perNumeroIdentificacion = @documentoId
								AND per.perTipoIdentificacion = @tipoDocumento)
--		AND (select solTipoTransaccion from solicitud sol 
--					INNER JOIN SolicitudLegalizacionDesembolso sld on sol.solid = sld.sldSolicitudGlobal
--					INNER JOIN LegalizacionDesembolso lgd on sld.sldLegalizacionDesembolso = lgd.lgdid
--					INNER JOIN PostulacionFOVIS pof on pof.pofId = sld.sldPostulacionFOVIS
...
--					AND pof.pofMotivoDesistimiento IS NULL
--					AND YEAR(sol.solFechaRadicacion) > 2013 -- VERIFICAR AÑO
--					AND per.perNumeroIdentificacion = @documentoId
--					AND per.perTipoIdentificacion = @tipoDocumento) IN 
--		('ADQUISICION_VIVIENDA_NUEVA_RURAL_ANTICIPADO_ESCRITURA','ADQUISICION_VIVIENDA_NUEVA_RURAL_GIRO_ANTICIPADO_AVAL_BANCARIO'
--,'ADQUISICION_VIVIENDA_NUEVA_RURAL_GIRO_ANTICIPADO_CONVENIO_CCF','ADQUISICION_VIVIENDA_NUEVA_RURAL_GIRO_ANTICIPADO_ENCARGO_FIDUCIARIO'
--,'ADQUISICION_VIVIENDA_NUEVA_URBANA_ANTICIPADO_ESCRITURA','ADQUISICION_VIVIENDA_NUEVA_URBANA_GIRO_ANTICIPADO_AVAL_BANCARIO'
--,'ADQUISICION_VIVIENDA_NUEVA_URBANA_GIRO_ANTICIPADO_CONVENIO_CCF','ADQUISICION_VIVIENDA_NUEVA_URBANA_GIRO_ANTICIPADO_ENCARGO_FIDUCIARIO'
--,'ADQUISICION_VIVIENDA_USADA_RURAL_ANTICIPADO_ESCRITURA','ADQUISICION_VIVIENDA_USADA_RURAL_GIRO_ANTICIPADO_AVAL_BANCARIO'
--,'ADQUISICION_VIVIENDA_USADA_RURAL_GIRO_ANTICIPADO_CONVENIO_CCF','ADQUISICION_VIVIENDA_USADA_RURAL_GIRO_ANTICIPADO_ENCARGO_FIDUCIARIO'
--,'ADQUISICION_VIVIENDA_USADA_URBANA_ANTICIPADO_ESCRITURA','ADQUISICION_VIVIENDA_USADA_URBANA_GIRO_ANTICIPADO_AVAL_BANCARIO'
--,'ADQUISICION_VIVIENDA_USADA_URBANA_GIRO_ANTICIPADO_CONVENIO_CCF','ADQUISICION_VIVIENDA_USADA_URBANA_GIRO_ANTICIPADO_ENCARGO_FIDUCIARIO'
--,'CONSTRUCCION_SITIO_PROPIO_RURAL_ANTICIPADO_ESCRITURA','CONSTRUCCION_SITIO_PROPIO_URBANO_ANTICIPADO_ESCRITURA'
--,'MEJORAMIENTO_VIVIENDA_RURAL_ANTICIPADO_ESCRITURA','MEJORAMIENTO_VIVIENDA_URBANA_ANTICIPADO_ESCRITURA')		-- Toma solo la ultima radicacion de pago por anticipo
	...
	-- INICIO: Calcular la fecha de contabilizacion (FECHA DE APROBACION) desde la base de datos de auditoria ya sea local en CORE o posteriormente en COREAUD
		...
		SELECT DISTINCT
			...
			'50' AS claveCont,
			'S' AS claseCuenta,
				(select case when rendimientoFinanciero  is null then 0 else rendimientoFinanciero end from openjson(@json, '$.datosNovedadRegularFovisDTO')
                with(
		...
		SELECT DISTINCT
			...
			'40' AS claveCont,
			'S' AS claseCuenta,
			(select case when importeDocumento is null then 0 else importeDocumento end  from openjson(@json, '$.datosNovedadRegularFovisDTO')
                with(
                        importeDocumento varchar(50) '$.valorRestituir'
                )) 
				+
		...
		--detecta que importe esta en cero para eliminarlo, estos no deben viajar a la integración
		IF (SELECT COUNT(*) FROM #IC_FOVIS_Det WHERE importeDocumento = 0 ) > 0
		begin 
...
BEGIN CATCH
	...
	INSERT INTO core.sap.LogErrorFovis 
			SELECT  GETDATE() -'05:00:00', @solNumeroRadicacion, @documentoId, @tipoDocumento,'F18_F19_F22', ERROR_NUMBER(),ERROR_SEVERITY(),ERROR_STATE(),ERROR_PROCEDURE(),ERROR_LINE(),ERROR_MESSAGE()

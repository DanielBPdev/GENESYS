CREATE OR ALTER PROCEDURE [dbo].[sp_gen_select_proc_anibol_arch_validos] 
...
DECLARE @s_queryTotal VARCHAR(MAX)='
	SELECT TOP 15000 
		...
		UPPER(COALESCE(dbo.LimpiarCaracteres(ISNULL(ubiDireccionFisica,'''')),''''))+''|''+
		ISNULL(Telefono,'''')+''|''+
		ISNULL(depCodigo,'''')+''|''+
		ISNULL(munCodigo,'''')+''|''+
		CAST(pedFechaNacimiento AS VARCHAR)+''|''+
		dbo.LimpiarCaracteres(ISNULL(pedEstadoCivil,''''))+''|''+
		ISNULL(pedGenero,'''') +''|''+
		ISNULL(perRazonSocial,'''')+''|''+
		(SELECT UPPER((dbo.LimpiarCaracteres(prmValor))) 
			FROM parametro 
			WHERE prmNombre=''NOMBRE_CCF'' )+''|''+
		(SELECT UPPER((dbo.LimpiarCaracteres(prmValor))) 
			FROM parametro 
			WHERE prmNombre=''RESPONSABLE_CCF'' )+''|''+
		(SELECT UPPER((dbo.LimpiarCaracteres(prmValor))) 
			FROM parametro 
			WHERE prmNombre=''DIRECCION_CCF'' )+''|''+
		(SELECT UPPER((dbo.LimpiarCaracteres(prmValor))) 
			FROM parametro 
			WHERE prmNombre=''CIUDAD_CCF'' )+''|''+
		(SELECT UPPER((dbo.LimpiarCaracteres(prmValor))) 
			FROM parametro 
			WHERE prmNombre=''DEPARTAMENTO_CCF'' )+''|''+
		(SELECT UPPER((dbo.LimpiarCaracteres(prmValor))) 
			FROM parametro 
			WHERE prmNombre=''TELEFONO_CCF'' ) AS registro 
	FROM (
';
DECLARE @s_queryWhereEstado VARCHAR(100) = '';
DECLARE @s_queryWhereRequeridos VARCHAR(300) = '';
DECLARE @TipoTransaccionBen VARCHAR(1000) = '';
...
BEGIN
	SET @s_queryTotal = @s_queryTotal + '
		WHERE (perNumeroIdentificacion IS NOT NULL 
		AND perPrimerNombre IS NOT NULL 
		AND perPrimerApellido IS NOT NULL 
		AND ubiDireccionFisica IS NOT NULL 
		AND Telefono IS NOT NULL 
		AND depCodigo IS NOT NULL 
		AND munCodigo IS NOT NULL 
		AND pedFechaNacimiento IS NOT NULL 
		AND pedEstadoCivil IS NOT NULL 
		AND pedGenero IS NOT NULL)
	';
END;

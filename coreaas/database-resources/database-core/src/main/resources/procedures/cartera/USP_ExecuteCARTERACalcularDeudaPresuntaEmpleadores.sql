...
BEGIN TRY
    ...
    SELECT @dFechaBeneficio = bemFechaVinculacion
    FROM BeneficioEmpleador,
         Beneficio
    WHERE bemBeneficioActivo = 1
      AND bemBeneficio = befId
      AND befTipoBeneficio = 'LEY_590'
      AND bemEmpleador = @idEmpleador


    SET @porcentajeEmpleador = dbo.UFN_GET_Porcentaje590(@fechaActual, @dFechaBeneficio)

    ---SELECT '@porcentajeEmpleador',@porcentajeEmpleador
...
-- Almacena los aportes del periodos por empresa/aportante
    ...
    BEGIN TRY
        ...
        WHERE apg.apgEmpresa = @idEmpresa
          AND apd.apdEstadoCotizante IN ('ACTIVO', 'INACTIVO')-----CAMBIO OLGA VEGA 20221215 CAMBIO 20230418 PARA LOS RETIRADOS CON PAGO Y NOV REINGRESO
...
--select '@tablaTrabajadoresCotizantesSinNov', * from @tablaTrabajadoresCotizantesSinNov
    ...
    DECLARE @calculodeudapresunta TABLE
                                  (
                                      ...
                                      apdSalarioIntegral             BIT,
                                      apdValorIBC					NUMERIC(19, 5),
									  apdEstadoCotizante VARCHAR (60)---20240626 arreglo 77536
                                  )


    INSERT INTO @calculodeudapresunta
    SELECT
        apdSalarioBasico ,  apgPeriodoAporte,
        apdPersona,
        roaEmpleador,
        SUM(apdDiasCotizados) AS apdDiasCotizados,
        apdTarifa,
        roaId ,apdSalarioIntegral,
        SUM(apdValorIBC) AS apdValorIBC
		,apdEstadoCotizante----arreglo 77536 20240626
    FROM (
             SELECT CASE
                    ...
                    apdx.apdPersona,
                    roaEmpleador,
                    CASE WHEN SUM(apdx.apdDiasCotizados)>30 THEN 30 ELSE SUM(apdx.apdDiasCotizados) END AS apdDiasCotizados,--CAMBIO OLGA VEGA 20230620
                    MAX(apdx.apdTarifa) as apdTarifa,
                    roaId ,
                    apdSalarioIntegral,
                    MAX(apdValorIBC) as apdValorIBC---CAMBIO OLGA VEGA 20230620
					,apdEstadoCotizante---20240626 arreglo 77536
             ...
             WHERE roaEmpleador = @idEmpleador
               ...
               AND px.perid NOT IN (SELECT norPersona FROM @novedadRetiro)
             ...
             GROUP BY apdx.apdPersona, roaEmpleador, apgPeriodoAporte,
                      roaId,apdSalarioIntegral ,apdEstadoCotizante
             --apdx.apdDiasCotizados,
             --               apdValorIBC
         ) X
    WHERE apdSalarioBasico IS NOT NULL
    GROUP  BY apgPeriodoAporte, apdPersona, roaEmpleador,
              apdSalarioBasico, apdTarifa,apdSalarioIntegral, roaId,apdEstadoCotizante
    ...
    SELECT  roaValorSalarioMesadaIngresos AS apdSalarioBasico,
            ...
            0.04                          as apdTarifa,
            roaId,0,0,roaEstadoAfiliado---cambio 20240626 por glpi 77253
    ...
    WHERE roaEmpleador = @idEmpleador
      AND roaTipoAfiliado = 'TRABAJADOR_DEPENDIENTE'
      AND px.perid NOT IN (SELECT norPersona FROM @novedadRetiro)
      AND px.perId NOT IN (SELECT apdPersona FROM @calculodeudapresunta WHERE roaEmpleador = @idEmpleador)
...
--  SELECT '@tablaAportesVigentes' ,* FROM @tablaAportesVigentes
    ...
    -- SELECT '@tablaTrabajadoresCotizantesSinNov', * FROM @tablaTrabajadoresCotizantesSinNov
-- select '@tablaAportesTemp', * from @tablaAportesTemp
---	SELECT '#Trabpordias',* FROM #Trabpordias
---CALCULO DEUDA PRESUNTA LC1
    ...
    WHILE @loop_counter4 > 0 AND @item_category_counter4 <= @loop_counter4
        BEGIN
            SELECT @Salario = CASE WHEN (apdSalarioIntegral  = 1
                AND apdSalarioBasico >= (SELECT ISNULL(prmValor,0)*13 FROM PARAMETRO WHERE prmNombre = 'SMMLV'))
                AND apdValorIBC>0 THEN apdValorIBC ELSE apdSalarioBasico END ,--- CAMBIO OLGA VEGA 20230503 SALARIOS INTEGRALES
                   @periodo = apgPeriodoAporte,
                   @perid = apdPersona,
                   @diascotizados = CASE WHEN apdDiasCotizados >30 THEN 30 ELSE apdDiasCotizados END ,
                   --@tarifa = apdTarifa,
                   @tarifa =0.04,
                   @roaid = roaId
            ...
            --SELECT *,'@calculodeudapresunta', @porcentajeEmpleador,'@porcentajeEmpleador'
            --FROM @calculodeudapresunta
            --         WHERE primary_key = @item_category_counter4

            --    SELECT  'loque calcula',(ISNULL(@Salario, r.roaValorSalarioMesadaIngresos) / 30) as saldia,ISNULL(@diascotizados, 30) as diascot,isnull(@tarifa, 0.04) as tarifa,
            ...
            SET cadDeudaPresunta = CASE WHEN @porcentajeEmpleador>0 THEN  ISNULL(@calculo, 0) * @porcentajeEmpleador ELSE ISNULL(@calculo, 0) END
            WHERE perId = @PerId
        ...
        END
    --FIN DEL CALCULO DE LA DEUDAPRESUNTA DE CADA TRABAJADOR
    ...
    WHERE presunta.roaEmpleador = @idEmpleador
      ...
      AND presunta.apdPersona NOT IN (SELECT perId FROM  @tablaPersonasConNovedad)
	 AND presunta.apdPersona NOT IN (SELECT norPersona FROM @novedadRetiro) ---MODIFICACION 20240305 OLGA VEGA POR GLPI 76888
	  AND Presunta.apdEstadoCotizante NOT IN ('NO_FORMALIZADO_SIN_AFILIACION_CON_APORTES')---20240626 OLGA VEGA GLPI 77536
	
	--AND presunta.apdPersona NOT IN (SELECT distinct p.perId FROM Persona P
		--							 inner join Afiliado A on p.perId = A.afiPersona
		--							 inner join RolAfiliado RA on ra.roaAfiliado = a.afiId
		--							 where ra.roaEstadoAfiliado = 'INACTIVO' OR ra.roaEstadoAfiliado IS NULL ) --MODIFICACION 01042024 GLPI 77536 JUAN C. AVILA MEZA
...
-- Se guarda valor original de la deuda presunta unitara caso LC1 y LC2
    ...
    --  INSERTAR LC2
    ---SELECT '1-@carteraCreada',* FROM @carteraCreada
    --select '@tablaAportesVigentes',* from @tablaAportesVigentes
    --select 'toda la tabla',presunta.apdPersona, presunta.apdSalarioBasico,sinnov.cadDeudaPresunta , SUM(Aportes.aporte)
    --                FROM @calculodeudapresunta AS presunta
    --         INNER JOIN  (SELECT sinnov.cadDeudaPresunta ,perId
    --	              FROM  @tablaTrabajadoresCotizantesSinNov AS sinnov
    --				  WHERE   trabajadorActivoAlMenosUnDia = 1
    --				  ) AS sinnov
    --				 ON   presunta.apdPersona = sinnov.perId
    ...
    --               HAVING ((sinnov.cadDeudaPresunta) - SUM(Aportes.aporte)) > 0
        ...
        IF @activaParametrizacionLC2 = 1
            BEGIN
                ...
                SELECT presunta.apdPersona,
                       ...
                       SUM(Aportes.aporte) AS aporte
                FROM @calculodeudapresunta AS presunta
                         INNER JOIN  (SELECT sinnov.cadDeudaPresunta ,perId
                                      FROM  @tablaTrabajadoresCotizantesSinNov AS sinnov
                                      WHERE   trabajadorActivoAlMenosUnDia = 1
                                      GROUP BY sinnov.cadDeudaPresunta ,perId
                ) AS sinnov
                                     ON   presunta.apdPersona = sinnov.perId
                         INNER JOIN @tablaAportesVigentes AS Aportes
                                    ON sinnov.perId = aportes.perId
                                        AND Aportes.tipoAporte IN ( 'OK','NO_VALIDADO_BD_APROBADO')
                WHERE presunta.roaEmpleador = @idEmpleador
                  AND apdPersona NOT IN (SELECT perIdtrab FROM #Trabpordias WHERE idempleador = @idEmpleador)---ES ES PARA LOS TRAB DE MEDIO TIEMPO
                AND  apdPersona NOT IN (SELECT norPersona FROM @novedadRetiro) ---MODIFICACION 20240305 OLGA VEGA POR GLPI 76888
			   GROUP BY presunta.apdPersona,
                         -- Aportes.aporte,
                         presunta.apdSalarioBasico, cadDeudaPresunta
                HAVING ((sinnov.cadDeudaPresunta) - SUM(Aportes.aporte)) > 0
                ...
                WHILE @loop_counter2 > 0 AND @item_category_counter2 <= @loop_counter2
                    BEGIN
                        SELECT @peridlc2 = apdPersona,
                               @Salariolc2 = SUM(SalarioBasico),---CUANDO SON DISTINTOS
                               @aporte = SUM(aporte)
                        FROM @LC2
                        ...
                        SET @deudaIndividuallc2 = (@Salariolc2 * 0.04) - ISNULL(@aporte, 0)

                        SET @deudaIndividuallc2 = CASE
                                                      WHEN (ISNULL(@deudaIndividuallc2, 0)) < 0 THEN 0
                                                      ELSE @deudaIndividuallc2 END

                        UPDATE @tablaAportesVigentes
                        SET deudaIndividual = ISNULL(@deudaIndividuallc2, 0)
                        WHERE perId = @Peridlc2

                        -- se llena la variable que calcula la deuda presunta por empleador
                        SET @DeudaEmpleadorlc2 = ISNULL(@DeudaEmpleadorlc2, 0) + ISNULL(@deudaIndividuallc2, 0)

                        --SELECT @DeudaEmpleadorlc2,'@DeudaEmpleadorlc2'

                        SET @Salariolc2 = 0
                        SET @deudaIndividuallc2 = 0
                        SET @aporte = 0
    ...
    IF (SELECT COUNT(*) FROM @LC2) > 0
        BEGIN
            ...
            BEGIN TRY
                ...
                ELSE
                    BEGIN
                        ...
                        SELECT DISTINCT ROUND(@DeudaEmpleadorlc2, 0),
                                        ROUND(@DeudaEmpleadorlc2, 0),
                                        'AL_DIA',
                                        'VIGENTE',
                                        @fechaActual,
                                        @idPersonaEmpleador,
            ...
            --- SELECT '2-@carteraCreada',* FROM @carteraCreada
            ...
            BEGIN TRY
                ...
                WHERE ISNULL(deudaIndividual, 0) > 0
    ...
    IF @activaParametrizacionLC3 = 1
        BEGIN
            ...
            SELECT dp.apdSalarioBasico,
                   ...
                   0                                                                        as DiasIncap,
                   SUM(a.aporte)  + (SELECT SUM(ISNULL(apv.aporte,0))
                                     FROM @tablaAportesVigentes apv
                                     WHERE apv.tipoAporte  IN ( 'OK','NO_VALIDADO_BD_APROBADO')---CAMBIO OLGA VEGA 2023-07-18 POR ACLARACIÓN DE APORTES
                                       AND apv.perId = B.apdPersona) as aporte---POR CULPA DE SERGIO
            ...
            WHERE B.apgPeriodoAporte = @periodoEvaluacion
              AND A.tipoAporte NOT IN ( 'OK','NO_VALIDADO_BD_APROBADO')---CAMBIO OLGA VEGA 2023-07-18 POR ACLARACIÓN DE APORTES
              AND A.perId NOT IN (SELECT perIdtrab FROM #Trabpordias WHERE idempleador = @idEmpleador)---ES ES PARA LOS TRAB DE MEDIO TIEMPO
              AND A.perid NOT IN (SELECT perId FROM @tablaPersonasConNovedad)
			   AND A.perid NOT IN (SELECT norPersona FROM @novedadRetiro) ---MODIFICACION 20240305 OLGA VEGA POR GLPI 76888
              AND A.perid NOT IN (SELECT cadpersona
                                  FROM @carteraDependienteCreada---que no este ya creado en la lc1 o lc2	por robinson se fue al verga
            )
              --AND A.perid NOT IN ( SELECT apv.perId
              --				 FROM @tablaAportesVigentes apv
              --				 WHERE apv.tipoAporte = 'OK'
              --				) por culpa de sergio se va todo a la verga again 20230526
            GROUP BY dp.apdSalarioBasico, a.perId, dp.apdTarifa,B.apdPersona


            --SELECT '@LC3',* FROM @LC3
            --SELECT '@carteraDependienteCreada', cadpersona
            --							 FROM @carteraDependienteCreada

            --SELECT 'ELSELECT ' ,     dp.apdSalarioBasico,
            --                 @periodoEvaluacion,
            --                 a.perId,
            --                 @idEmpleador,
            --                 CASE WHEN SUM(dp.apdDiasCotizados) >30   THEN 30 ELSE SUM(dp.apdDiasCotizados) END as apdDiasCotizados,
            --                 CASE WHEN ISNULL(dp.apdTarifa, 0) = 0 THEN 0.04000 ELSE dp.apdTarifa END as tarifa,
            --                 0                                                                        as DiasIncap,
            --                 SUM(a.aporte)  + (SELECT SUM(ISNULL(apv.aporte,0))
            --                                   FROM @tablaAportesVigentes apv
            ...
            --    INNER JOIN @calculodeudapresunta dp ON dp.apdPersona = a.perId
            --          WHERE B.apgPeriodoAporte = @periodoEvaluacion
            --            AND A.tipoAporte NOT IN ( 'OK','NO_VALIDADO_BD_APROBADO')
            --     GROUP BY dp.apdSalarioBasico, a.perId, dp.apdTarifa,B.apdPersona
            ...
            WHILE @loop_counter3 > 0 AND @item_category_counter3 <= @loop_counter3
                BEGIN
                    ...
                    SELECT @Salariolc3 = SUM(apdSalarioBasico),
                           @peridlc3 = apdPersona,
                           @diascotizadoslc3 = SUM(apdDiasCotizados),
                           @tarifalc3 = MAX(Tarifa),
                           @DiasIncaplc3 = SUM(DiasIncap),
                           @aporte = SUM(aporte)
                    FROM @LC3
                    ...
                    --calculo
                    SET @deudaIndividuallc3 = (@Salariolc3 * 0.04000) - ISNULL(@aporte,0)

                    SET @deudaIndividuallc3 = CASE
                                                  WHEN (ISNULL(@deudaIndividuallc3,0)) < 0 THEN 0
                                                  ELSE ISNULL(@deudaIndividuallc3,0) END
                    ...
                    -- se llena la variable que calcula la deuda presunta por empleador
                    SET @DeudaEmpleadorlc3 = ROUND(ISNULL(@DeudaEmpleadorlc3,0), 0) + ROUND(ISNULL(@deudaIndividuallc3,0), 0)
    ...
    IF (SELECT COUNT(*) FROM @LC3) > 0
        BEGIN
            ...
            BEGIN CATCH
                ...
                THROW;
            END CATCH
            ----SELECT '3-@carteraCreada',* FROM @carteraCreada
...
--		select '@CarteraDependienteVigente',* from @CarteraDependienteVigente
                        ...
                        BEGIN
                            ...
                            BEGIN TRY
                                ...
                                FROM CarteraDependiente cad
                                    ...
                                    WHERE tipoAporte  NOT IN ( 'OK','NO_VALIDADO_BD_APROBADO')---CAMBIO OLGA VEGA 2023-07-18 POR ACLARACIÓN DE APORTES
                        ...
                        WHERE exclusiones.perId IS NULL
                          ...
                          AND (
                            -- se activó en el periodo de estudio
                                    ...
                                    DATEADD(MONTH, DATEDIFF(MONTH, 0, roa.roaFechaAfiliacion), 0)
                                -- aún activo
                              ---  OR roa.roaFechaRetiro IS NULL CAMBIO 20230911 PORQUE SI SE EVALUA POR ERROR PERIODOS VIEJOS NO DEBE INSERTAR CUANDO ESTA ACTIVO Y SE ACTUALIZAN APORTES ANTERIORES
                                -- se retiró en el mes de estudio
                                OR CONVERT(DATE, @periodoEvaluacion + '-01', 121) =
                                   DATEADD(MONTH, DATEDIFF(MONTH, 0, roa.roaFechaRetiro), 0)
                            )
                                    ...
                                    BEGIN
                                        ...
                                        BEGIN TRY
                                            ...
                                            WHERE tap.perId IS NULL -- personas con novedad de reingreso y sin aportes vigentes
                                              AND cadId IS NULL
                                            -- personas que no se encuentren en la cartera actualmente
                                            ...
                                            BEGIN
                                                ...
                                                BEGIN TRY
                                                    ...
                                                    FROM @tablaPersonasConNovedadReingreso pre
                                                        ...
                                                        WHERE tipoAporte NOT IN ( 'OK','NO_VALIDADO_BD_APROBADO')
                                                    ) AS montoAportes
...
-- Se crea el histórico de CarteraDependiente
                        ...
                        BEGIN TRY
                            ...
                            --select 'para retirar',* from @carteraDependienteCreada
--SELECT '@tablaAportesTemp',* FROM @tablaAportesTemp
...
------cambio 20221227
                            ...
                            SELECT cad.cadpersona,
                                   ISNULL(cadc.cadDeudaPresunta, 0)           as deudapresunta,
                                   SUM(ISNULL(atemp.apdAporteObligatorio, 0)) as aporte
                            FROM CarteraDependiente cad
                                     INNER JOIN @carteraDependienteCreada cadc ON cadc.cadId = cad.cadId
                            ...
                            WHERE ca.persona is null
                                and cdcAccion = 'D'
                               OR (cdcAccion = 'U' AND cadc.cadDeudaPresunta <= 0)
                            GROUP BY cad.cadpersona, cadc.cadDeudaPresunta
...
---      INNER JOIN #CarteraActualizar nov ON nov.persona = cadc.cadPersona
                    ...
                    ----VALIDAR LOS ESTADOS 20230531
                    UPDATE cartera SET carestadooperacion = 'NO_VIGENTE', carestadocartera ='AL_DIA'
                    FROM persona
                             INNER JOIN cartera on carpersona =perid
                             INNER JOIN CarteraDependiente on cadcartera = carid
                    WHERE cardeudapresunta =0
                      AND carestadooperacion = 'VIGENTE'
                      AND carId = @idCartera
...
END CATCH
BEGIN
    DECLARE @Cantlineas INT = 0

    --select '@@tablaAportesTemp',* from @tablaAportesTemp
    --SELECT ' @tablaTrabajadoresCotizantesSinNov',* FROM @tablaTrabajadoresCotizantesSinNov
    --SELECT ' @@tablaAportesVigentes',* FROM @tablaAportesVigentes
    --select '@crearNuevoRegistro', @crearNuevoRegistro

    SELECT @Cantlineas = SUM(ISNULL(Linea, 0))
    FROM (
             SELECT CONVERT(INT, RIGHT(carTipoLineaCobro, 1)) AS linea,
                    carTipoLineaCobro,
                    carpersona,
                    carPeriodoDeuda
             FROM Cartera c
                      INNER JOIN Persona p ON p.perid = carPersona
             WHERE carEstadoOperacion = 'VIGENTE'
               AND carDeudaPresunta > 0
               AND carTipoLineaCobro IN ('LC1', 'LC2', 'LC3')
               AND carPersona = @idPersonaEmpleador
               AND LEFT(carPeriodoDeuda
                       , 7) = @periodoEvaluacion
             GROUP BY carpersona, carPeriodoDeuda, carTipoLineaCobro
         ) x
    GROUP BY carpersona, carPeriodoDeuda

    IF @crearNuevoRegistro = 1 AND @Cantlineas = 1 ---TOTAL
        BEGIN
            IF (SELECT COUNT(*)
                FROM @tablaAportesTemp a
                WHERE a.apgPeriodoAporte = @periodoEvaluacion
                  AND a.apdEstadoRegistroAporteArchivo  IN ( 'OK','NO_VALIDADO_BD_APROBADO')) > 0 ---CAMBIO OLGA VEGA 2023-07-18 POR ACLARACIÓN DE APORTES
                UPDATE cartera
                SET carDeudaParcial='MORA_PARCIAL'
                WHERE carPersona = @idPersonaEmpleador
                  AND LEFT(carPeriodoDeuda, 7) = @periodoEvaluacion

            ELSE

                UPDATE cartera
                SET carDeudaParcial='MORA_TOTAL'
                WHERE carPersona = @idPersonaEmpleador
                  AND LEFT(carPeriodoDeuda, 7) = @periodoEvaluacion
        END
    ELSE

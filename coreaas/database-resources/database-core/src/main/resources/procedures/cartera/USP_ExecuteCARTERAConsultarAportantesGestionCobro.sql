...
INSERT INTO @Filtros1Y2EstadosAportante (
	personaId, tipoIdentificacion,	numeroIdentificacion, razonSocial, tipoSolicitante, empresaId, empleadorId, estadoEmpleador, estadoCartera, numeroOperacion
)
SELECT DISTINCT pers.perId, pers.perTipoIdentificacion, pers.perNumeroIdentificacion, pers.perRazonSocial, cart.carTipoSolicitante, empr.empId, empl.empId, empl.empEstadoEmpleador, cart.carEstadoCartera, carg.cagNumeroOperacion
FROM Persona pers
...
SELECT apgEmpresa, apgTipoSolicitante, promedio  FROM (SELECT 
...
GROUP BY ap.apgEmpresa, ap.apgTipoSolicitante) AS promedioAportes
WHERE promedio>=@smlv  
...
INSERT INTO @Resultado (
	personaId, tipoIdentificacion, numeroIdentificacion, razonSocial, tipoSolicitante, estadoCartera, promedio, totalTrabajadoresActivos, totalVecesMoroso, estadoEmpleador, numeroOperacion
)
SELECT DISTINCT TOP (SELECT CAST(ROUND(COUNT(*) * @criterio1Promedio, 0) AS INTEGER) FROM @Filtros3PromedioAportes fc1)
	f1y2.personaId, f1y2.tipoIdentificacion, f1y2.numeroIdentificacion, f1y2.razonSocial, f1y2.tipoSolicitante, f1y2.estadoCartera, f3.promedio, f4.totalTrabajadoresActivos, f5.totalVecesMoroso, f1y2.estadoEmpleador, f1y2.numeroOperacion
FROM @Filtros1Y2EstadosAportante f1y2 
INNER JOIN @Filtros3PromedioAportes f3 ON f1y2.empresaId = f3.apgEmpresa
LEFT JOIN @Filtros4TrabajadoresActivos f4 ON f1y2.empleadorId = f4.empId
LEFT JOIN @Filtros5MorosoAlgunaVez f5 ON f1y2.personaId = f5.carPersona

-- Criterio 2
INSERT INTO @Resultado (
	personaId, tipoIdentificacion, numeroIdentificacion, razonSocial, tipoSolicitante, estadoCartera, promedio, totalTrabajadoresActivos, totalVecesMoroso, estadoEmpleador, numeroOperacion
)
SELECT DISTINCT TOP (SELECT CAST(ROUND(COUNT(*) * @criterio2Activos, 0) AS INTEGER) FROM @Filtros4TrabajadoresActivos fc2)
	f1y2.personaId, f1y2.tipoIdentificacion, f1y2.numeroIdentificacion, f1y2.razonSocial, f1y2.tipoSolicitante, f1y2.estadoCartera, f3.promedio, f4.totalTrabajadoresActivos, f5.totalVecesMoroso, f1y2.estadoEmpleador, f1y2.numeroOperacion
FROM @Filtros1Y2EstadosAportante f1y2 
INNER JOIN @Filtros4TrabajadoresActivos f4 ON f1y2.empleadorId = f4.empId
LEFT JOIN @Filtros3PromedioAportes f3 ON f1y2.empresaId = f3.apgEmpresa
LEFT JOIN @Filtros5MorosoAlgunaVez f5 ON f1y2.personaId = f5.carPersona
...
SELECT DISTINCT TOP (SELECT CAST(ROUND(COUNT(*) * @criterio3VecesMoroso, 0) AS INTEGER) FROM @Filtros5MorosoAlgunaVez fc3)
	f1y2.personaId, f1y2.tipoIdentificacion, f1y2.numeroIdentificacion, f1y2.razonSocial, f1y2.tipoSolicitante, f1y2.estadoCartera, f3.promedio, f4.totalTrabajadoresActivos, f5.totalVecesMoroso, f1y2.estadoEmpleador, f1y2.numeroOperacion
FROM @Filtros1Y2EstadosAportante f1y2
INNER JOIN @Filtros5MorosoAlgunaVez f5 ON f1y2.personaId = f5.carPersona
LEFT JOIN @Filtros3PromedioAportes f3 ON f1y2.empresaId = f3.apgEmpresa
LEFT JOIN @Filtros4TrabajadoresActivos f4 ON f1y2.empleadorId = f4.empId
LEFT JOIN @Resultado res ON res.personaId = f1y2.personaId
WHERE res.personaId IS NULL	
...
SELECT TOP(@corte)  
	tipoIdentificacion, numeroIdentificacion, razonSocial, tipoSolicitante, estadoCartera, promedio, totalTrabajadoresActivos, totalVecesMoroso, estadoEmpleador, numeroOperacion
FROM @Resultado

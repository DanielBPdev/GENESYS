/****** Object:  StoredProcedure [dbo].[USP_ExecuteCARTERAAsignarAccionCobro]    Script Date: 2023-09-07 12:58:13 PM ******/
/****** Object:  StoredProcedure [dbo].[USP_ExecuteCARTERAAsignarAccionCobro]    Script Date: 2023-08-11 4:17:33 PM ******/
/****** Object:  StoredProcedure [dbo].[USP_ExecuteCARTERAAsignarAccionCobro]    Script Date: 10/05/2023 4:14:09 p. m. ******/
-- =============================================
...
-- HU164
---EXEC USP_ExecuteCARTERAAsignarAccionCobro 'A2','LC1','METODO_2'
-- =============================================
CREATE OR ALTER   PROCEDURE   [dbo].[USP_ExecuteCARTERAAsignarAccionCobro] @accionCobroFutura VARCHAR(4), @lineaCobro VARCHAR(3), @metodo VARCHAR(8)
  AS
      SET NOCOUNT ON
  ...
  BEGIN TRY
      ...
      IF EXISTS(SELECT 1 FROM BitacoraEjecucionSp WHERE besNombreStoredProcedure = 'USP_ExecuteCARTERAAsignarAccionCobro')
          BEGIN
              UPDATE BitacoraEjecucionSp SET besUltimoInicio = dbo.getLocalDate() WHERE besNombreStoredProcedure = 'USP_ExecuteCARTERAAsignarAccionCobro'
          END
      ELSE
          BEGIN
      ...
      DECLARE @fechaActual DATE = dbo.GetLocalDate()
      DECLARE @fechaDummy DATE
    ...
    -- Varialbles para crear la auditoría
      ...
      DECLARE @camposTablaCartera VARCHAR(1000)

      SELECT @camposTablaCartera = COALESCE(@camposTablaCartera + ', ', '') + COLUMN_NAME
      FROM INFORMATION_SCHEMA.COLUMNS
      WHERE TABLE_NAME = 'Cartera' AND TABLE_SCHEMA = 'dbo'

      DECLARE @CarteraAud AS TablaBigintIdType
      DECLARE @inicioDiasConteoD VARCHAR(13)
      DECLARE @inicioDiasConteoE VARCHAR(13)
      DECLARE @inicioDiasConteoEPersuasivo VARCHAR(13)
      DECLARE @inicioDiasConteoF VARCHAR(13)
      DECLARE @inicioDiasConteoG VARCHAR(13)
      DECLARE @siguienteAccionF VARCHAR(29)
      DECLARE @siguienteAccion VARCHAR(20)
      DECLARE @diasLimitePagoA INT
      DECLARE @diasParametrizadosA INT
      DECLARE @diasLimiteEnvioComunicadoA INT
      DECLARE @diasGeneracionAvisoB INT
      DECLARE @limiteEnvioComunicadoB INT
      DECLARE @diasLiquidacionC INT
      DECLARE @limiteEnvioDocumentoC INT
      DECLARE @diasEjecucionC INT
      DECLARE @diasTranscurridosD INT
      DECLARE @diasLimiteEnvioD INT
      DECLARE @diasTranscurridosE INT
      DECLARE @diasTranscurridosEPersuasivo INT
      DECLARE @diasTranscurridosF INT
      DECLARE @diasLimiteEnvioE INT
      DECLARE @diasLimiteEnvioEPersuasivo INT
      DECLARE @limiteEnvioF INT
      DECLARE @diasParametrizadosF INT
      DECLARE @diasNotificacionF INT
      DECLARE @diasEdictoF INT
      DECLARE @diasTranscurridosG INT
      DECLARE @limiteEnvioG INT
      DECLARE @diasRegistroH INT
      DECLARE @diasParametrizadosH INT
      ...
      IF @accionCobroFutura IN ('A1','AB1','A2','AB2') -- Parametrización Acción 1A | 2A
          BEGIN
      ...
      IF @accionCobroFutura IN ('AB1','B1','BC1','AB2','B2','BC2') -- Parametrización Acción 1B | 2B
          BEGIN
      ...
      IF @accionCobroFutura IN ('BC1','C1','CD1','E1') -- Parametrización Acción 1C
          BEGIN
      ...
      IF @accionCobroFutura IN ('C2') -- Parametrización Acción 2C
          BEGIN
              SELECT @diasEjecucionC = aoc.aocDiasEjecucion
              FROM AccionCobro2C aoc
          END

      IF @accionCobroFutura IN ('CD2','D2','DE2') -- Parametrización Acción 2D
          BEGIN
      ...
      IF @accionCobroFutura IN ('D1','DE1','E1') -- Parametrización Acción 1D
          BEGIN
      ...
      IF @accionCobroFutura IN ('E1','EF1') -- Parametrización Acción 1E
          BEGIN
      ...
      IF @accionCobroFutura IN ('E2','EF2') -- Parametrización Acción 2E
          BEGIN
      ...
      IF @accionCobroFutura = 'F1' -- Parametrización Acción 1F
          BEGIN
      ...
      IF @accionCobroFutura IN ('F2','FG2') -- Parametrización Acción 2F
          BEGIN
      ...
      IF @accionCobroFutura IN ('G2','GH2') -- Parametrización Acción 2G
          BEGIN
      ...
      IF @accionCobroFutura IN ('H2') -- Parametrización Acción 2H
          BEGIN
      ...
      DECLARE @accionCobroReferencia VARCHAR(4)
      ...
      DECLARE @carteraActualizada AS TABLE (carId BIGINT,carTipoAccionCobro VARCHAR(4),carFechaAsignacionAccion DATETIME)

      -- Asigna estado inicial
      INSERT INTO @carteraActualizada (carId, carTipoAccionCobro)
      SELECT carId,
             CASE
                 WHEN @accionCobroFutura = 'A1' AND carTipoLineaCobro in ('LC1','LC4','LC5') AND carMetodo = 'METODO_1' THEN 'A01'
                 WHEN @accionCobroFutura = 'A2' AND carTipoLineaCobro in ('LC1','LC4','LC5') AND carMetodo = 'METODO_2' THEN 'A02'
             ...
             END
      FROM Cartera
      WHERE carTipoAccionCobro IS NULL AND carTipoLineaCobro = @lineaCobro

      ------------------------------------------------------------------------------------------------------
      -- Inicio actualización de datos de cartera con su auditoría
      IF (SELECT COUNT(1) FROM @carteraActualizada) > 0
          BEGIN
              UPDATE car SET car.carTipoAccionCobro = cac.carTipoAccionCobro
              FROM Cartera car
              INNER JOIN @carteraActualizada cac ON car.carId = cac.carId

              EXEC USP_UTIL_GET_CrearRevision 'com.asopagos.entidades.ccf.cartera.Cartera', 1, '', 'USUARIO_SISTEMA', @iRevision OUTPUT

              INSERT INTO @CarteraAud(id)
              SELECT carId
              FROM @carteraActualizada

              SET @sql = 'INSERT INTO aud.Cartera_aud (' + @camposTablaCartera + ', REV, REVTYPE)' + ' SELECT ' + @camposTablaCartera + ', ' + CAST(@iRevision AS VARCHAR) + ', 1' + ' FROM Cartera INNER JOIN @CarteraAud ON carId = id'

              EXEC sp_executesql @sql, N'@CarteraAud TablaBigintIdType READONLY', @CarteraAud

              DELETE @carteraActualizada
              DELETE @CarteraAud
        END
      -- Fin actualización de datos de cartera con su auditoría
      ------------------------------------------------------------------------------------------------------

      DECLARE @tCarteraVigenteSinExclusionConvenio AS TABLE (carId BIGINT,carPersona BIGINT,carTipoLineaCobro VARCHAR(3), carTipoAccionCobro VARCHAR(4),carTipoSolicitante VARCHAR(13),carPeriodoDeuda DATE, carFechaCreacion DATE, carFechaAsignacionAccion DATE)
      DECLARE @exclusionesConveniosInactivos AS TABLE (eciPersona BIGINT,eciTipoSolicitante VARCHAR(13),eciFecha DATE)

      IF @lineaCobro IN ('LC1', 'LC2', 'LC3')
          BEGIN
              INSERT INTO @tCarteraVigenteSinExclusionConvenio (carId, carPersona, carTipoLineaCobro, carTipoAccionCobro,carTipoSolicitante, carPeriodoDeuda, carFechaCreacion,carFechaAsignacionAccion)
              SELECT car.carId,car.carPersona,car.carTipoLineaCobro,car.carTipoAccionCobro,car.carTipoSolicitante,car.carPeriodoDeuda,CAST(car.carFechaCreacion AS DATE),CAST(car.carFechaAsignacionAccion AS DATE)
              FROM Empleador emd
              ...
              INNER JOIN (SELECT CASE WHEN c.carPeriodoDeuda = MIN(c.carPeriodoDeuda) OVER (PARTITION BY carpersona ) THEN c.carPeriodoDeuda ELSE NULL END AS carPeriodoDeuda,cagCartera
                        FROM CarteraAgrupadora
                          JOIN Cartera c ON c.carId = cagCartera
                        WHERE carEstadoOperacion = 'VIGENTE' AND carTipoSolicitante = 'EMPLEADOR' AND carTipoLineaCobro = @lineaCobro
                       ...
                       LEFT JOIN ExclusionCartera exc ON (exc.excEstadoExclusionCartera = 'ACTIVA' AND exc.excTipoSolicitante = car.carTipoSolicitante AND exc.excPersona = car.carPersona
                    and @fechaActual between excFechaInicio and excFechaFin
                      AND exc.excTipoExclusionCartera IN ('EXCLUSION_MORA','EXCLUSION_NEGOCIO','RIESGO_INCOBRABILIDAD', 'IMPOSICION_RECURSO', 'ACLARACION_MORA') -- ajuste GLPI 79412
                  )
                       LEFT JOIN ( SELECT cop.copPersona, cop.copTipoSolicitante, ppc.ppcPeriodo
                                    FROM ConvenioPago cop
                                    JOIN PagoPeriodoConvenio ppc ON ppc.ppcConvenioPago = cop.copId
                                    WHERE cop.copEstadoConvenioPago = 'ACTIVO' ) AS con ON (con.copPersona = car.carPersona AND con.copTipoSolicitante = car.carTipoSolicitante AND con.ppcPeriodo = car.carPeriodoDeuda)
              WHERE excId IS NULL AND con.copPersona IS NULL
              ...
              WHERE cp.copPersona = cv.carPersona AND cp.copTipoSolicitante = cv.carTipoSolicitante AND cp.copEstadoConvenioPago = 'ANULADO'
                AND cp.copMotivoAnulacion = 'INCUMPLIMIENTO' AND CONVERT(DATE, dbo.getLocalDate(), 111) = CONVERT(DATE, cp.copFechaAnulacion, 111)
              UNION
              SELECT ec.excPersona, ec.excTipoSolicitante, ec.excFechaRegistro
              FROM ExclusionCartera ec,
                   @tCarteraVigenteSinExclusionConvenio cv
              WHERE ec.excPersona = cv.carPersona AND ec.excTipoSolicitante = cv.carTipoSolicitante AND ec.excEstadoExclusionCartera = 'NO_ACTIVA'
                AND CONVERT(DATE, dbo.getLocalDate(), 111) = CONVERT(DATE, ec.excFechaRegistro, 111)
          END

      IF @lineaCobro IN ('LC4', 'LC5')
          BEGIN
              INSERT INTO @tCarteraVigenteSinExclusionConvenio (carId, carPersona, carTipoLineaCobro, carTipoAccionCobro, carTipoSolicitante, carPeriodoDeuda, carFechaCreacion, carFechaAsignacionAccion)
              SELECT carId,carPersona,carTipoLineaCobro,carTipoAccionCobro,carTipoSolicitante,carPeriodoDeuda,
              ...
              INNER JOIN CarteraAgrupadora cag ON cag.cagCartera = car.carId
              INNER JOIN (SELECT MIN(cagCartera) AS cagCartera
                            FROM CarteraAgrupadora
                            JOIN Cartera ON carId = cagCartera
                            WHERE carEstadoOperacion = 'VIGENTE' AND carTipoSolicitante IN ('INDEPENDIENTE', 'PENSIONADO') AND carTipoLineaCobro = @lineaCobro
                              AND (carDeudaPresunta IS NOT NULL AND carDeudaPresunta > 0) AND carTipoAccionCobro = @accionCobroReferencia
                            GROUP BY cagNumeroOperacion ) cav ON car.carId = cav.cagCartera
              LEFT JOIN ExclusionCartera exc ON (exc.excEstadoExclusionCartera = 'ACTIVA' AND exc.excTipoSolicitante = car.carTipoSolicitante
                      AND exc.excPersona = car.carPersona AND exc.excTipoExclusionCartera IN ('EXCLUSION_NEGOCIO', 'IMPOSICION_RECURSO', 'ACLARACION_MORA')  )
              LEFT JOIN (SELECT cop.copPersona, cop.copTipoSolicitante, ppc.ppcPeriodo
                        FROM ConvenioPago cop
                          JOIN PagoPeriodoConvenio ppc ON ppc.ppcConvenioPago = cop.copId
                        WHERE cop.copEstadoConvenioPago = 'ACTIVO') AS con ON (con.copPersona = car.carPersona AND con.copTipoSolicitante = car.carTipoSolicitante AND con.ppcPeriodo = car.carPeriodoDeuda)
      ...
      -- Escenarios
      ------------------------------------------------------------------------
      IF @lineaCobro in ('LC1','LC4','LC5')
          BEGIN
              IF @metodo = 'METODO_1'
                  BEGIN
                      -- METODO_1
                      IF @accionCobroFutura = 'A1'
                          BEGIN
                              IF @diasLimitePagoA = 1
                                  BEGIN
                                      ...
                                      WHERE carTipoAccionCobro = 'A01'
                                        AND @accionCobroFutura = 'A1'
                                        AND (@fechaActual = dbo.UFN_SumarDiasFecha(carFechaCreacion, (@diasLimitePagoA), 1)
                                          OR 0 < (SELECT COUNT(*)
                      ...
                      IF
                          ...
                          BEGIN
                              ...
                              SELECT carId, @accionCobroFutura, @fechaActual
                              FROM @tCarteraVigenteSinExclusionConvenio
                              WHERE carTipoAccionCobro = 'A1'
                                AND @accionCobroFutura = 'AB1'
                                AND @fechaActual =
                                    dbo.UFN_SumarDiasFecha(dbo.UFN_SumarDiasFecha(EOMonth(carFechaCreacion),  @diasGeneracionAvisoB, 1),  - @diasLimiteEnvioComunicadoA, 1)
                        END
  -- fin a              |-- inicio b --------------------------------------------------------------|
                      ...
                      IF
                          ...
                          BEGIN
                              ...
                              SELECT carId, @accionCobroFutura, @fechaActual
                              FROM @tCarteraVigenteSinExclusionConvenio
                              WHERE carTipoAccionCobro = 'AB1'
                                AND @accionCobroFutura = 'B1'
                                AND (@fechaActual = dbo.UFN_SumarDiasFecha(EOMonth(carFechaCreacion),@diasGeneracionAvisoB, 1)
                                  OR 0 < (SELECT COUNT(*)
                                          FROM @exclusionesConveniosInactivos ec
                                          WHERE ec.eciPersona = carPersona
                                            AND ec.eciTipoSolicitante = carTipoSolicitante))
                      ...
                      IF
                          ...
                          BEGIN
                              ...
                              SELECT carId, @accionCobroFutura, @fechaActual
                              FROM @tCarteraVigenteSinExclusionConvenio
                              WHERE carTipoAccionCobro = 'B1'
                                AND @accionCobroFutura = 'BC1'
                                AND @fechaActual = dbo.UFN_SumarDiasFecha(dbo.UFN_SumarDiasFecha(carFechaAsignacionAccion, @limiteEnvioComunicadoB,1),- @diasGeneracionAvisoB, 1)
                          END
                      ...
                      IF
                          ...
                          BEGIN
                              ...
                              WHERE carTipoAccionCobro = 'BC1'
                                ...
                                AND (@fechaActual = dbo.UFN_SumarDiasFecha(EOMonth(carFechaAsignacionAccion),@diasLiquidacionC, 1)
                                  OR 0 < (SELECT COUNT(*)
                                          FROM @exclusionesConveniosInactivos ec
                                          WHERE ec.eciPersona = carPersona
                                            AND ec.eciTipoSolicitante = carTipoSolicitante))
                      ...
                      IF
                          ...
                          BEGIN
                              ...
                              WHERE carTipoAccionCobro = 'C1'
                                ...
                                AND (
                                          @fechaActual = dbo.UFN_SumarDiasFecha(carFechaAsignacionAccion,@limiteEnvioDocumentoC, 0)
                                  ...
                                  BEGIN
                                      ...
                                      WHERE carTipoAccionCobro = 'CD1'
                                        ...
                                        AND (@fechaActual = dbo.UFN_SumarDiasFecha(
                                              ISNULL(dbo.UFN_ObtenerFechaComunicadoCartera('CC1', @inicioDiasConteoD, carId), carFechaAsignacionAccion), @diasTranscurridosD - 1, 0)
                                          OR 0 < (SELECT COUNT(*)
                                                  FROM @exclusionesConveniosInactivos ec
                                                  WHERE ec.eciPersona = carPersona
                                                    AND ec.eciTipoSolicitante = carTipoSolicitante))
                      ...
                      BEGIN
                          ...
                          WHERE carTipoAccionCobro = 'CD1'
                            AND @accionCobroFutura = 'D1'
                            AND (@fechaActual = dbo.UFN_SumarDiasFecha(ISNULL(dbo.UFN_ObtenerFechaComunicadoCartera('CC1', @inicioDiasConteoD, carId),carFechaAsignacionAccion), @diasTranscurridosD, 0)
                              OR 0 < (SELECT COUNT(*)
                                      FROM @exclusionesConveniosInactivos ec
                                      WHERE ec.eciPersona = carPersona
                                        AND ec.eciTipoSolicitante = carTipoSolicitante))
                      ...
                      IF
                          ...
                          BEGIN
                              ...
                              WHERE carTipoAccionCobro = 'D1'
                                ...
                                AND (@fechaActual = dbo.UFN_SumarDiasFecha(carFechaAsignacionAccion,@diasLimiteEnvioD - @diasTranscurridosD,0)
                          ...
                          BEGIN
                              ...
                              WHERE @accionCobroFutura = 'E1'
                                AND ((
                                             ...
                                             AND @fechaActual = dbo.UFN_SumarDiasFecha(dbo.UFN_ObtenerFechaComunicadoCartera('D1', @inicioDiasConteoEPersuasivo,carId),@diasTranscurridosE, 0)
                                         )
                                  OR (
                                             0 < (SELECT COUNT(*)
                                                  FROM @exclusionesConveniosInactivos ec
                      ...
                      IF
                          ...
                          BEGIN
                              ...
                              WHERE carTipoAccionCobro = 'E1'
                                ...
                                AND (@fechaActual = dbo.UFN_SumarDiasFecha(carFechaAsignacionAccion,@diasLimiteEnvioE - @diasTranscurridosE,0)
                                      OR @fechaActual = dbo.UFN_SumarDiasFecha(carFechaAsignacionAccion,@diasLimiteEnvioEPersuasivo -@diasTranscurridosEPersuasivo,0)
                                      OR dbo.UFN_ObtenerFechaComunicadoCartera('C1', @inicioDiasConteoE,carId) IS NOT NULL
                                      OR dbo.UFN_ObtenerFechaComunicadoCartera('D1', @inicioDiasConteoEPersuasivo,carId) IS NOT NULL
                                  )
                          END

                      IF @accionCobroFutura = 'F1'
                          BEGIN
                              SET @siguienteAccion = 'FECHA_ENVIO'
                              IF @siguienteAccionF = 'REGISTRO_RECEPCION_COMUNICADO'
                                  BEGIN
                                      SET @siguienteAccion = 'FECHA_ENTREGA'
                                  END
                              ...
                              WHERE carTipoAccionCobro = 'EF1'
                                ...
                                AND (@fechaActual = dbo.UFN_SumarDiasFecha( dbo.UFN_ObtenerFechaComunicadoCartera('E1', @siguienteAccion, carId),@diasParametrizadosF,0)
                                  OR 0 < (SELECT COUNT(*)
                                          FROM @exclusionesConveniosInactivos ec
                                          WHERE ec.eciPersona = carPersona
                                            AND ec.eciTipoSolicitante = carTipoSolicitante))
              ...
              ELSE
                  BEGIN
                      ...
                      DECLARE @tBitacoras AS TABLE (idCartera BIGINT,actividad VARCHAR(50),fechaBitacora DATE)
                          ...
                          BEGIN
                              IF
                                  ...
                                  BEGIN
                                      ...
                                      SELECT carId, @accionCobroFutura, @fechaActual
                                      FROM @tCarteraVigenteSinExclusionConvenio
                                      WHERE carTipoAccionCobro = 'A02'
                                        AND @accionCobroFutura = 'A2'
                                        AND (@fechaActual = dbo.UFN_SumarDiasFecha(carFechaCreacion,
                                                                                   @diasLimitePagoA, 1)
                                          OR 0 < (SELECT COUNT(*)
                                                  FROM @exclusionesConveniosInactivos ec
                                                  WHERE ec.eciPersona = carPersona
                              ...
                              ELSE
                                  BEGIN
                                      ...
                                      WHERE carTipoAccionCobro = 'A02'
                                        AND @accionCobroFutura = 'A2'
                                        AND (@fechaActual =
                                             dbo.UFN_SumarDiasFecha(carFechaCreacion, (@diasLimitePagoA - 1), 1)
                                          OR 0 < (SELECT COUNT(*)
                                                  FROM @exclusionesConveniosInactivos ec
                                                  WHERE ec.eciPersona = carPersona
                                                    AND ec.eciTipoSolicitante = carTipoSolicitante))
                      ...
                      IF @accionCobroFutura = 'AB2'
                          BEGIN
                              INSERT INTO @carteraActualizada (carId, carTipoAccionCobro, carFechaAsignacionAccion)
                              SELECT carId, @accionCobroFutura, @fechaActual
                              FROM @tCarteraVigenteSinExclusionConvenio
                              WHERE carTipoAccionCobro = 'A2'
                                AND @accionCobroFutura = 'AB2'
                                AND @fechaActual =
                                    dbo.UFN_SumarDiasFecha(dbo.UFN_SumarDiasFecha(EOMonth(carFechaCreacion),
                                                                                  @diasGeneracionAvisoB, 1),
                                                           - @diasLimiteEnvioComunicadoA, 1)
                          END

                      IF @accionCobroFutura = 'B2'
                          BEGIN
                              INSERT INTO @carteraActualizada (carId, carTipoAccionCobro, carFechaAsignacionAccion)
                              SELECT carId, @accionCobroFutura, @fechaActual
                              FROM @tCarteraVigenteSinExclusionConvenio
                              WHERE carTipoAccionCobro = 'AB2'
                                AND @accionCobroFutura = 'B2'
                                AND (@fechaActual = dbo.UFN_SumarDiasFecha(EOMonth(carFechaCreacion),
                                                                           @diasGeneracionAvisoB, 1)
                                  OR 0 < (SELECT COUNT(*)
                                          FROM @exclusionesConveniosInactivos ec
                                          WHERE ec.eciPersona = carPersona
                      ...
                      IF @accionCobroFutura = 'C2'
                          BEGIN
                              ...
                              WHERE carTipoAccionCobro = 'BC2'
                                ...
                                AND (@fechaActual = dbo.UFN_SumarDiasFecha(EOMonth(carFechaAsignacionAccion),@diasEjecucionC, 1)
                                  OR 0 < (SELECT COUNT(*)
                                          FROM @exclusionesConveniosInactivos ec
                                          WHERE ec.eciPersona = carPersona
                                            AND ec.eciTipoSolicitante = carTipoSolicitante))
                      ...
                      IF @accionCobroFutura = 'CD2'
                          BEGIN
                              ...
                              WHERE carTipoAccionCobro = 'C2'
                                ...
                                AND dbo.UFN_ObtenerFechaComunicadoCartera('C2','FECHA_ENVIO',carId) IS NOT NULL
                          END

                      IF @accionCobroFutura = 'D2'
                          BEGIN
                              INSERT INTO @carteraActualizada (carId, carTipoAccionCobro, carFechaAsignacionAccion)
                            SELECT carId, @accionCobroFutura, @fechaActual
                               FROM @tCarteraVigenteSinExclusionConvenio
                            LEFT JOIN NotificacionPersonal ON ntpCartera = carId
                            LEFT JOIN BitacoraCartera bca ON bca.bcaPersona = carPersona AND bca.bcaActividad ='REGISTRO_NOTIFICACION_PERSONAL'
                              WHERE carTipoAccionCobro = 'CD2'
                                AND @accionCobroFutura = 'D2'
                                AND ntpId IS NULL
                              AND bcaid IS NULL
                                AND (@fechaActual = dbo.UFN_SumarDiasFecha(dbo.UFN_ObtenerFechaComunicadoCartera('C2', @inicioDiasConteoD, carId),@diasTranscurridosD, 1)
                                  OR 0 < (SELECT COUNT(*)
                                          FROM @exclusionesConveniosInactivos ec
                                          WHERE ec.eciPersona = carPersona
                                            AND ec.eciTipoSolicitante = carTipoSolicitante))
                      ...
                      IF @accionCobroFutura = 'E2'
                          BEGIN
                              ...
                              WHERE carTipoAccionCobro = 'DE2'
                                ...
                                AND ntpId IS NULL
                                AND (@fechaActual = dbo.UFN_SumarDiasFecha((SELECT bcaFecha
                          ...
                          END
                      IF @accionCobroFutura = 'EF2'
                          BEGIN
                              INSERT INTO @carteraActualizada (carId, carTipoAccionCobro, carFechaAsignacionAccion)
                              SELECT carId, @accionCobroFutura, @fechaActual
                              FROM Solicitud sol
                              INNER JOIN SolicitudGestionCobroFisico sgf ON sol.solId = sgf.sgfSolicitud
                              INNER JOIN DetalleSolicitudGestionCobro dsg ON sgf.sgfId IN (dsg.dsgSolicitudPrimeraRemision,dsg.dsgSolicitudSegundaRemision)
      ...
      -- Inicio actualización de datos de cartera con su auditoría
      IF
              (SELECT COUNT(1) FROM @carteraActualizada) > 0
          BEGIN
              UPDATE car SET car.carTipoAccionCobro = cac.carTipoAccionCobro,car.carFechaAsignacionAccion = cac.carFechaAsignacionAccion
              FROM Cartera car
              INNER JOIN @carteraActualizada cac ON car.carId = cac.carId

              UPDATE car
              SET car.carTipoAccionCobro = @accionCobroFutura,car.carFechaAsignacionAccion = @fechaActual
              FROM Cartera car
      ...
      UPDATE BitacoraEjecucionSp SET besUltimoExito = dbo.getLocalDate(),besRegistrosAfectados = (SELECT COUNT(1) FROM @carteraActualizada car)
  ...
  -- Resultados del procedimiento. Se retornan únicamente aquellos registros para los cuales se enviará comunicado o se escalará solicitud
      IF @lineaCobro IN ('LC1', 'LC2', 'LC3')
          BEGIN
            ...
            INNER JOIN @carteraActualizada cac ON car.carId = cac.carId
            WHERE car.carTipoAccionCobro IN ('A1', 'B1', 'C1', 'D1', 'E1', 'F1', 'A2', 'B2', 'C2', 'D2', 'E2', 'F2', 'G2', 'H2', 'LC2A', 'LC3A','LC4A', 'LC4C', 'LC5A', 'LC5C')
                AND ube.ubeTipoUbicacion = 'UBICACION_PRINCIPAL'
          END
      IF @lineaCobro IN ('LC4', 'LC5')
          BEGIN
              SELECT ISNULL(ubi.ubiAutorizacionEnvioEmail, 0) autoriza,
                     car.carTipoSolicitante,cac.carId,cac.carTipoAccionCobro,per.perTipoIdentificacion tipoIdentificacion,per.perNumeroIdentificacion numeroIdentificacion
              FROM Cartera car
              INNER JOIN Persona per ON per.perId = car.carPersona
      ...
      SET @cantidadRegistros = (SELECT COUNT(*) FROM @carteraActualizada)

      INSERT INTO TiempoProcesoCartera(tpcNombreProceso, tpcFechaInicio, tpcFechaFin, tpcRegistros, tpcMensaje)
      VALUES ('USP_ExecuteCARTERAAsignarAccionCobro', @fechaInicioEjecucion, dbo.getLocalDate(), @cantidadRegistros, @accionCobroFutura);

  END TRY
  BEGIN CATCH
      -- se marca en bitácora el fin de la ejecución fallida
      update BitacoraEjecucionSp set besUltimoFallo = dbo.getLocalDate(),besRegistrosAfectados = 0
      where besNombreStoredProcedure = 'USP_ExecuteCARTERAAsignarAccionCobro'
      ...
      VALUES (dbo.getLocalDate(), 'USP_ExecuteCARTERAAsignarAccionCobro debug:',+ '|accionCobroFutura:' + CAST(@accionCobroFutura AS VARCHAR)+ '|lineaCobro:' + CAST(@lineaCobro AS VARCHAR)+ '|metodo:' + CAST(@metodo AS VARCHAR));

      INSERT INTO CarteraLogError(cleFecha, cleNombreSP, cleaMensaje)
      VALUES (dbo.getLocalDate(), 'USP_ExecuteCARTERAAsignarAccionCobro error linea:' + CAST(ERROR_LINE() AS VARCHAR),@ErrorMessage);

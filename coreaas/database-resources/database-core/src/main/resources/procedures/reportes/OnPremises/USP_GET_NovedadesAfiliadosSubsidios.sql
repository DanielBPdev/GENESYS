...
-- Create date: 2018/31/10
-- Update date: 2018/20/11
-- Description:	SP que lista los datos del reporte normativo Archivo de novedades de afiliados y subsidios de compensación familiar
-- =============================================
ALTER PROCEDURE USP_GET_NovedadesAfiliadosSubsidios
	@dFechaInicial DATE,
	@dFechaFin DATE,
	@bCount BIT
AS
BEGIN
	print 'Inicia USP_GET_NovedadesAfiliadosSubsidios'
	...
	CREATE TABLE #PersonasModificacionDatosBasicos(perId BIGINT, actualNumeroId VARCHAR(16) COLLATE Latin1_General_CI_AI , actualTipoId VARCHAR(20) COLLATE Latin1_General_CI_AI , anteriorNumeroId VARCHAR(16) COLLATE Latin1_General_CI_AI , anteriorTipoId VARCHAR(20) COLLATE Latin1_General_CI_AI )
	CREATE TABLE #FechasNovedadesSolicituNovedad(snoId BIGINT, fechaRegistroNovedadCerrada DATE)
	CREATE TABLE #FechasNovedadesSolicituNovedadFovis(snfId BIGINT, fechaRegistroNovedadCerrada DATE)
	...
	SET @sql = '
	 ...
	 FROM ##core_aud##.dbo.Persona_aud perA
	 INNER JOIN ( 
		 SELECT per.perId,per.REV,per.perNumeroIdentificacion, per.perTipoIdentificacion
		 FROM ##core_aud##.dbo.Persona_aud per 
		 inner JOIN ##core_aud##.dbo.Persona_aud perAux ON perAux.perId = per.perId AND perAux.perNumeroIdentificacion <> per.perNumeroIdentificacion
	 ...
	 FROM ##core_aud##.dbo.Persona_aud perB
	 INNER JOIN(
	 	 SELECT  per.perId, per.REV, per.perNumeroIdentificacion, per.perTipoIdentificacion
		 FROM ##core_aud##.dbo.Persona_aud per 
		 INNER JOIN ##core_aud##.dbo.Persona_aud perAux ON perAux.perId = per.perId AND perAux.perTipoIdentificacion <> per.perTipoIdentificacion
		 GROUP BY per.perNumeroIdentificacion, per.perTipoIdentificacion,per.REV,per.perId 
	...
	INSERT INTO #PersonasModificacionDatosBasicos (perId , actualNumeroId, actualTipoId, anteriorNumeroId, anteriorTipoId)
	...
	FROM ##core_aud##.dbo.SolicitudNovedad_aud sno 
	INNER JOIN ##core_aud##.dbo.Revision rev ON (sno.REV = rev.revId)
	INNER JOIN(
		SELECT sno1.snoId, sno1.REV  
		FROM  ##core_aud##.dbo.SolicitudNovedad_aud sno1 
	...
	INSERT INTO #FechasNovedadesSolicituNovedad(snoId, fechaRegistroNovedadCerrada)
	EXEC sp_executesql @sql	
   ...
   --SE OBTIENEN LAS FECHAS DE REGISTRO DE LAS NOVEDADES DE FOVISO QUE FUERON APROBADAS Y LUEGO CERRADAS CON EL ID DE LA SOLICITUD DE NOVEDAD FOVIS
	...
	FROM ##core_aud##.dbo.SolicitudNovedadFovis_aud sno 
	INNER JOIN ##core_aud##.dbo.Revision rev ON (sno.REV = rev.revId)
	INNER JOIN(
		SELECT sno1.snfId, sno1.REV  
		FROM  ##core_aud##.dbo.SolicitudNovedadFovis_aud sno1 
	...
	INSERT INTO #FechasNovedadesSolicituNovedadFovis(snfId, fechaRegistroNovedadCerrada)
	EXEC sp_executesql @sql	

IF @bCount = 0
BEGIN
	
	--PARTE CUOTA MONETARIA
	SELECT
	2 AS tipoRegistro,
			  ...
			  EXISTS
	     	  ( 
	     	    ...
	     	    AND abono.casIdCuentaAdmonSubsidioRelacionado IN (
		     	  	...
		     	  	AND CuentaAdministradorSubsidio.casFechaHoraCreacionRegistro BETWEEN @dFechaInicial AND @dFechaFin
		     	 )
	     	  	UNION
	     	  	...
	     	  	AND detalleAux.dsaFechaHoraCreacion BETWEEN @dFechaInicial AND @dFechaFin
	     	  )
	     	  THEN 1
	     	  ELSE 0 END
	     	 ) = 1 THEN 'C09' 
	...
	CASE
		...
		ELSE NULL
	END AS fechaNacimientoAfiliado,
	CASE
		WHEN dbo.ParametrizacionNovedad.novTipoTransaccion IN ('CAMBIO_NOMBRE_APELLIDOS_PERSONAS_DEPWEB', 'CAMBIO_NOMBRE_APELLIDOS_PERSONAS',
		     'CAMBIO_NOMBRE_APELLIDOS_PERSONAS_WEB', 'CAMBIO_GENERO_PERSONAS')--codigo noveddad c01
		THEN perAfiCore.perPrimerApellido 
		ELSE NULL
	END AS primerApellidoAfiliado,
	CASE
		WHEN dbo.ParametrizacionNovedad.novTipoTransaccion IN ('CAMBIO_NOMBRE_APELLIDOS_PERSONAS_DEPWEB', 'CAMBIO_NOMBRE_APELLIDOS_PERSONAS',
		     'CAMBIO_NOMBRE_APELLIDOS_PERSONAS_WEB', 'CAMBIO_GENERO_PERSONAS')--codigo noveddad c01
		THEN perAfiCore.perSegundoApellido 
		ELSE NULL
	END AS segundoApellidoAfiliado,
	CASE
		WHEN dbo.ParametrizacionNovedad.novTipoTransaccion IN ('CAMBIO_NOMBRE_APELLIDOS_PERSONAS_DEPWEB', 'CAMBIO_NOMBRE_APELLIDOS_PERSONAS',
		     'CAMBIO_NOMBRE_APELLIDOS_PERSONAS_WEB', 'CAMBIO_GENERO_PERSONAS') --codigo noveddad c01
		THEN perAfiCore.perPrimerNombre 
		ELSE NULL
	END AS primerNombreAfiliado,
	CASE
		WHEN dbo.ParametrizacionNovedad.novTipoTransaccion IN ('CAMBIO_NOMBRE_APELLIDOS_PERSONAS_DEPWEB', 'CAMBIO_NOMBRE_APELLIDOS_PERSONAS',
		     'CAMBIO_NOMBRE_APELLIDOS_PERSONAS_WEB', 'CAMBIO_GENERO_PERSONAS') --codigo noveddad c01
		THEN perAfiCore.perSegundoNombre 
		ELSE NULL
	END AS segundoNombreAfiliado,
	CASE 
		WHEN dbo.ParametrizacionNovedad.novTipoTransaccion IN ('CAMBIO_NOMBRE_APELLIDOS_PERSONAS_DEPWEB', 'CAMBIO_NOMBRE_APELLIDOS_PERSONAS',
		     'CAMBIO_NOMBRE_APELLIDOS_PERSONAS_WEB', 'CAMBIO_GENERO_PERSONAS')
	...
	( 
	  (dbo.ParametrizacionNovedad.novTipoTransaccion IN ('CAMBIO_NOMBRE_APELLIDOS_PERSONAS_DEPWEB', 'CAMBIO_NOMBRE_APELLIDOS_PERSONAS',
	   ...
	   'RETIRO_PENSIONADO_MENOR_1_5SM_2','RETIRO_PENSIONADO_PENSION_FAMILIAR','RETIRO_CONYUGE','RETIRO_HJO_BIOLOGICO','RETIRO_HIJASTRO','RETIRO_HERMANO_HUERFANO',
	   'RETIRO_HIJO_ADOPTIVO','RETIRO_BENEFICIARIO_CUSTODIA','RETIRO_PADRE','RETIRO_MADRE','RETIRAR_BENEFICIARIO_EDAD_MAYOR_IGUAL_18_NO_TIENE_CEDULA',
	   'RETIRAR_AUTOMATICAMENTE_BENEFICIARIO_X_EDAD','RETIRO_AUTOMATICO_POR_MORA') AND  novedadesAprobadas.fechaRegistroNovedadCerrada BETWEEN @dFechaInicial AND @dFechaFin)
	   OR (
			  SELECT CASE WHEN
			  EXISTS
	     	  ( 
	   ...
	   OR (afiModificacionCore.perId IS NOT NULL AND rolAfiCore.roaFechaAfiliacion BETWEEN @dFechaInicial AND @dFechaFin)
	   OR (benModificacionCore.perId IS NOT NULL AND benCore.benFechaAfiliacion BETWEEN @dFechaInicial AND @dFechaFin)
	   OR 
	   (
		   (
			  SELECT CASE WHEN
			  EXISTS
		 	  ...
		 	  THEN 1
		 	  ELSE 0 END
		 	) = 1  AND benCore.benFechaAfiliacion BETWEEN @dFechaInicial AND @dFechaFin
	   )
	)
	...
	CASE
	    ...
	    WHEN dbo.SolicitudNovedadFovis.snfId IS NOT NULL THEN 'C09' --si tiene registro en la tabla SolicituNovedadFovis es porque se realizo algún cambio en el subsidio
	...
	( 
	  (dbo.ParametrizacionNovedad.novTipoTransaccion IN ('CAMBIO_NOMBRE_APELLIDOS_PERSONAS_DEPWEB', 'CAMBIO_NOMBRE_APELLIDOS_PERSONAS',
	   ...
	   'RETIRO_HIJO_ADOPTIVO','RETIRO_BENEFICIARIO_CUSTODIA','RETIRO_PADRE','RETIRO_MADRE','RETIRAR_BENEFICIARIO_EDAD_MAYOR_IGUAL_18_NO_TIENE_CEDULA',
	   'RETIRAR_AUTOMATICAMENTE_BENEFICIARIO_X_EDAD','RETIRO_AUTOMATICO_POR_MORA') AND  novedadesAprobadas.fechaRegistroNovedadCerrada BETWEEN @dFechaInicial AND @dFechaFin)
	   OR (dbo.SolicitudNovedadFovis.snfId IS NOT NULL AND novedadesFovisAprobadas.fechaRegistroNovedadCerrada BETWEEN @dFechaInicial AND @dFechaFin)
	   OR (afiModificacionFovis.perId IS NOT NULL AND rolAfiFovis.roaFechaAfiliacion BETWEEN @dFechaInicial AND @dFechaFin)
	   OR (benModificacionFovis.perId IS NOT NULL AND benFovis.benFechaAfiliacion BETWEEN @dFechaInicial AND @dFechaFin)
	   OR 
	   (
		   (
			  SELECT CASE WHEN
			  EXISTS
		 	  ...
		 	  THEN 1
		 	  ELSE 0 END
		 	) = 1  AND benFovis.benFechaAfiliacion BETWEEN @dFechaInicial AND @dFechaFin
	   )
	)
...
BEGIN
	...
	FROM (
		...
		perAfiCore.perTipoIdentificacion, perAfiCore.perNumeroIdentificacion, perAfiCore.perPrimerApellido, perAfiCore.perSegundoApellido,
		perAfiCore.perPrimerNombre, perAfiCore.perSegundoNombre,perDetAfiCore.pedGenero,perDetAfiCore.pedFechaNacimiento, dbo.SolicitudLiquidacionSubsidio.slsFechaInicio,
		detalle.dsaValorSubsidioMonetario, cuenta.casEstadoTransaccionSubsidio, munAfiCore.munCodigo, depAfiCore.depCodigo,
		perBenCore.perTipoIdentificacion, perBenCore.perNumeroIdentificacion, perDetBenCore.pedGenero, perDetBenCore.pedFechaExpedicionDocumento,
		perBenCore.perPrimerApellido, perBenCore.perSegundoApellido, perBenCore.perPrimerNombre, perBenCore.perSegundoNombre, dbo.SolicitudLiquidacionSubsidio.slsId,
		rolAfiCore.roaFechaIngreso,rolAfiCore.roaTipoAfiliado, rolAfiCore.roaFechaRetiro, rolAfiCore.roaMotivoDesafiliacion, aporteGenAfiCore.apgPersona, 
		perAportanteAfiCore.perTipoIdentificacion, perAportanteAfiCore.perNumeroIdentificacion, perAportanteAfiCore.perPrimerNombre ,perAportanteAfiCore.perSegundoNombre,
...
END 
	print 'Finaliza USP_GET_NovedadesAfiliadosSubsidios'
END;

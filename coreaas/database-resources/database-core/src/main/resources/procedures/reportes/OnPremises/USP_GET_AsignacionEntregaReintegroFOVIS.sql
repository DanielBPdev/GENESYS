...
BEGIN
	...
	CREATE TABLE #Postulacion(pofId BIGINT, pofEstadoHogar VARCHAR(100), pofSolicitudAsignacion BIGINT)
		
	SET @sql = '
			SELECT DISTINCT pof.pofId , pof.pofEstadoHogar, pof.pofSolicitudAsignacion
			FROM ##core_aud##.dbo.PostulacionFovis_aud pof
			JOIN ##core_aud##.dbo.Revision rev ON (pof.REV = rev.revId)
	...
	SET @sql = REPLACE(@sql, '@dFechaInicial', @dFechaInicial)
	SET @sql = REPLACE(@sql, '@dFechaFin', @dFechaFin)
	SET @sql = N'SELECT * FROM OPENQUERY(LinkedServerCore, ''' + @sql + ''')'

	INSERT INTO #Postulacion (pofId, pofEstadoHogar, pofSolicitudAsignacion)

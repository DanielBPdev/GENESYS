...
-- Description:	Define el cursor para obtener la información de las plantillas
-- SUS_NTF_NO_PAG (Suspensión automática de servicios por mora y notificación de no recaudo) y 
-- NTF_NO_REC_APO (Notificación de no recaudo de aportes)
-- =================================================================================================
CREATE PROCEDURE [dbo].[USP_UTIL_CURSOR_SUS_NTF_NO_PAG__NTF_NO_REC_APO] 
	@idPlantilla bigint, 
...
AS
 ...
 SET @paramCursor = CURSOR FAST_FORWARD FOR
	...
	(
		...
		THEN prl.perSegundoNombre ELSE '' END + ' ' + prl.perPrimerApellido + ' ' + CASE WHEN prl.perSegundoApellido IS NOT NULL 
		...
		CONVERT(VARCHAR, '${direccionRepresentanteLegal}') as llave4, CONVERT(VARCHAR(100), ubi.ubiDireccionFisica) AS valor4,
		CONVERT(VARCHAR, '${telefonoRepresentanteLegal}') as llave5, CONVERT(VARCHAR, (CASE WHEN ubi.ubiTelefonoFijo+'' IS NOT NULL
		THEN ubi.ubiTelefonoFijo ELSE ubi.ubiTelefonoCelular END)) AS valor5,
		CONVERT(VARCHAR, '${ciudadRepresentanteLegal}') as llave6, CONVERT(VARCHAR(50), mun.munNombre) AS valor6,
		CONVERT(VARCHAR, '${fechaCorte}') as llave7, CONVERT(VARCHAR, getDate(), 120) as valor7,
		CONVERT(VARCHAR, '${periodos}')   as llave8, 'com.asopagos.comunicados.ejb.ObtenerPeriodosCartera' as valor8
		FROM Empresa empr,

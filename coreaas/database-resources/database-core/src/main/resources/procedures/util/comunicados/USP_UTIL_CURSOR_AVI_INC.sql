...
-- Description:	Define el cursor para obtener la información de la plantilla AVI_INC (Aviso de Incumplimiento)
-- =======================================================================================================================================
CREATE PROCEDURE [dbo].[USP_UTIL_CURSOR_AVI_INC] 
	@idPlantilla bigint, 
...
AS
 ...
 SET @paramCursor = CURSOR FAST_FORWARD FOR
	...
	(
		SELECT TOP 1 CONVERT(VARCHAR, '${fechaDelSistema}') as llave1, 
		...
		ELSE ubi.ubiTelefonoCelular END))  AS valor5,
		CONVERT(VARCHAR, '${ciudad}') as llave6, CONVERT(VARCHAR(50), mun.munNombre) AS valor6,
		CONVERT(VARCHAR, '${periodoMora}') as llave7, CONVERT(VARCHAR, LEFT(CONVERT(VARCHAR, min(car.carPeriodoDeuda), 120), 10)) as valor7,
		CONVERT(VARCHAR, '${fechaCorte}') as llave8, CONVERT(VARCHAR, getDate(), 120) as valor8
		FROM Empresa empr,
		...
		ubi.ubiTelefonoFijo,ubi.ubiTelefonoCelular, mun.munNombre, car.carPeriodoDeuda
	) p
	cross apply
	(
	     values

...
-- Description:	Define el cursor para obtener la información de la plantilla AVI_INC_PER(Aviso de incumplimiento Persona)
-- =======================================================================================================================================
CREATE PROCEDURE [dbo].[USP_UTIL_CURSOR_AVI_INC_PER] 
	@idPlantilla bigint, 
...
AS
 ...
 SET @paramCursor = CURSOR FAST_FORWARD FOR
	...
	(
		...
		CONVERT(VARCHAR, '${periodoMora}') as llave6, CONVERT(VARCHAR, LEFT(CONVERT(VARCHAR, min(car.carPeriodoDeuda), 120), 10)) as valor6,
		...
		AND (car.carTipoLineaCobro = 'LC4' or car.carTipoLineaCobro = 'LC5')
		AND car.carEstadoOperacion 		= 'VIGENTE'
		AND mun.munId					= ubi.ubiMunicipio
		AND ubi.ubiId					= per.perUbicacionPrincipal
		AND per.perTipoIdentificacion	= @tipoIdentificacion
		AND per.perNumeroIdentificacion	= @numeroIdentificacion
		GROUP by  per.perId, per.perPrimerNombre ,per.perSegundoNombre, 
		per.perSegundoNombre,per.perPrimerApellido , per.perSegundoApellido,  
		per.perSegundoApellido, ubi.ubiDireccionFisica, ubi.ubiTelefonoFijo,ubi.ubiTelefonoCelular, 
		mun.munNombre, car.carPeriodoDeuda
	) p
	cross apply
	(
	     values

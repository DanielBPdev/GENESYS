/****** Object:  StoredProcedure [dbo].[SP_FOVIS_Procesos_archivo_Cruces]    Script Date: 17/10/2024 7:47:04 a. m. ******/
/****** Object:  StoredProcedure [dbo].[SP_FOVIS_Procesos_archivo_Cruces]    Script Date: 7/09/2024 6:30:47 p.m. ******/
...
/****** Object:  StoredProcedure [dbo].[SP_FOVIS_Procesos_archivo_Cruces]    Script Date: 21/12/2023 15:51:00 ******/
/****** Object:  StoredProcedure [dbo].[SP_FOVIS_Procesos_archivo_Cruces]    Script Date: 17/10/2024 09:28:00 ******/
-- =============================================
-- Author:      <John Martínez>
-- Create Date: <2023-09-20>
-- Description: <Creación tabla temporal para generación de vista>
-- AJUSTADO: <2024-09-09>
-- Description: <Parametros para comparación de cruces disponibles en la base>
-- =============================================
ALTER   PROCEDURE [dbo].[SP_FOVIS_Procesos_archivo_Cruces]
(
	@idCargue bigint
)
AS
...
from
    CargueArchivoCruceFovisCedula
        ...
        AND crdNitEntidad = cfaNitEntidad
        AND crdNombreEntidad = cfaNombreEntidad
...
select cfbCargueArchivoCruceFovis
     ...
     ,cfbar.cfbApellidos apellidos
     ,cfbar.cfbNombres nombres
     ,cffFechaActualizacion
     ,cffFechaCorte
...
from
    CargueArchivoCruceFovisCedula
        inner join CargueArchivoCruceFovisBeneficiario cfbar
        ...
        INNER JOIN SolicitudPostulacion ON pofId = spoPostulacionFOVIS
        INNER JOIN Solicitud ON spoSolicitudGlobal = solId
        LEFT JOIN (select crd.crdid, crd.crdTipodocumento, crd.crdNumeroIdentificacion, crd.crdNitEntidad,
					crd.crdNombreEntidad, cfb.cfbValorAsignado, cfb.cfbFechaAsignacion,
					cfb.cfbNombres, cfb.cfbApellidos
					from CruceDetalle crd
					inner join Cruce CR on crd.crdCruce = CR.cruId
					inner join CargueArchivoCruceFovisBeneficiario cfb ON CR.cruCargueArchivoCruceFovis = cfb.cfbCargueArchivoCruceFovis
					WHERE crd.crdTipo='BENEFICIARIOS') crdc
					ON crdc.crdTipodocumento = cfbTipoIdentificacion
					AND crdc.crdNumeroIdentificacion = cfbIdentificacion
					AND crdc.crdNitEntidad = cfbNitEntidad
					AND crdc.crdNombreEntidad = cfbNombreEntidad
					AND crdc.cfbNombres = cfbar.cfbNombres
					AND crdc.cfbApellidos = cfbar.cfbApellidos
					AND crdc.cfbValorAsignado = cfbar.cfbValorAsignado
					AND crdc.cfbFechaAsignacion = cfbar.cfbFechaAsignacion
where cfcCargueArchivoCruceFovis = @idCargue
  AND pofEstadoHogar in ('POSTULADO','HABIL','HABIL_SEGUNDO_ANIO')
...
select cfbCargueArchivoCruceFovis
     ...
     ,tipoDocumento
     ,'BENEFICIARIOS' tipo
	 from(select cfbCargueArchivoCruceFovis
     ,solNumeroRadicacion
     ,perId
     ,CASE WHEN crdid IS NULL THEN 'NUEVO' ELSE 'PREVIAMENTE_REPORTADO' END estadoCruce
     ,GETDATE() fechaRegistro
...
from
    CargueArchivoCruceFovisCedula
        inner join CargueArchivoCruceFovisBeneficiario cfbar on cfcCargueArchivoCruceFovis = cfbCargueArchivoCruceFovis
         and cfcNroCedula = cfbIdentificacion
        INNER JOIN CargueArchivoCruceFovisFechasCorte ON cffCargueArchivoCruceFovis = cfbCargueArchivoCruceFovis
...
group by
    cfbCargueArchivoCruceFovis
	   ...
	   ,cfbId)as c
 inner join (select cfbid, min (cffFechaActualizacion) cffFechaActualizacion, min(cffFechaCorte)cffFechaCorte  from CargueArchivoCruceFovisFechasCorte
inner join CargueArchivoCruceFovisBeneficiario ON cffCargueArchivoCruceFovis = cfbCargueArchivoCruceFovis
 AND cffNitEntidad = cfbNitEntidad and cffTipoInformacion = 'BENEFICIARIOS' and cffNitEntidad in (8301212085,8999990389)
group by cfbid
)as cv on cv.cfbId = c.cfbId
...
from
    CargueArchivoCruceFovisCedula
        ...
        AND crdNitEntidad = cfdNitEntidad
        AND crdNombreEntidad = cfdNombreEntidad
        AND crdApellidosNombres = cfdApellidosNombres
        AND crdCedulaCatastral = cfdCedulaCatastral
        AND crdDireccionInmueble = cfdDireccion
        AND crdMatriculaInmobiliaria = cfdMatriculaInmobiliaria
        AND crdDepartamento = cfdDepartamento
        AND crdMunicipio = cfdMunicipio
where cfcCargueArchivoCruceFovis = @idCargue
  AND pofEstadoHogar in ('POSTULADO','HABIL','HABIL_SEGUNDO_ANIO')
  AND crdid IS NULL

...
/**
 * CLASE QUE VALIDA:
 * si la persona está registrada como afiliado dependiente y estado inactivo.
 * 
 * @author Mario Monroy <mamonroy@heinsohn.com.co>
 */
public class ValidadorPersonaDependienteInactivo extends ValidadorAbstract {
    /*
     * (non-Javadoc)
     * 
    ...
    public ValidacionDTO execute(Map<String, String> datosValidacion) {
        logger.debug("Inicio de método ValidadorPersonaDependienteInactivo.execute");
        try{
            ...
            if(datosValidacion.get(ConstantesValidaciones.TIPO_ID_BENEF_PARAM)!= null && 
                ...
                if(rolAfiliado != null && !rolAfiliado.isEmpty()){
                    //validación fallida
                    logger.debug("HABILITADA- Fin de método ValidadorPersonaDependienteInactivo.execute");
                    return crearMensajeExitoso(ValidacionCoreEnum.VALIDACION_PERSONA_DEPENDIENTE_INACTIVO);
                    
                }else{
                    //validación exitosa
                    logger.debug("NO HABILITADA- Fin de método ValidadorPersonaDependienteInactivo.execute");
                    return crearValidacion(myResources.getString(ConstantesValidaciones.KEY_PERSONA_DEPENDIENTE_ACTIVO_REVERSE),ResultadoValidacionEnum.NO_APROBADA,
                            ValidacionCoreEnum.VALIDACION_PERSONA_DEPENDIENTE_INACTIVO,
            ...
            return crearMensajeNoEvaluado(ConstantesValidaciones.KEY_VALIDACION_NOVEDAD_ABORTADA_POR_EXCEPCION,
                    ValidacionCoreEnum.VALIDACION_PERSONA_DEPENDIENTE_INACTIVO, TipoExcepcionFuncionalEnum.EXCEPCION_TECNICA);
        }
    }
}
...
/**
 * CLASE QUE VALIDA:
 * Validar que el beneficiario tipo padre sujeto de la novedad 
 * en el campo "Estado con respecto al afiliado principal" hasta 
 * el momento de la recepción de la novedad se encuentre "Activo"
 * 
 * 
 * @author Steven Quintero González <squintero@heinsohn.com.co>
 */
public class ValidadorPadreActivo extends ValidadorAbstract {
	/*
	 * (non-Javadoc)
	 * 
	...
	public ValidacionDTO execute(Map<String, String> datosValidacion) {
		logger.debug("Inicio de método ValidadorPadreActivo.execute");
		try{
			...
			if(beneficiario != null && ValidacionPersonaUtils.obtenerListaEstadoActivo().contains(beneficiario.getEstadoBeneficiarioAfiliado())
					&& ValidacionPersonaUtils.obtenerClasificacionPadre().contains(beneficiario.getTipoBeneficiario())){
				//validación exitosa
				logger.debug("HABILITADA- Fin de método ValidadorPadreActivo.execute");
				return crearMensajeExitoso(ValidacionCoreEnum.VALIDACION_PADRE_ACTIVO);	
			}else{
				//validación fallida
				logger.debug("NO HABILITADA- Fin de método ValidadorPadreActivo.execute");
				return crearValidacion(myResources.getString(ConstantesValidaciones.KEY_PADRE_ACTIVO),ResultadoValidacionEnum.NO_APROBADA,
						ValidacionCoreEnum.VALIDACION_PADRE_ACTIVO,
						TipoExcepcionFuncionalEnum.EXCEPCION_FUNCIONAL_TIPO_2);
			}
		} catch (Exception e) {
			logger.error("No evaluado - Ocurrió alguna excepción", e);
			return crearMensajeNoEvaluado(ConstantesValidaciones.KEY_VALIDACION_NOVEDAD_ABORTADA_POR_EXCEPCION,
					ValidacionCoreEnum.VALIDACION_PADRE_ACTIVO, TipoExcepcionFuncionalEnum.EXCEPCION_TECNICA);
		}
	}
}
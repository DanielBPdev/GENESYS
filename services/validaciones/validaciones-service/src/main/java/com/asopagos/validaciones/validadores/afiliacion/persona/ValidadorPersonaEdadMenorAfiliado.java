 ...
 */
public class ValidadorPersonaEdadMenorAfiliado extends ValidadorAbstract {

	/**
	 * Metodo encargado de verificar si la fecha de nacimiento del beneficiario
	 * es menor a la del afiliado principal
	 */
	@Override
	public ValidacionDTO execute(Map<String, String> datosValidacion) {
		try {
			logger.debug("Inicio ValidadorPersonaEdadMenorAfiliado");
			if (datosValidacion != null && !datosValidacion.isEmpty()) {
				...
				//Condicion para cuando la validacion viene desde ingreso beneficiario PILA
				...
				if (fechaNacimiento == null || fechaNacimiento.equals("")) {
					return crearMensajeExitoso(ValidacionCoreEnum.VALIDACION_PERSONA_MENOR_AFILIADO);
				} else {
					if (tipoIdentificacion != null && !tipoIdentificacion.equals("") && numeroIdentificacion != null
						...
						List<Persona> personaTipoIdentificacion = entityManager
								...
								.getResultList();
						logger.info("personaTipoIdentificacion " +personaTipoIdentificacion.toString());
						try {
							personaDetalle = entityManager
									...
									.setParameter(ConstantesValidaciones.NUM_ID_PARAM, numeroIdentificacion)
									.getSingleResult();
							logger.info("personaDetalle " +personaDetalle.toString());
						} catch (NoResultException e) {
							personaDetalle = null;
						}
						if (!personaTipoIdentificacion.isEmpty()) {
							logger.info("personaEncontrado 1.0 " +personaEncontrado);
							...
							List<Persona> personaIdentificacion = entityManager
									...
									.setParameter(ConstantesValidaciones.NUM_ID_PARAM, numeroIdentificacion)
									.getResultList();
							logger.info("personaIdentificacion " +personaDetalle.toString());
							try {
								personaDetalle = entityManager
										.createNamedQuery(
										...
										.setParameter(ConstantesValidaciones.NUM_ID_PARAM, numeroIdentificacion)
										.getSingleResult();
								logger.info("personaDetalle 2.0" +personaDetalle.toString());
							} catch (NoResultException e) {
								personaDetalle = null;
							}
							if (!personaIdentificacion.isEmpty()) {
								logger.info("personaEncontrado 2.0 " +personaEncontrado);
								personaEncontrado = personaTipoIdentificacion.get(0);
							}
						}
						logger.info("personaEncontrado 3.0 " +personaEncontrado);
						logger.info("personaDetalle 3.0 " +personaDetalle);
						if (personaEncontrado != null && personaDetalle != null) {
							 ...
							 */
							logger.info("fNacimiento.getTime() 3.0 " +personaDetalle.getFechaNacimiento().getTime());
							if (fNacimiento.getTime() > personaDetalle.getFechaNacimiento().getTime()) {
								logger.info("ingresa a validacion ");
								return crearValidacion(
										myResources
												.getString(ConstantesValidaciones.KEY_PERSONA_MENOR_AFILIADO_PRINCIPAL),
										ResultadoValidacionEnum.NO_APROBADA,
										ValidacionCoreEnum.VALIDACION_PERSONA_MENOR_AFILIADO,
	...
	private ValidacionDTO crearMensajeNoEvaluado() {
		return crearValidacion(
				myResources.getString(ConstantesValidaciones.KEY_MENSAJE_NO_EV) + SEPARADOR
						+ myResources.getString(ConstantesValidaciones.KEY_PERSONA_MENOR_AFILIADO_PRINCIPAL),
				ResultadoValidacionEnum.NO_EVALUADA, ValidacionCoreEnum.VALIDACION_PERSONA_MENOR_AFILIADO,

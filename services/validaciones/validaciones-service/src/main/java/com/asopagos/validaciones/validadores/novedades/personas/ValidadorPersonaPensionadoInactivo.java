...
/**
 * CLASE QUE VALIDA:
 * si la persona está registrada como afiliado pensionado y estado inactivo.
 * 
 * @author Mario Monroy <mamonroy@heinsohn.com.co>
 */
public class ValidadorPersonaPensionadoInactivo extends ValidadorAbstract {
    /*
     * (non-Javadoc)
     * 
    ...
    public ValidacionDTO execute(Map<String, String> datosValidacion) {
        logger.debug("Inicio de método ValidadorPersonaPensionadoInactivo.execute");
        try{
            ...
            if(datosValidacion.get(ConstantesValidaciones.TIPO_ID_BENEF_PARAM)!= null && 
                ...
                if(rolAfiliado != null && !rolAfiliado.isEmpty()){
                    //validación exitosa
                    logger.debug("HABILITADA- Fin de método ValidadorPersonaPensionadoInactivo.execute");
                    return crearMensajeExitoso(ValidacionCoreEnum.VALIDACION_PERSONA_PENSIONADO_INACTIVO);
                    
                }else{
                    //validación fallida
                    logger.debug("NO HABILITADA- Fin de método ValidadorPersonaPensionadoInactivo.execute");
                    return crearValidacion(myResources.getString(ConstantesValidaciones.KEY_PERSONA_PENSIONADO_ACTIVO_REVERSE),ResultadoValidacionEnum.NO_APROBADA,
                            ValidacionCoreEnum.VALIDACION_PERSONA_PENSIONADO_INACTIVO,
            ...
            return crearMensajeNoEvaluado(ConstantesValidaciones.KEY_VALIDACION_NOVEDAD_ABORTADA_POR_EXCEPCION,
                    ValidacionCoreEnum.VALIDACION_PERSONA_PENSIONADO_INACTIVO, TipoExcepcionFuncionalEnum.EXCEPCION_TECNICA);
        }
    }
}
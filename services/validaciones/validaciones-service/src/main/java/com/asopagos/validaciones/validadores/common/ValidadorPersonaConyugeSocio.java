 ...
 */
public class ValidadorPersonaConyugeSocio extends ValidadorAbstract {
    ...
    public ValidacionDTO execute(Map<String, String> datosValidacion) {
        logger.debug("Inicio de método ValidadorPersonaConyugeSocio.execute");
        try {
            if (datosValidacion != null && !datosValidacion.isEmpty()) {
                ...
                if (tipoIdentificacion != null && !tipoIdentificacion.equals("")) {
                    ...
                    List<SocioEmpleador> personasConTipoYNumero = entityManager
                            .createNamedQuery(NamedQueriesConstants.BUSCAR_CONYUGE_SOCIO_EMPLEADOR_POR_SOCIO_EMPLEADOR_TIPO_SOLICITANTE)
                            .setParameter(ConstantesValidaciones.TIPO_ID_PARAM, tipoIdentificacion)
                            .setParameter(ConstantesValidaciones.NUM_ID_PARAM, numeroIdentificacion)
                            .setParameter(ConstantesValidaciones.CLASIFICACION_PARAM,
                                    ValidacionPersonaUtils.obtenerClasificacionNaturalYDomestico())
                    ...
                    if (personasConTipoYNumero == null || personasConTipoYNumero.isEmpty()) {
                        List<SocioEmpleador> personasConNumero = entityManager
                                .createNamedQuery(NamedQueriesConstants.BUSCAR_CONYUGE_SOCIO_EMPLEADOR_POR_SOCIO_EMPLEADOR_TIPO_SOLICITANTE)
                                .setParameter(ConstantesValidaciones.TIPO_ID_PARAM, null)
                                .setParameter(ConstantesValidaciones.NUM_ID_PARAM, numeroIdentificacion)
                                .setParameter(ConstantesValidaciones.CLASIFICACION_PARAM,
                                        ValidacionPersonaUtils.obtenerClasificacionNaturalYDomestico())
                                .setParameter(ConstantesValidaciones.TIPO_ID_EMPLEADOR_PARAM, tipoIdentificacionEmpleador)
                    ...
                    if (existe) {
                        logger.debug(
                                "No aprobada- La persona es conyuge del socio del empleador y esta como tipo solicitante persona natural o domestico");
                        return crearValidacion(myResources.getString(ConstantesValidaciones.KEY_PERSONA_CONYUGE_SOCIO_EMPLEADOR),
                                ResultadoValidacionEnum.NO_APROBADA, ValidacionCoreEnum.VALIDACION_PERSONA_CONYUGE_SOCIO,
            ...
            return crearMensajeExitoso(ValidacionCoreEnum.VALIDACION_PERSONA_CONYUGE_SOCIO);
        } catch (Exception e) {
    ...
    private ValidacionDTO crearMensajeNoEvaluado() {
        return crearValidacion(
                myResources.getString(ConstantesValidaciones.KEY_MENSAJE_NO_EV) + SEPARADOR
                        + myResources.getString(ConstantesValidaciones.KEY_PERSONA_CONYUGE_SOCIO_EMPLEADOR),
                ResultadoValidacionEnum.NO_EVALUADA, ValidacionCoreEnum.VALIDACION_PERSONA_CONYUGE_SOCIO,
                TipoExcepcionFuncionalEnum.EXCEPCION_FUNCIONAL_TIPO_1);
    }

...
/**
 * CLASE QUE VALIDA:
 * El sistema verifica que para el empleador de origen, el campo "estado de afiliación" tenga el valor "Activo".
 *  
 * @author Steven Quintero González <squintero@heinsohn.com.co>
 */
public class ValidadorEmpleadorOrigenActivo extends ValidadorAbstract {
    /*
     * (non-Javadoc)
     * 
    ...
    public ValidacionDTO execute(Map<String, String> datosValidacion) {
        logger.debug("Inicio de método ValidadorEmpleadorOrigenActivo.execute");
        try{
            ...
            if(estadosValidos.contains(empleador.getEstadoEmpleador())){
                logger.debug("VALIDACION EXITOSA- Fin de método ValidadorEmpleadorOrigenActivo.execute");
                // Validación exitosa, Validador 28 El empleador destino debe estar "Activo"
                return crearMensajeExitoso(ValidacionCoreEnum.VALIDACION_EMPLEADOR_ORIGEN_ACTIVO);  
            }else{
                logger.debug("VALIDACION FALLIDA- Fin de método ValidadorEmpleadorOrigenActivo.execute");
                // Validación no aprobada, 28   El empleador destino debe estar "Activo"
                return crearValidacion(myResources.getString(ConstantesValidaciones.KEY_EMPLEADOR_ORIGEN_ACTIVO),ResultadoValidacionEnum.NO_APROBADA,
                        ValidacionCoreEnum.VALIDACION_EMPLEADOR_ORIGEN_ACTIVO,
                        TipoExcepcionFuncionalEnum.EXCEPCION_FUNCIONAL_TIPO_2);
            }
        } catch (Exception e) {
            logger.error("No evaluado - Ocurrió alguna excepción", e);
            return crearMensajeNoEvaluado(ConstantesValidaciones.KEY_VALIDACION_NOVEDAD_ABORTADA_POR_EXCEPCION,
                    ValidacionCoreEnum.VALIDACION_EMPLEADOR_ORIGEN_ACTIVO, TipoExcepcionFuncionalEnum.EXCEPCION_TECNICA);
        }
    }
}

...
/**
 * CLASE QUE VALIDA:
 * si la persona está registrada como beneficiario conyuge y estado activo
 * 
 * @author Steven Quintero González <squintero@heinsohn.com.co>
 */
public class ValidadorPersonaComoConyugeActivo extends ValidadorAbstract {
	/*
	 * (non-Javadoc)
	 * 
	...
	public ValidacionDTO execute(Map<String, String> datosValidacion) {
		logger.info("Inicio de método ValidadorAfiliadoActivo.execute");
		try{
			...
			if(datosValidacion.get(ConstantesValidaciones.TIPO_ID_BENEF_PARAM)!= null && 
				...
				logger.info("ValidadorPersonaComoConyugeActivo isIngresoRetiro " +isIngresoRetiro);
				...
				logger.info("ValidadorPersonaComoConyugeActivo beneficiario " +beneficiario.size());
		        //se realiza la validación
				if(beneficiario != null && !beneficiario.isEmpty()){
					if(isIngresoRetiro != null && isIngresoRetiro){
						logger.info("ValidadorPersonaComoConyugeActivo isIngresoRetiro " +isIngresoRetiro);
						return crearMensajeExitoso(ValidacionCoreEnum.VALIDACION_PERSONA_CONYUGE_ACTIVO);
					}
					//validación fallida
					logger.info("NO HABILITADA- Fin de método ValidadorAfiliadoActivo.execute 1");
					return crearValidacion(myResources.getString(ConstantesValidaciones.KEY_PERSONA_CONYUGE_ACTIVO),ResultadoValidacionEnum.NO_APROBADA,
							ValidacionCoreEnum.VALIDACION_PERSONA_CONYUGE_ACTIVO,
				...
				if(beneficiario != null && !beneficiario.isEmpty()){
					//validación fallida
					logger.info("NO HABILITADA- Fin de método ValidadorAfiliadoActivo.execute 2");
					return crearValidacion(myResources.getString(ConstantesValidaciones.KEY_PERSONA_CONYUGE_ACTIVO),ResultadoValidacionEnum.NO_APROBADA,
							ValidacionCoreEnum.VALIDACION_PERSONA_CONYUGE_ACTIVO,
							TipoExcepcionFuncionalEnum.EXCEPCION_FUNCIONAL_TIPO_2);	
				}else{
			...
			return crearMensajeNoEvaluado(ConstantesValidaciones.KEY_VALIDACION_NOVEDAD_ABORTADA_POR_EXCEPCION,
					ValidacionCoreEnum.VALIDACION_PERSONA_CONYUGE_ACTIVO, TipoExcepcionFuncionalEnum.EXCEPCION_TECNICA);
		}
	}
}
...
/**
 * CLASE QUE VALIDA:
 * Valor del campo "Estado con respecto al afiliado principal": "Activo", "Inactivo"
 * 
 * @author Steven Quintero González <squintero@heinsohn.com.co>
 */
public class ValidadorEstadoRespectoAfiliadoPpal extends ValidadorAbstract {
	/*
	 * (non-Javadoc)
	 * 
	...
	public ValidacionDTO execute(Map<String, String> datosValidacion) {
		logger.debug("Inicio de método ValidadorEstadoRespectoAfiliadoPpal.execute");
		try{
			...
			if(beneficiario != null && ValidacionPersonaUtils.obtenerEstadosAfiliado().contains(beneficiario.getEstadoBeneficiarioAfiliado())){
				//validación exitosa
				logger.debug("HABILITADA- Fin de método ValidadorEstadoRespectoAfiliadoPpal.execute");
				return crearMensajeExitoso(ValidacionCoreEnum.VALIDACION_ESTADO_RESPECTO_AFILIADO_PPAL);	
			}else{
				//validación fallida
				logger.debug("NO HABILITADA- Fin de método ValidadorEstadoRespectoAfiliadoPpal.execute");
				return crearValidacion(myResources.getString(ConstantesValidaciones.KEY_ESTADO_RESPECTO_AFILIADO_PPAL),ResultadoValidacionEnum.NO_APROBADA,
						ValidacionCoreEnum.VALIDACION_ESTADO_RESPECTO_AFILIADO_PPAL,
						TipoExcepcionFuncionalEnum.EXCEPCION_FUNCIONAL_TIPO_2);
			}
		} catch (Exception e) {
			logger.error("No evaluado - Ocurrió alguna excepción", e);
			return crearMensajeNoEvaluado(ConstantesValidaciones.KEY_VALIDACION_NOVEDAD_ABORTADA_POR_EXCEPCION,
					ValidacionCoreEnum.VALIDACION_ESTADO_RESPECTO_AFILIADO_PPAL, TipoExcepcionFuncionalEnum.EXCEPCION_TECNICA);
		}
	}
}
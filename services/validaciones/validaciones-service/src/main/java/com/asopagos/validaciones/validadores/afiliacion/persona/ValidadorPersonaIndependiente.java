...
/**
 * Clase que contiene la lógica para validar cuando una persona está afiliada
 * como trabajador independiente.
 * 
 * @author Angélica Toro Murillo <atoro@heinsohn.com.co>
 */
public class ValidadorPersonaIndependiente extends ValidadorAbstract {
	/*
	 * (non-Javadoc)
	 * 
	...
	public ValidacionDTO execute(Map<String, String> datosValidacion) {
		logger.debug("Inicio de método ValidadorPersonaIndependiente.execute");
		try {
			if (datosValidacion != null && !datosValidacion.isEmpty()) {
				...
				if (tipoIdentificacion != null && !tipoIdentificacion.equals("")){
					...
					if(existe){
						/*No aprobada- Existe persona afiliada como pensionado*/
						return crearValidacion(myResources.getString(ConstantesValidaciones.KEY_PERSONA_INDEPENDIENTE),
								ResultadoValidacionEnum.NO_APROBADA,
								ValidacionCoreEnum.VALIDACION_PERSONA_INDEPENDIENTE,TipoExcepcionFuncionalEnum.EXCEPCION_FUNCIONAL_TIPO_2);					
					}
				}else{
			...
			return crearMensajeExitoso(ValidacionCoreEnum.VALIDACION_PERSONA_INDEPENDIENTE);
		}catch(Exception e){
	...
	private  ValidacionDTO crearMensajeNoEvaluado(){
		return crearValidacion(
				myResources.getString(ConstantesValidaciones.KEY_MENSAJE_NO_EV) + SEPARADOR
						+ myResources.getString(ConstantesValidaciones.KEY_PERSONA_INDEPENDIENTE),
				ResultadoValidacionEnum.NO_EVALUADA, ValidacionCoreEnum.VALIDACION_PERSONA_INDEPENDIENTE,

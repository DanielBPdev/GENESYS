 ...
 */
public class ValidadorAfiliadoActivo extends ValidadorAbstract {
	/*
	 * (non-Javadoc)
	 * 
	...
	public ValidacionDTO execute(Map<String, String> datosValidacion) {
		...
		try{
			...
			if(datosValidacion.get(ConstantesValidaciones.TIPO_BENEFICIARIO_PARAM) != null){
				tipoIdentificacion = TipoIdentificacionEnum.valueOf(datosValidacion.get(ConstantesValidaciones.TIPO_IDENTIFICACION_AFILIADO));
				numeroIdentificacion = datosValidacion.get(ConstantesValidaciones.NUMERO_IDENTIFICACION_AFILIADO);
			}else{
			...
			logger.info("ValidadorAfiliadoActivo tipoIdentificacion ValidadorAfiliadoActivo --> " +tipoIdentificacion);
			logger.info("ValidadorAfiliadoActivo numeroIdentificacion ValidadorAfiliadoActivo --> " +numeroIdentificacion);
	        // CC 220114
	        ...
	        Long cantidadRolesActivos = (Long) entityManager.createNamedQuery(NamedQueriesConstants.CONTAR_ROL_AFILIADO_POR_TIPO_NUMERO_IDENTIFICACION_Y_ESTADO)
					...
					.getSingleResult();
			logger.info("ValidadorAfiliadoActivo cantidadRolesActivos " +cantidadRolesActivos);

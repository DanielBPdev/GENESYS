...
/**
 * Clase que contiene la lógica para validar el estado de un empleador para una nueva afiliaciòn.
 * 
 * @author Angélica Toro Murillo <atoro@heinsohn.com.co>
 *
 */
public class ValidadorEstadoNuevaAfiliacion extends ValidadorAbstract{
	...
	public ValidacionDTO execute(Map<String, String> datosValidacion) {
		logger.debug("Inicio de método ValidadorEstadoNuevaAfiliacion.execute");
		try{
            ...
            estadoEmpleadorList.add(EstadoEmpleadorEnum.NO_FORMALIZADO_SIN_AFILIACION_CON_APORTES);
            estadoEmpleadorList.add(EstadoEmpleadorEnum.NO_FORMALIZADO_CON_INFORMACION);
            if (estadoEmpleadorList.contains(empleador.getEstadoEmpleador())) {
                return crearMensajeExitoso(ValidacionCoreEnum.VALIDACION_ESTADO_NUEVA_AFILIACION);
            }
            else {
                return crearValidacion(myResources.getString(ConstantesValidaciones.KEY_ESTADO_NUEVA_AFILIACION),
                        ResultadoValidacionEnum.NO_APROBADA, ValidacionCoreEnum.VALIDACION_ESTADO_NUEVA_AFILIACION, null);
            }

		}catch (Exception e){
			logger.error("Ocurrio un error en el método ValidadorEstadoNuevaAfiliacion.execute");
	...
	private ValidacionDTO crearMensajeNoEvaluado() {
		return crearValidacion(
				myResources.getString(ConstantesValidaciones.KEY_MENSAJE_NO_EV) + SEPARADOR
						+ myResources.getString(ConstantesValidaciones.KEY_ESTADO_NUEVA_AFILIACION),
				ResultadoValidacionEnum.NO_EVALUADA, ValidacionCoreEnum.VALIDACION_ESTADO_NUEVA_AFILIACION,

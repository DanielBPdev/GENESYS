 ...
 *
 */
public class ValidadorEstadoEmpleador implements ValidadorCore {
				...
				if (result != null && !result.isEmpty()) {
					...
					try {
						if (emp != null) {
							if (estadoEmpleador!=null && estadoEmpleador.toString().equals(EstadoEmpleadorEnum.ACTIVO.toString())) {
								return crearValidacion(
										...
										ValidacionCoreEnum.VALIDACION_EMPLEADOR_ACTIVO);
							} else {
								return crearValidacion(
						...
						return crearValidacion(myResources.getString(ConstantesValidaciones.KEY_MENSAJE_EMP_NO_ACTIVO),
								ResultadoValidacionEnum.APROBADA, ValidacionCoreEnum.VALIDACION_EMPLEADOR_ACTIVO);
					}
				}
			} else {
				return crearValidacion(myResources.getString(ConstantesValidaciones.KEY_MENSAJE_NO_PARAMS),
						ResultadoValidacionEnum.NO_APROBADA, ValidacionCoreEnum.VALIDACION_EMPLEADOR_ACTIVO);
			}
		}
		return crearValidacion(myResources.getString(ConstantesValidaciones.KEY_MENSAJE_NO_EV),
				ResultadoValidacionEnum.NO_EVALUADA, ValidacionCoreEnum.VALIDACION_EMPLEADOR_ACTIVO);
	}

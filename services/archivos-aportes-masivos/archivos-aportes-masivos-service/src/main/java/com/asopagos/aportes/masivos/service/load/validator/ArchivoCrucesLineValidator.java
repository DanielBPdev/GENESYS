 ...
 */
public class ArchivoCrucesLineValidator extends LineValidator {

    /**
     * Referencia al logger
     */
    private ILogger logger = LogManager.getLogger(ArchivoCrucesLineValidator.class);
    ...
    public void validate(LineArgumentDTO arguments) throws FileProcessingException {
        ...
        ((List<Long>) arguments.getContext().get(CamposArchivoConstants.TOTAL_REGISTRO)).add(1L);
             System.out.println("ArchivoCrucesLineValidator - CamposArchivoConstants.TOTAL_REGISTRO: "+  arguments.getContext().get(CamposArchivoConstants.TOTAL_REGISTRO));
        try {
            TipoIdentificacionEnum tipoIdentificacionAportante = null;
            try { 
                tipoIdentificacionAportante = TipoIdentificacionEnum
                        .obtenerTiposIdentificacionPILAEnum(line.get(CamposArchivoConstants.TIPO_IDENTIFICACION_APORTANTE).toString());
                if (tipoIdentificacionAportante == null) {
                    lstHallazgos.add(crearHallazgo(arguments.getLineNumber(), CamposArchivoConstants.TIPO_IDENTIFICACION_APORTANTE,
                ...
                lstHallazgos.add(crearHallazgo(arguments.getLineNumber(), CamposArchivoConstants.TIPO_IDENTIFICACION_APORTANTE,
                        "El registro no cumple con la estructura de contenido y/o la obligatoriedad de los datos"));
                tipoIdentificacionAportante = null;
            }
            verificarNumeroDocumento(tipoIdentificacionAportante, arguments, CamposArchivoConstants.NUMERO_IDENTIFICACION_APORTANTE);

            String tipoAportante = null;
            try {
                tipoAportante = line.get(CamposArchivoConstants.TIPO_APORTANTE).toString();
                if (tipoAportante == null) {
                    lstHallazgos.add(crearHallazgo(arguments.getLineNumber(), CamposArchivoConstants.TIPO_APORTANTE,
                            "El registro no cumple con la estructura de contenido y/o la obligatoriedad de los datos"));    
                }else if(!tipoAportante.equals("EMPLEADOR") && !tipoAportante.equals("TRABAJADOR_DEPENDIENTE") && 
                    !tipoAportante.equals("TRABAJADOR_INDEPENDIENTE") && !tipoAportante.equals("PENSIONADO") ){
                    lstHallazgos.add(crearHallazgo(arguments.getLineNumber(), CamposArchivoConstants.TIPO_APORTANTE,
                            "El tipo de Aportante no es Valido")); 
                    }
            ...
            try {
            	razonSocial = line.get(CamposArchivoConstants.RAZON_SOCIAL).toString() ;
                if (razonSocial == null ) {
                    lstHallazgos.add(crearHallazgo(arguments.getLineNumber(), CamposArchivoConstants.RAZON_SOCIAL,
                            "El registro no cumple con la estructura de contenido y/o la obligatoriedad de los datos"));
                }else if (razonSocial.length() > 50) {
                    lstHallazgos.add(crearHallazgo(arguments.getLineNumber(), CamposArchivoConstants.RAZON_SOCIAL,
                            "La razón social supera los 50 caracteres"));
                }
            } catch (Exception e) {
            	e.printStackTrace();
                razonSocial = null;
    ...
    private void validarRegex(LineArgumentDTO arguments, String campoKey, String regex, String mensaje, String campoMSG) {
        try {
            ...
            if (campoKey.equals(CamposArchivoConstants.NUMERO_IDENTIFICACION_APORTANTE)) {

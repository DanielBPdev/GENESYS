 ...
 */
public class ValidadorPersonaDependiente extends ValidadorFovisAbstract {
	...
	public ValidacionDTO execute(Map<String, String> datosValidacion) {
		logger.debug("Inicio de método ValidadorPersonaDependiente.execute");
		try {
			if (datosValidacion != null && !datosValidacion.isEmpty()) {
				...
				if (tipoIdentificacion != null && !tipoIdentificacion.equals("")){
					...
					if(existe){
						...
						return crearValidacion(myResources.getString(ConstantesValidaciones.KEY_PERSONA_TRABJADOR_DEPENDIENTE),
								ResultadoValidacionEnum.NO_APROBADA,
								ValidacionCoreEnum.VALIDACION_PERSONA_DEPENDIENTE,TipoExcepcionFuncionalEnum.EXCEPCION_FUNCIONAL_TIPO_2);					
					}
				}else{
			...
			return crearMensajeExitoso(ValidacionCoreEnum.VALIDACION_PERSONA_DEPENDIENTE);
		}catch(Exception e){
	...
	private  ValidacionDTO crearMensajeNoEvaluado(){
		return crearValidacion(
				myResources.getString(ConstantesValidaciones.KEY_MENSAJE_NO_EV) + SEPARADOR
						+ myResources.getString(ConstantesValidaciones.KEY_PERSONA_TRABJADOR_DEPENDIENTE),
				ResultadoValidacionEnum.NO_EVALUADA, ValidacionCoreEnum.VALIDACION_PERSONA_DEPENDIENTE,

 ...
 */
public class ValidadorPostulacionEnProceso extends ValidadorFovisAbstract {
	...
	private boolean postulacionEnProceso(String tipoIdentificacion, String numeroIdentificacion,
		...
		logger.info("datosSolicitud " +datosSolicitud.size());

		if (datosSolicitud != null && !datosSolicitud.isEmpty()) {
			postulacionEnProceso = Boolean.TRUE;
			logger.info("ingresa al primero if --> " + postulacionEnProceso);
		}
		logger.info("postulacionEnProceso ?" +postulacionEnProceso);
		...
		if (!postulacionEnProceso) {
			...
			datosSolicitud = entityManager
					...
					.getResultList();
			logger.info("datosSolicitud integrante" +datosSolicitud.size());
			if (datosSolicitud != null && !datosSolicitud.isEmpty()) {
				postulacionEnProceso = Boolean.TRUE;
				logger.info("ingresa al segundo if --> " + postulacionEnProceso);
			}

		}

		logger.info("postulacionEnProceso 2.0?" +postulacionEnProceso);
		logger.info("idSolicitudPostulacion" +idSolicitudPostulacion);
		...
		if (postulacionEnProceso && idSolicitudPostulacion != null
			...
			logger.info("postulacionEnProceso 3.0?" +postulacionEnProceso);
			for (BigInteger idSolicitudProceso : datosSolicitud) {
				/*Si es diferente a la solicitud en proceso.*/
				logger.info("idSolicitudPostulacion" +idSolicitudProceso);
				logger.info("idSolicitudPostulacion" +idSolicitudPostulacion);
				if (idSolicitudProceso != null &&
						idSolicitudProceso.longValue() != Long.parseLong(idSolicitudPostulacion)) {
					postulacionEnProceso = Boolean.TRUE;
					logger.info("postulacionEnProceso 4.0?" +postulacionEnProceso);
				}

			}
		}else if (idSolicitudPostulacion == null && postulacionEnProceso){
			postulacionEnProceso = Boolean.TRUE;
			logger.info("postulacionEnProceso 5.0?" +postulacionEnProceso);
		}else{
			postulacionEnProceso = Boolean.FALSE;
			logger.info("postulacionEnProceso 6.0?" +postulacionEnProceso);
		}

		logger.info("postulacionEnProceso final" +postulacionEnProceso);

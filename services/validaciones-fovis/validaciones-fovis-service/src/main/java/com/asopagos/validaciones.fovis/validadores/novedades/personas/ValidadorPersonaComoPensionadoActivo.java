...
/**
 * CLASE QUE VALIDA:
 * si la persona está registrada como afiliado pensionado y estado activo.
 * 
 * @author Steven Quintero González <squintero@heinsohn.com.co>
 */
public class ValidadorPersonaComoPensionadoActivo extends ValidadorFovisAbstract {
	/*
	 * (non-Javadoc)
	 * 
	...
	public ValidacionDTO execute(Map<String, String> datosValidacion) {
		logger.info("Inicio de método ValidadorPersonaComoPensionadoActivo.execute");
		try{
			...
			if(datosValidacion.get(ConstantesValidaciones.TIPO_ID_BENEF_PARAM)!= null && 
		      ...
		      //se consulta el beneficiario con el tipo y número de documento, el tipo de beneficiario y su estado
				...
				if(rolAfiliado != null && !rolAfiliado.isEmpty()){
					//validación fallida
					logger.debug("NO HABILITADA- Fin de método ValidadorPersonaComoPensionadoActivo.execute");
					return crearValidacion(myResources.getString(ConstantesValidaciones.KEY_PERSONA_PENSIONADO_ACTIVO),ResultadoValidacionEnum.NO_APROBADA,
							ValidacionCoreEnum.VALIDACION_PERSONA_PENSIONADO_ACTIVO,
							TipoExcepcionFuncionalEnum.EXCEPCION_FUNCIONAL_TIPO_2);	
				}else{
					//validación exitosa
					logger.debug("HABILITADA- Fin de método ValidadorPersonaComoPensionadoActivo.execute");
					return crearMensajeExitoso(ValidacionCoreEnum.VALIDACION_PERSONA_PENSIONADO_ACTIVO);
				}
			}
			else{
			...
			return crearMensajeNoEvaluado(ConstantesValidaciones.KEY_VALIDACION_NOVEDAD_ABORTADA_POR_EXCEPCION,
					ValidacionCoreEnum.VALIDACION_PERSONA_PENSIONADO_ACTIVO, TipoExcepcionFuncionalEnum.EXCEPCION_TECNICA);
		}
	}
}
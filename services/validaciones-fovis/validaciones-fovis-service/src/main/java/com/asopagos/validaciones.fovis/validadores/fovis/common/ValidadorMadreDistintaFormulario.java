...
/**
 * Validadador que verifica si el jefe de hogar registra en el formulario FOVIS a su madre
 * y esta es distinta a la que se encuentra registrado en base de datos (v32)
 * 
 * @author <a href="mailto:mamonroy@heinsohn.com.co">Mario Andres Monroy Monroy</a>
 */
public class ValidadorMadreDistintaFormulario extends ValidadorFovisAbstract {

    @Override
    public ValidacionDTO execute(Map<String, String> datosValidacion) {
        logger.debug("Inicio ValidadorMadreDistintaFormulario.execute");
        try {
            if (datosValidacion == null || datosValidacion.isEmpty()) {
                logger.debug("NO EVALUADA - No llegó el mapa con valores");
            ...
            if (tipoIdentificaBeneficiario == null || tipoIdentificaBeneficiario.isEmpty() || numeroIdentificacionBeneficiario == null
                ...
                logger.debug("NO EVALUADA - No llegaron todos los parámetros");
                return crearMensajeNoEvaluado();
            }
            ...
            if (!beneficiarioIgualRegistrado(tipoIdentificacionBeneficiario, numeroIdentificacionBeneficiario,
                    tipoIdentificacionJefe, numeroIdentificacionJefeHogar)) {
                logger.debug("NO APROBADA - Fin ValidadorMadreDistintaFormulario.execute");
                return crearValidacion(
                        myResources.getString(ConstantesValidaciones.KEY_MADRE_FORMULARIO_DISTINTA_BD),
                        ResultadoValidacionEnum.NO_APROBADA,
                        ValidacionCoreEnum.VALIDACION_MADRE_DISTINTA_FORMULARIO,
                        TipoExcepcionFuncionalEnum.EXCEPCION_FUNCIONAL_TIPO_2);
            }
            logger.debug("APROBADA - Fin ValidadorMadreDistintaFormulario.execute");
            return crearMensajeExitoso(ValidacionCoreEnum.VALIDACION_MADRE_DISTINTA_FORMULARIO);
        } catch (Exception e) {
            logger.error("NO EVALUADA - Ocurrió una excepción", e);
            return crearMensajeNoEvaluado();
        }
    }
    ...
    private ValidacionDTO crearMensajeNoEvaluado() {
        logger.debug("Fin ValidadorMadreDistintaFormulario.execute");
        return crearValidacion(
                myResources.getString(ConstantesValidaciones.KEY_MENSAJE_NO_EV) + SEPARADOR
                        + myResources.getString(ConstantesValidaciones.KEY_MADRE_FORMULARIO_DISTINTA_BD),
                ResultadoValidacionEnum.NO_EVALUADA, ValidacionCoreEnum.VALIDACION_MADRE_DISTINTA_FORMULARIO,
                TipoExcepcionFuncionalEnum.EXCEPCION_FUNCIONAL_TIPO_1);
    }

    /**
     * Verifica si el Jefe Hogar tiene registrado un beneficiario madre diferente al indicado en el registro de postulacion
    ...
    private boolean beneficiarioIgualRegistrado(TipoIdentificacionEnum tipoIdentificacionBeneficiario,
        ...
        Beneficiario beneficiario = consultarBeneficiarioMadreAsociadoJefe(numeroIdentificacionJefeHogar, tipoIdentificacionJefeHogar);
     ...
     */
    private Beneficiario consultarBeneficiarioMadreAsociadoJefe(String numeroIdentificacionJefe,
        ...
        try {
            return entityManager
                    ...
                    .setParameter(ConstantesValidaciones.TIPO_BENEFICIARIO_PARAM, ClasificacionEnum.MADRE).getSingleResult();

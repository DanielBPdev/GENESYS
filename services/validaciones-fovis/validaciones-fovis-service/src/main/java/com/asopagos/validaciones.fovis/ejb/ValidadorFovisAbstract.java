...
public abstract class ValidadorFovisAbstract implements ValidadorFovisCore {
     ...
     */
    protected ILogger logger = LogManager.getLogger(ValidadorFovisAbstract.class);
    ...
    public Boolean verificarAsignacionMismoAnio(Map<String, String> datosValidacion) {
        logger.info("ingresa a verificarAsignacionMismoAnio");
        Long idPostulacion = getIdPostulacion(datosValidacion);
        logger.info("idPostulacion " + idPostulacion);
        try {
            entityManager.createNamedQuery(NamedQueriesConstants.CONSULTAR_ASIGNACION_MISMO_ANIO)
                    .setParameter(ConstantesValidaciones.ID_POSTULACION, idPostulacion).getSingleResult();
            logger.info("retorna true verificarAsignacionMismoAnio");
            return true;
        } catch (NoResultException e) {
            logger.info("retorna false verificarAsignacionMismoAnio");
            return false;
        }
    }

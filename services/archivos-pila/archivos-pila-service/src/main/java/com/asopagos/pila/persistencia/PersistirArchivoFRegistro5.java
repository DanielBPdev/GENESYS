...
import com.asopagos.entidades.pila.archivolinea.PilaArchivoFRegistro5;
...
/**
 * <b>Descripción:</b> Clase que se encarga de llevar a cabo la persistencia de los registros tipo 5 del archivo F<br>
 * <b>Módulo:</b> ArchivosPILAService - HU 407 <br/>
 * 
 * @author <a href="mailto:abaquero@heinsohn.com.co">Alfonso Baquero E.</a>
 * @author <a href="mailto:rarboleda@heinsohn.com.co">Robinson Arboleda V.</a>
 */
public class PersistirArchivoFRegistro5 implements IPersistLine {
    private List<Long> idRegistro = new ArrayList<>();

    /**
     * Referencia al logger
     */
    final ILogger logger = LogManager.getLogger(PersistirArchivoFRegistro5.class);
    ...
    public void persistLine(List<LineArgumentDTO> paramList, EntityManager emCore) throws FileProcessingException {
        ...
        try {
            ...
            for (LineArgumentDTO lineArgumentDTO : paramList) {
                ...
                PilaArchivoFRegistro5 registro = new PilaArchivoFRegistro5();
                registro.setIndicePlanilla(indicePlanilla);

                registro.setNumeroCuenta((String) datos.get(EtiquetaArchivoFEnum.F52.getNombreCampo()));
                registro.setTipoCuenta(((Integer) datos.get(EtiquetaArchivoFEnum.F53.getNombreCampo())).shortValue());
                registro.setNumeroLote(((Integer) datos.get(EtiquetaArchivoFEnum.F54.getNombreCampo())).shortValue());
                registro.setSistemaPago((String) datos.get(EtiquetaArchivoFEnum.F55.getNombreCampo()));
        ...
        logger.info("Finaliza persistLine(List<LineArgumentDTO>, EntityManager)");
    }
    ...
    public void setRollback(EntityManager em) throws FileProcessingException {
        ...
        if (idRegistro != null && !idRegistro.isEmpty()) {
            ...
            while (BATCH_SIZE < idRegistro.size()) {
                ...
                emPila.createNamedQuery(NamedQueriesConstants.ROLLBACK_ARCHIVO_F_REGISTRO_5).setParameter("listaId", listaIdsLote).executeUpdate();
                listaIdsLote.clear();
            }
            
            if(idRegistro.size() > 0) {
                emPila.createNamedQuery(NamedQueriesConstants.ROLLBACK_ARCHIVO_F_REGISTRO_5).setParameter("listaId", idRegistro).executeUpdate();
            }
        }
    }

...
/**
 * <b>Descripción:</b> Clase que se encarga de llevar a cabo la persistencia de los registros tipo 3 del archivo IP<br>
 * <b>Módulo:</b> ArchivosPILAService - HU 391 <br/>
 * 
 * @author <a href="mailto:abaquero@heinsohn.com.co">Alfonso Baquero E.</a>
 */
public class PersistirArchivoIPRegistro3 implements IPersistLine {
    private List<Long> idRegistro = new ArrayList<>();

    /**
     * Referencia al logger
     */
    final ILogger logger = LogManager.getLogger(PersistirArchivoIPRegistro3.class);
    ...
    public void persistLine(List<LineArgumentDTO> paramList, EntityManager emCore) throws FileProcessingException {
        ...
        try {
            ...
            for (LineArgumentDTO lineArgumentDTO : paramList) {
                ...
                PilaArchivoIPRegistro3 registro = new PilaArchivoIPRegistro3();
                registro.setIndicePlanilla(indicePlanilla);
                registro.setValorTotalAporte((BigDecimal) datos.get(EtiquetaArchivoIPEnum.IP32.getNombreCampo()));
                registro.setDiasMora(((Integer) datos.get(EtiquetaArchivoIPEnum.IP33.getNombreCampo())).shortValue());
                registro.setValorMora((BigDecimal) datos.get(EtiquetaArchivoIPEnum.IP34.getNombreCampo()));
                registro.setValorTotalPagar((BigDecimal) datos.get(EtiquetaArchivoIPEnum.IP35.getNombreCampo()));
    ...
    public void setRollback(EntityManager em) throws FileProcessingException {
        ...
        if (idRegistro != null && !idRegistro.isEmpty()) {
            ...
            while (BATCH_SIZE < idRegistro.size()) {
                ...
                emPila.createNamedQuery(NamedQueriesConstants.ROLLBACK_ARCHIVO_IP_REGISTRO_3).setParameter("listaId", listaIdsLote).executeUpdate();
                listaIdsLote.clear();
            }
            
            if(idRegistro.size() > 0) {
                emPila.createNamedQuery(NamedQueriesConstants.ROLLBACK_ARCHIVO_IP_REGISTRO_3).setParameter("listaId", idRegistro).executeUpdate();
            }
        }
    }

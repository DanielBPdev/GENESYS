 ...
 */
public class CruceHojaCedulaPersistLine implements IPersistLine {
        ...
        for (int i = 1; i < lines.size(); i++) {
            ...
            if (hallazgos.isEmpty()) {
                try {
                    ...
                    // Se asigna el campo No 1 - NRO_CEDULA 
                    if (esCampoValido(line, FileFieldConstants.NRO_CEDULA)) {
                        String nroCedula = getValorCampo(line, FileFieldConstants.NRO_CEDULA);
                        informacionHojaCruceFovisDTO.setNroCedula(nroCedula);
                    }

                    ((List<InformacionHojaCruceFovisDTO>) lineArgumentDTO.getContext()
                            .get(ArchivoMultipleCampoConstants.LISTA_CANDIDATOS)).add(informacionHojaCruceFovisDTO);
                } catch (Exception e) {
                    ((List<InformacionHojaCruceFovisDTO>) lineArgumentDTO.getContext()
                            .get(ArchivoMultipleCampoConstants.LISTA_CANDIDATOS)).add(informacionHojaCruceFovisDTO);
                }

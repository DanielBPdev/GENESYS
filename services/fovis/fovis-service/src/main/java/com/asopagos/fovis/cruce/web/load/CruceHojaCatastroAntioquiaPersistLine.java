 ...
 */
public class CruceHojaCatastroAntioquiaPersistLine implements IPersistLine {
        ...
        for (int i = 1; i < lines.size(); i++) {
            ...
            if (hallazgos.isEmpty()) {
                try {
                    ...
                    if (esCampoValido(line, FileFieldConstants.APELLIDOS)) {
                        String apellidos = getValorCampo(line, FileFieldConstants.APELLIDOS);
                        informacionHojaCruceFovisDTO.setApellidos(apellidos);
                    }
                    // Se valida el campo No 5 - NOMBRES
                    if (esCampoValido(line, FileFieldConstants.NOMBRES)) {
                        String nombres = getValorCampo(line, FileFieldConstants.NOMBRES);
                        informacionHojaCruceFovisDTO.setNombres(nombres);
                    }
                    // Se valida el campo No 6 - CEDULA CATASTRAL
                    if (esCampoValido(line, FileFieldConstants.CEDULA_CATASTRAL)) {
                        String nroCedula = getValorCampo(line, FileFieldConstants.CEDULA_CATASTRAL);
                        informacionHojaCruceFovisDTO.setCedulaCatastral(nroCedula);
                    }
                    // Se valida el campo No 7 - DIRECCIÓN INMUEBLE
                    if (esCampoValido(line, FileFieldConstants.DIRECCION_INMUEBLE)) {
                        String direccion = getValorCampo(line, FileFieldConstants.DIRECCION_INMUEBLE);
                        informacionHojaCruceFovisDTO.setDireccionInmueble(direccion);
                    }

...
/**
 * <b>Descripcion:</b> Clase que valida la estructura del archivo de Retiro Trabajadores<br/>
 * <b>Módulo:</b> Asopagos - HU-13-XXX<br/>
 *
 * @author <a href="mailto:maria.cuellar@eprocess.com.co">Maria Cuellar</a>
 */

public class ActualizacionRetiroAfiliadoPersistLine implements IPersistLine {
    ...
    public void persistLine(List<LineArgumentDTO> lines, EntityManager em) throws FileProcessingException {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            if (!hallazgos.isEmpty() && hallazgos.stream().anyMatch(iteHallazgo -> 
                    ...
                    System.out.println("PERSISTENCIA LINEA: " + lineArgumentDTO.getLineNumber());
                persist = false;
            }
            
            if (persist) {
                System.out.println("ENTRA A PERSISTIR: "+ lineArgumentDTO.getLineNumber());
                try {
                    ...
                    // Se asigna el campo No 3 - Sucursal del trabajador o cabeza de familia 
                    ...
                    // Se asigna el campo No 6 - Fecha de inicio labores del trabajador o cabeza de familia 
                    if (esCampoValido(line, ArchivoCampoNovedadConstante.FECHA_INICIO_LABORES_CON_EMPLEADOR)) {
                        String fechaInicioLaboresText = getValorCampo(line, ArchivoCampoNovedadConstante.FECHA_INICIO_LABORES_CON_EMPLEADOR);
                        Date fechaInicioLabores = CalendarUtils.darFormatoYYYYMMDDGuionDate(fechaInicioLaboresText);
                        afiliadoModeloDTO.setFechaInicioLaboresAfiliado(fechaInicioLabores.getTime());
                    }
                    // Se asigna el campo No 7 - Fecha de inicio afiliacion del trabajador o cabeza de familia 
                    if (esCampoValido(line, ArchivoCampoNovedadConstante.FECHA_INICIO_AFILIACION)) {
                        String fechaInicioAfiliacionText = getValorCampo(line, ArchivoCampoNovedadConstante.FECHA_INICIO_AFILIACION);
                        Date fechaInicioAfiliacion = CalendarUtils.darFormatoYYYYMMDDGuionDate(fechaInicioAfiliacionText);
                        afiliadoModeloDTO.setFechaInicioAfiliacion(fechaInicioAfiliacion.getTime());
                    }
                    ...
                    // Se asigna el campo No 9 - Fecha retiro del trabajador o cabeza de familia 
                    if (esCampoValido(line, ArchivoCampoNovedadConstante.FECHA_RETIRO_AFILIADO)) {
                        String fechaRetiroAfiliadoText = getValorCampo(line, ArchivoCampoNovedadConstante.FECHA_RETIRO_AFILIADO);
                        Date fechaRetiroAfiliado = CalendarUtils.darFormatoYYYYMMDDGuionDate(fechaRetiroAfiliadoText);
                        afiliadoModeloDTO.setFechaRetiroAfiliado(fechaRetiroAfiliado.getTime());
                    }
                    // Se asigna el campo No 10 - motivo retiro del trabajador o cotizante o cabeza de familia 
                    if (esCampoValido(line, ArchivoCampoNovedadConstante.MOTIVO_RETIRO_AFILIADO)) {
                        String motivoRetiroAfiliado = getValorCampo(line, ArchivoCampoNovedadConstante.MOTIVO_RETIRO_AFILIADO)
                                .toUpperCase();
                        MotivoDesafiliacionAfiliadoEnum motivoDesafiliacionTrabajador = 
                                MotivoDesafiliacionAfiliadoEnum.valueOf(motivoRetiroAfiliado);                        
                        afiliadoModeloDTO.setMotivoDesafiliacionTrabajador(motivoDesafiliacionTrabajador);

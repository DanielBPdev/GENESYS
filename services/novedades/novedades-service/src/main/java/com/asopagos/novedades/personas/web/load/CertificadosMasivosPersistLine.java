...
public class CertificadosMasivosPersistLine implements IPersistLine{
    ...
    public void persistLine(List<LineArgumentDTO> arg0, EntityManager emCore) throws FileProcessingException {
        ...
        for (LineArgumentDTO lineArgumentDTO : arg0) {
            ...
            try {
                ...
                if (!errorLinea) {
                     ...
                     CargueCertificadosMasivos cargue = new CargueCertificadosMasivos();
                     cargue.setTipoIdentificacion(calcularTipoIdentificacion(values.get(ArchivoMultipleCampoConstants.TIPO_IDENTIFICACION_AFILIADO).toString()));
                     cargue.setNumeroIdentificacion(values.get(ArchivoMultipleCampoConstants.NUMERO_IDENTIFICACION_AFILIADO).toString());
                     ...
                     cargue.setFechaYHoraCargue(new Date());

                     ((List<CargueCertificadosMasivos>) lineArgumentDTO.getContext().get(ArchivoMultipleCampoConstants.LISTA_CARGUE_CERTIFICADOS_MASIVOS)).add(cargue);
        ...
        logger.info("Finaliza persistLine CertificadosMasivos(List<LineArgumentDTO>, EntityManager)");
    }

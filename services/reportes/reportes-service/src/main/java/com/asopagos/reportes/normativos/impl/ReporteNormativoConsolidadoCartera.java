...
/**
 * <b>Descripción: </b> Clase encargada de la generación del reporte consolidado
 * de cartera solicitado por la Unidad de Gestión Pensional y Parafiscales con
 * los parámetros establecidos por dicha entidad en la Resolución 2082 de 2016
 * <br/>
 ...
 *         Benavides</a>
 */
public class ReporteNormativoConsolidadoCartera extends ReporteNormativoAbstract {
     ...
     */
    private static final ILogger logger = LogManager.getLogger(ReporteNormativoConsolidadoCartera.class);
	 ...
	 *            Entity manager
	 */
	public ReporteNormativoConsolidadoCartera(EntityManager entityManager) {
	...
	public Integer pregenerarReporte(ReporteNormativoEnum reporteNormativo, Date fechaInicio, Date fechaFin) {
		try {
			logger.debug("Pregenerando ReporteNormativoConsolidadoCartera...");
			Integer resultado = (Integer)entityManager.createNamedQuery(NamedQueriesConstants.CONSULTAR_CONSOLIDADO_CARTERA_COUNT)
					.setParameter("fechaFin", fechaFin, TemporalType.DATE)
					.getSingleResult();
			logger.debug("ReporteNormativoConsolidadoCartera pregenerado");
			return resultado;
		} catch (Exception e) {
			logger.error("Error pregenerando ReporteNormativoConsolidadoCartera", e);
	...
	@SuppressWarnings("unchecked")
	 @Override
	    ...
	    try {
			logger.debug("Generando datos ReporteNormativoConsolidadoCartera...");
			List<String[]> data = entityManager.createNamedQuery(NamedQueriesConstants.CONSULTAR_CONSOLIDADO_CARTERA)
					.setParameter("fechaFin", fechaFin, TemporalType.DATE)
					.getResultList();
			logger.debug("ReporteNormativoConsolidadoCartera datos generados");
			return data;
		} catch (Exception e) {
			logger.error("Error generando datos ReporteNormativoConsolidadoCartera", e);
			throw new TechnicalException(MensajesGeneralConstants.ERROR_TECNICO_INESPERADO, e);
		}
	}
    ...
    public String obtenerValorTotalDeuda(Date fechaFin) {        
    	BigInteger suma = (BigInteger) entityManager.createNamedQuery(NamedQueriesConstants.CONSULTAR_CONSOLIDADO_CARTERA_TOTAL)
                    .setParameter("fechaFin", fechaFin, TemporalType.DATE)                    
                    .getSingleResult();

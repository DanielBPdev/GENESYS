...
/**
 * <b>Descripción:</b> EJB Singleton que ejecuta el proceso automático de rechazo de solicitudes suspendidas por cambio de año.
 * 
 * proceso 3.2.1
 * 
...
@Singleton
@Startup
public class ScheduleRechazoSolicitudesSuspendidasXCambioAnio extends ScheduleAbstract {

    /**
     * Instancia del gestor de registro de eventos.
     */
    private static final ILogger logger = LogManager.getLogger(ScheduleRechazoSolicitudesSuspendidasXCambioAnio.class);

    /**
     * 
     */
    private static final ProcesoAutomaticoEnum CURRENT_PROCESS = ProcesoAutomaticoEnum.RECHAZO_AUTOMATICO_SOLICITUDES_SUSPENDIDAS_X_CAMBIO_ANIO;
    ...
    public void run(Timer timer) {
        logger.debug("Inicio de método ScheduleRechazoSolicitudesSuspendidasXCambioAnio.run(Timer timer)");
        try {
            ...
            solNovedadDTO.setTipoTransaccion(TipoTransaccionEnum.RECHAZO_SOLICITUDES_SUSPENDIDAS_CAMBIO_ANIO_AUTOMATICA);
            ...
            logger.debug("Fin de método ScheduleRechazoSolicitudesSuspendidasXCambioAnio.run(Timer timer)");
        } catch (Exception e) {
            logger.error("Ocurrió un error inesperado en ScheduleRechazoSolicitudesSuspendidasXCambioAnio.run(Timer timer)", e);

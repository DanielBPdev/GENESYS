...
import com.asopagos.usaurios.clients.InactivarUsuarioKeycloak;
@Singleton
@Startup
 public class ScheduleInactivarAutomaticamenteUsuariosPorDiasDeInactividad extends ScheduleAbstract{

    private static final ILogger logger = LogManager.getLogger(ScheduleDesistirSolicitudAfiliacion.class);
    private static final ProcesoAutomaticoEnum CURRENT_PROCESS = ProcesoAutomaticoEnum.INACTIVAR_AUTOMATICAMENTE_USUARIOS_POR_DIAS_DE_INACTIVIDAD;
    ...
    public void run(Timer timer) { 
        ...
        logger.debug("Inicio proceso controlado: [EJB:ScheduleInactivarAutomaticamenteUsuariosPorDiasDeInactividad]");
        System.out.println("Inicio proceso controlado: [EJB:ScheduleInactivarAutomaticamenteUsuariosPorDiasDeInactividad]");

        try {
            logger.info("Esta en el schedule");
            initContextUsuarioCore();
            InactivarUsuarioKeycloak usuarios = new InactivarUsuarioKeycloak();
            usuarios.execute();
        } catch (Exception e) {
           logger.error("Ocurrió un error inesperado en ScheduleInactivarAutomaticamenteUsuariosPorDiasDeInactividad.run(Timer timer)", e);

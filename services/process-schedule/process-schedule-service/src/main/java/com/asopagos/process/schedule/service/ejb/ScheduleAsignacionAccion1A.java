...
@Singleton
@Startup
public class ScheduleAsignacionAccion1A extends ScheduleAbstract {

	/**
	 * Proceso actual
	 */
	private static final ProcesoAutomaticoEnum CURRENT_PROCESS = ProcesoAutomaticoEnum.ASIGNACION_ACCION_1A;
	...
	public void run(Timer timer) {
		try {
			logger.debug("Inicio de método ScheduleAsignacionAccion1A.run(Timer timer)");
			// Se indica el usuario al contexto
			initContextUsuarioCore();
			ejecutarProcesoAutomaticoGestionCobro(TipoAccionCobroEnum.A1);
			logger.debug("Fin de método ScheduleAsignacionAccion1A.run(Timer timer)");
		} catch (Exception e) {
			logger.error("Ocurrió un error inesperado en ScheduleAsignacionAccion1A.run(Timer timer)", e);

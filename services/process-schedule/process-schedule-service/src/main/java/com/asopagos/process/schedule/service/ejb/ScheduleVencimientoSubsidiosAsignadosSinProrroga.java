...
@Singleton
@Startup
public class ScheduleVencimientoSubsidiosAsignadosSinProrroga extends ScheduleAbstract {

    /**
     * Instancia del gestor de registro de eventos.
     */
    private static final ILogger logger = LogManager.getLogger(ScheduleVencimientoSubsidiosAsignadosSinProrroga.class);

    /**
     * 
     */
    private static final ProcesoAutomaticoEnum CURRENT_PROCESS = ProcesoAutomaticoEnum.VENCIMIENTO_SUBSIDIO_FOVIS_ASIGNADO_SIN_PRORROGA;
    ...
    public void run(Timer timer) {
        logger.debug("Inicio de método ScheduleVencimientoSubsidiosAsignadosSinProrroga.run(Timer timer)");
        try {
            ...
            solNovedadDTO.setParametro(ParametroFOVISEnum.PLAZO_VENCIMIENTO_SIN_PRORROGA);
            ...
            logger.debug("Fin de método ScheduleVencimientoSubsidiosAsignadosSinProrroga.run(Timer timer)");
        } catch (Exception e) {
            logger.error("Ocurrió un error inesperado en ScheduleVencimientoSubsidiosAsignadosSinProrroga.run(Timer timer)", e);

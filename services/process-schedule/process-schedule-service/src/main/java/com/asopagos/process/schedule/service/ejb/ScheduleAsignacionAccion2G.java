...
@Singleton
@Startup
public class ScheduleAsignacionAccion2G extends ScheduleAbstract {

	/**
	 * Proceso actual
	 */
	private static final ProcesoAutomaticoEnum CURRENT_PROCESS = ProcesoAutomaticoEnum.ASIGNACION_ACCION_2G;
	...
	public void run(Timer timer) {
		try {
			logger.debug("Inicio de método ScheduleAsignacionAccion2G.run(Timer timer)");
			// Se indica el usuario al contexto
			initContextUsuarioCore();
			ejecutarProcesoAutomaticoGestionCobro(TipoAccionCobroEnum.G2);
			logger.debug("Fin de método ScheduleAsignacionAccion2G.run(Timer timer)");
		} catch (Exception e) {
			logger.error("Ocurrió un error inesperado en ScheduleAsignacionAccion2G.run(Timer timer)", e);

...
/**
 * <b>Descripción:</b> EJB Singleton que ejecuta el proceso automático de levantar inhabilidad o sancion automáticamente.
 * 
 * proceso 3.2.1
 * 
...
@Singleton
@Startup
public class ScheduleLevantarInhabilidadSancion extends ScheduleAbstract {

    /**
     * Instancia del gestor de registro de eventos.
     */
    private static final ILogger logger = LogManager.getLogger(ScheduleLevantarInhabilidadSancion.class);

    /**
     * 
     */
    private static final ProcesoAutomaticoEnum CURRENT_PROCESS = ProcesoAutomaticoEnum.LEVANTAR_INHABILIDAD_SANCION_AUTOMATICAMENTE;
    ...
    public void run(Timer timer) {
        logger.debug("Inicio de método ScheduleLevantarInhabilidadSancion.run(Timer timer)");
        try {
            ...
            solNovedadDTO.setTipoTransaccion(TipoTransaccionEnum.LEVANTAR_INHABILIDAD_SANCION_AUTOMATICA);
            ...
            logger.debug("Fin de método ScheduleLevantarInhabilidadSancion.run(Timer timer)");
        } catch (Exception e) {
            logger.error("Ocurrió un error inesperado en ScheduleLevantarInhabilidadSancion.run(Timer timer)", e);

...
/**
 * <b>Descripción:</b> EJB Singleton que ejecuta el proceso automático de Inactivación de Cuentas Web Empleador
 * Novedad 30.
 * 
...
@Singleton
@Startup
public class ScheduleInactivacionCuentaWebEmpleador extends ScheduleAbstract {

    /**
     * Instancia del gestor de registro de eventos.
     */
    private static final ILogger logger = LogManager.getLogger(ScheduleInactivacionCuentaWebEmpleador.class);

    /**
     * 
     */
    private static final ProcesoAutomaticoEnum CURRENT_PROCESS = ProcesoAutomaticoEnum.INACTIVAR_AUTOMATICAMENTE_CUENTA_WEB_EMPLEADOR;
    ...
    public void run(Timer timer) {
        logger.debug("Inicio de método ScheduleInactivacionCuentaWebEmpleador.run(Timer timer)");
        try {
            ...
            solNovedadDTO.setTipoTransaccion(TipoTransaccionEnum.INACTIVAR_AUTOMATICAMENTE_CUENTA_WEB_EMPLEADOR);
            ...
            logger.debug("Fin de método ScheduleInactivacionCuentaWebEmpleador.run(Timer timer)");
        } catch (Exception e) {
            logger.error("Ocurrió un error inesperado en ScheduleInactivacionCuentaWebEmpleador.run(Timer timer)", e);

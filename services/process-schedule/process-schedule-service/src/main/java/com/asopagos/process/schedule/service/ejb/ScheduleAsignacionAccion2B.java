...
@Singleton
@Startup
public class ScheduleAsignacionAccion2B extends ScheduleAbstract {

	/**
	 * Proceso actual
	 */
	private static final ProcesoAutomaticoEnum CURRENT_PROCESS = ProcesoAutomaticoEnum.ASIGNACION_ACCION_2B;
	...
	public void run(Timer timer) {
		try {
			logger.debug("Inicio de método ScheduleAsignacionAccion2B.run(Timer timer)");
			// Se indica el usuario al contexto
			initContextUsuarioCore();
			ejecutarProcesoAutomaticoGestionCobro(TipoAccionCobroEnum.B2);
			logger.debug("Fin de método ScheduleAsignacionAccion2B.run(Timer timer)");
		} catch (Exception e) {
			logger.error("Ocurrió un error inesperado en ScheduleAsignacionAccion2B.run(Timer timer)", e);

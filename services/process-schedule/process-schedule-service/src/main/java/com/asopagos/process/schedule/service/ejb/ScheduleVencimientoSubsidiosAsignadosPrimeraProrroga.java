...
@Singleton
@Startup
public class ScheduleVencimientoSubsidiosAsignadosPrimeraProrroga extends ScheduleAbstract {

    /**
     * Instancia del gestor de registro de eventos.
     */
    private static final ILogger logger = LogManager.getLogger(ScheduleVencimientoSubsidiosAsignadosPrimeraProrroga.class);

    /**
     * 
     */
    private static final ProcesoAutomaticoEnum CURRENT_PROCESS = ProcesoAutomaticoEnum.VENCIMIENTO_SUBSIDIO_FOVIS_ASIGNADO_PRIMERA_PRORROGA;
    ...
    public void run(Timer timer) {
        logger.debug("Inicio de método ScheduleVencimientoSubsidiosAsignadosPrimeraProrroga.run(Timer timer)");
        try {
            ...
            solNovedadDTO.setParametro(ParametroFOVISEnum.PLAZO_VENCIMIENTO_PRIMERA_PRORROGA);
            ...
            logger.debug("Fin de método ScheduleVencimientoSubsidiosAsignadosPrimeraProrroga.run(Timer timer)");
        } catch (Exception e) {
            logger.error("Ocurrió un error inesperado en ScheduleVencimientoSubsidiosAsignadosPrimeraProrroga.run(Timer timer)", e);

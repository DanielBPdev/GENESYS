...
@Singleton
@Startup
public class ScheduleAsignacionAccion1B extends ScheduleAbstract {

	/**
	 * Proceso actual
	 */
	private static final ProcesoAutomaticoEnum CURRENT_PROCESS = ProcesoAutomaticoEnum.ASIGNACION_ACCION_1B;
	...
	public void run(Timer timer) {
		try {
			logger.debug("Inicio de método ScheduleAsignacionAccion1B.run(Timer timer)");
			// Se indica el usuario al contexto
			initContextUsuarioCore();
			ejecutarProcesoAutomaticoGestionCobro(TipoAccionCobroEnum.B1);
			logger.debug("Fin de método ScheduleAsignacionAccion1B.run(Timer timer)");
		} catch (Exception e) {
			logger.error("Ocurrió un error inesperado en ScheduleAsignacionAccion1B.run(Timer timer)", e);

...
@Singleton
@Startup
public class ScheduleAsignacionAccion2C extends ScheduleAbstract {

	/**
	 * Proceso actual
	 */
	private static final ProcesoAutomaticoEnum CURRENT_PROCESS = ProcesoAutomaticoEnum.ASIGNACION_ACCION_2C;
	...
	public void run(Timer timer) {
		try {
			logger.debug("Inicio de método ScheduleAsignacionAccion2C.run(Timer timer)");
			// Se indica el usuario al contexto
			initContextUsuarioCore();
			ejecutarProcesoAutomaticoGestionCobro(TipoAccionCobroEnum.C2);
			logger.debug("Fin de método ScheduleAsignacionAccion2C.run(Timer timer)");
		} catch (Exception e) {
			logger.error("Ocurrió un error inesperado en ScheduleAsignacionAccion2C.run(Timer timer)", e);

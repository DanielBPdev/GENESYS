 ...
 * LC2A. Ver <code>AsignacionSolicitudGestionCobroFactory</code> <br/>
 * <b>Historia de Usuario: </b> HU-164
 * 
 * @author <a href="mailto:fvasquez@heinsohn.com.co">Ferney Alonso Vásquez
 *         Benavides</a>
 */
public class AsignacionSolicitudGestionCobroLC2A extends AsignacionSolicitudGestionCobro {

	/**
	 * Logger
	 */
	private final ILogger logger = LogManager.getLogger(AsignacionSolicitudGestionCobroLC2A.class);
	 ...
	 */
	public AsignacionSolicitudGestionCobroLC2A(UserDTO userDTO) {
		super(userDTO);
		this.setAccionCobro(TipoAccionCobroEnum.LC2A);
		this.setTipoParametrizacion(TipoParametrizacionGestionCobroEnum.LINEA_COBRO);
		this.setTipoTransaccionElectronica(TipoTransaccionEnum.ACCION_COBRO_LC2A_ELECTRONICO);
		this.setTipoTransaccionFisica(TipoTransaccionEnum.ACCION_COBRO_LC2A_FISICO);
	}
	...
	public void consultarParametrizacion(List<Object> lista) {
		...
		for (Object registro : lista) {
			...
			if (parametrizacion.containsKey(ConstanteCartera.TIPO_LINEA_COBRO) && parametrizacion.get(ConstanteCartera.TIPO_LINEA_COBRO).toString().equals(TipoLineaCobroEnum.LC2.name())) {
				break;
			}
		}

...
public class TrabajadorIndependiente extends DestinatarioNotificacion{

    /**
     * Referencia al logger
     */
    private final ILogger logger = LogManager.getLogger(TrabajadorIndependiente.class);
    ...
    public List<CorreoPrioridadPersonaDTO> obtenerCorreoDestinatario(TipoTransaccionEnum tipoTx, Map<String, Object> parametros, EntityManager em) {
        ...
        //Persona por solicitud  de afiliacion de una persona,  Persona por solicitud de novedad de persona
        ...
        if (!parametros.containsKey(ID_SOLICITUD)) {
            ...
            if(TipoSolicitudEnum.NOVEDAD_PERSONA.equals(solicitante)){
                try {
                    List<Persona> personas = em.createNamedQuery(NamedQueriesConstants.CONSULTAR_PERSONA_POR_SOLICITUD_NOVEDAD_ROL_AFILIADO, Persona.class)
                ...
                try {
                    List<Persona> personas = em.createNamedQuery(NamedQueriesConstants.CONSULTAR_PERSONA_POR_SOLICITUD_AFILIACION_ROL_AFILIADO, Persona.class)
                            .setParameter("idSolicitud", parametros.get(ID_SOLICITUD)).getResultList();
				...
				try {
					List<Empleador> personasEmpleador = em.createNamedQuery(NamedQueriesConstants.CONSULTAR_PERSONA_POR_SOLICITUD_NOVEDAD_EMPLEADOR_ROL_AFILIADO_TRABAJADOR_INDEPENDIENTE, Empleador.class)
							.setParameter("idSolicitud", parametros.get(ID_SOLICITUD)).getResultList();
					try {
					...
					logger.debug(
							"Finaliza obtenerCorreoDestinatario(TipoTransaccionEnum tipoTx, Map<String, Long> parametros, EntityManager em) :No fue encontrada la solicitud");
					return null;
				} catch (NullPointerException ne) {
					logger.debug(
							"Finaliza obtenerCorreoDestinatario(TipoTransaccionEnum tipoTx, Map<String, Long> parametros, EntityManager em) :La persona no contiene la ubicacion");
					return null;
				}
			} else {

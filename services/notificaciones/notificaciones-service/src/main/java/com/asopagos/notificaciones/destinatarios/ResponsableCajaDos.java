...
public class ResponsableCajaDos extends DestinatarioNotificacion{

    /**
     * Referencia al logger
     */
    private final ILogger logger = LogManager.getLogger(ResponsableCajaDos.class);
        ...
        if (!parametros.containsKey(ID_SOLICITUD)) {
            ...
            if (TipoSolicitudEnum.NOVEDAD_EMPLEADOR.equals(solicitante)) {
                try {
                    ...
                    result = em.createNamedQuery(NamedQueriesConstants.CONSULTAR_ASESOR_RESPONSABLE_EMPLEADOR_DOS_NOVEDAD, Object[].class)
                ...
                try {
                    List<String> nombreUsuarios = new ArrayList<String>();
                    List<Object[]> result = new ArrayList<Object[]>();
                    result = em.createNamedQuery(NamedQueriesConstants.CONSULTAR_ASESOR_RESPONSABLE_EMPLEADOR_DOS_AFILIACION,Object[].class)
                                .setParameter("idSolicitud", parametros.get(ID_SOLICITUD)).getResultList();
                    for (Object[] objects : result) {
						UsuarioCCF usuario = obtenerDatoUsuario(objects[0].toString());
						if (usuario!=null) {
